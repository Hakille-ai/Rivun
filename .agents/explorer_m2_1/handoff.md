...
A detailed investigation of the Milestone 2 codebase (`crates/rivun-store`, `crates/rivun-pack`, `crates/rivun-cli`) and synthesis of the four gate reviewer/challenger reports (`reviewer_m2_1`, `reviewer_m2_2`, `challenger_m2_1`, `challenger_m2_2`) revealed 8 distinct, verified defects across compilation, security, CLI execution, dependency resolution, policy validation, and auditing.

### 1. Workspace Compilation Failures (Struct & Enum Field Mismatches)
- **`DomainPackStatus` Enum**:
  - File: `crates/rivun-store/src/lib.rs:513-518` defines variants `Active`, `Deprecated`, `Revoked`.
  - Usage: `crates/rivun-store/src/bundle.rs:257` attempts to match `"draft" => DomainPackStatus::Draft`. Variant `Draft` is missing.
- **`DomainPackCompatibility` Struct**:
  - File: `crates/rivun-store/src/lib.rs:530-540` defines `min_@@rivun_HEADER@@version`, `max_@@rivun_HEADER@@version`, `runtimes`, `abi_versions`.
  - Usage: `crates/rivun-store/src/resolver.rs:99,104` queries `capabilities_required` and `capabilities_provided`.
  - Usage: `crates/rivun-cli/src/main.rs:7758-7763` and `crates/rivun-store/tests/pack_tests.rs:132-137` attempt to instantiate `@@rivun_HEADER@@version_req`, `abi_version_req`, `capabilities_required`, `capabilities_provided`.
- **`DomainPackRegistryEntry` Struct**:
  - File: `crates/rivun-store/src/lib.rs:553-576` defines `id`, `name`, `version`, `status`, `risk`, `description`, `deprecated_reason`, `revoked_reason`, `compatibility`, `manifest`, `archive`, `policies`, `schemas`, `labels`.
  - Usage: `crates/rivun-cli/src/main.rs:7751-7775`, `crates/rivun-store/tests/pack_tests.rs:125-149`, and `crates/rivun-store/tests/adversarial_m2_tests.rs:103-122` attempt to construct `author_node_id`, `drivers`, `metadata`.
- **`DomainPackArtifact` Struct**:
  - File: `crates/rivun-store/src/lib.rs:542-550` defines `path`, `hash`, `content_type`, `size_bytes`.
  - Usage: `crates/rivun-cli/src/main.rs:7764-7769` and `crates/rivun-store/tests/pack_tests.rs:138-143` instantiate with `relative_path` and `sha256_hex`.

### 2. Critical Security Vulnerability: Zip Slip / Path Traversal in Pack Extraction
- File: `crates/rivun-store/src/bundle.rs:495-510` (`extract_to_dir`)
  ```rust
...
### 3. Hardcoded Facade Check & Missing Signature Error in `rivun pack verify`
- File: `crates/rivun-cli/src/main.rs:7617-7688` (`pack_verify`)
...
### 4. Unenforced Dependency Resolution in `rivun pack install`
- File: `crates/rivun-cli/src/main.rs:7690-7785` (`pack_install`)
...
- File: `crates/rivun-store/src/resolver.rs:40-67` (`matches_version_req`):
...
- File: `crates/rivun-store/src/resolver.rs:120-166` (`resolve_dep`):
...
- File: `crates/rivun-store/src/validator.rs:27,40,57,98,111,128`:
...
- File: `crates/rivun-store/src/audit.rs:25-95` (`audit_pack_dir`):
...
- File: `crates/rivun-store/src/bundle.rs:156-177`:
...
1. **Self-Certifying Work & Build Failure**:
   - Mismatches between struct definitions in `crates/rivun-store/src/lib.rs` and call sites in `resolver.rs`, `main.rs`, and test files prevent `cargo check` and `cargo test` from succeeding.
...
2. **Zip Slip Exploitation**:
   - An attacker crafting a `.zpack` archive with paths like `../../../../tmp/malicious.sh` causes `target_dir.join(rel_path)` to escape `target_dir` during extraction (`rivun pack install`).
...
### STEP 1: Align Structs & Enums in `crates/rivun-store/src/lib.rs`
...
2. Update `DomainPackCompatibility`:
   ...
   pub struct DomainPackCompatibility {
       ...
       pub @@rivun_HEADER@@version_req: String,
       #[serde(default, skip_serializing_if = "String::is_empty")]
...
### STEP 2: Secure `extract_to_dir` Against Zip Slip in `crates/rivun-store/src/bundle.rs`
   ...
   pub fn extract_to_dir(&self, target_dir: &Path) -> Result<(), RivunStoreError> {
       ...
       for (rel_path, content) in &self.files {
           ...
           let canonical_parent = out_path
               .parent()
               .ok_or_else(|| RivunStoreError::InvalidDomainPackArtifactPath(rel_path.clone()))?
               .canonicalize()
               .map_err(|e| RivunStoreError::IoError(e.to_string()))?;
...
In `crates/rivun-store/src/bundle.rs`:
```rust
...
### STEP 4: Fix Resolver SemVer, Fallthrough & Transitive Traversal in `crates/rivun-store/src/resolver.rs`
...
### STEP 5: Enhance Policy Validator & Audit in `crates/rivun-store/`
...
### STEP 6: Fix CLI Handlers in `crates/rivun-cli/src/main.rs`
1. `pack_verify`:
