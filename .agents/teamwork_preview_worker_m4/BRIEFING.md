...
## Mission
Execute Milestone 4: AI Agent Gateway & MCP Server implementation (`crates/zap-gateway`, Cryptographic Provenance Chain Engine, MCP JSON-RPC 2.0 Engine, Multi-Transport Gateway HTTP/SSE/WS, CLI subcommands, E2E tests).
...
## Task Summary
- **What to build**: `crates/zap-gateway` with Provenance Engine ($H_{intent} \dots H_{receipt} \to H_{root}$ + Ed25519 signature), MCP JSON-RPC 2.0 protocol engine (stdio loop, tools, resources, prompts), Multi-transport engine (HTTP, SSE, WebSocket with 4MB max frame size), CLI commands (`zap gateway start`, `zap gateway status`, `zap provenance verify`), and tests.
...
- **Code layout**: `crates/zap-gateway/` and updates to `crates/zap-cli/`, `crates/zap-node/`, `tests/e2e/`.

## Key Decisions Made
- Use native asynchronous networking (`tokio::net`) without heavy web frameworks, adhering to ZAP's performance and determinism principles.
...
## Change Tracker
- **Files modified**:
  - `Cargo.toml`: Added zap-gateway crate to workspace
  - `crates/zap-gateway/`: Complete implementation (config, error, provenance, mcp, transports, server, tests)
  - `crates/zap-cli/src/main.rs`: Added `GatewayCommand` (`start`, `status`), `ProvenanceCommand` (`verify`), updated `ReceiptsCommand::Verify` with `--provenance` flag
  - `crates/zap-cli/tests/gateway_cli_tests.rs`: Added comprehensive CLI integration tests
