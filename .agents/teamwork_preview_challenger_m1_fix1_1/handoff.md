...
Empirical stress testing of Milestone 1 durable replay remediation was conducted by running targeted stress test suites and workspace test targets across `rivun-net`, `rivun-node`, `rivun-journal`, and `rivun-ledger`.

### Test Execution Commands & Results

1. **`cargo test --test durable_replay_stress -p rivun-net -p rivun-node`**:
   - `rivun-net`: 5/5 stress tests passed:
   ...
   - `rivun-node`: 5/5 stress tests passed:
...
2. **Full M1 Crates Test Suite (`cargo test -p rivun-net -p rivun-node -p rivun-journal -p rivun-ledger`)**:
   - Total tests executed: 65
   - Total passed: 65
   - Total failed: 0

3. **M1 Clippy Verification (`cargo clippy -p rivun-net -p rivun-node -p rivun-journal -p rivun-ledger --all-targets -- -D warnings`)**:
...
1. **Partial Write Corruption Recovery**:
   - *Observation*: `DurableNonceStore::open()` (`crates/rivun-net/src/durable_replay.rs:58-63`) and `DurableReplayStore::open()` (`crates/rivun-node/src/durable_replay.rs:52-57`) calculate `valid_len = 8 + (valid_records * RECORD_LEN)` and invoke `file.set_len(valid_len)` when trailing unaligned bytes exist.
...
2. **Clock Jump Overflow Prevention**:
   - *Observation*: `DurableReplayStore::check_and_insert()` (`crates/rivun-node/src/durable_replay.rs:88-89`) uses saturating addition (`ts.saturating_add(self.max_clock_skew_micros) < now_micros || ts > now_micros.saturating_add(self.max_clock_skew_micros)`).
...
3. **Peer WAL Isolation**:
   - *Observation*: `ZapEndpoint::add_peer()` (`crates/rivun-net/src/lib.rs`) constructs distinct per-peer WAL file paths using `{stem}.{peer.node_id}.{ext}` when `durable_nonce_store_path` is specified.
...
- Workspace crates `rivun-net`, `rivun-node`, `rivun-journal`, `rivun-ledger`: 65/65 tests PASS, 0 clippy warnings.
...
1. Run durable replay stress tests:
   ```powershell
   cargo test --test durable_replay_stress -p rivun-net -p rivun-node
   ```
