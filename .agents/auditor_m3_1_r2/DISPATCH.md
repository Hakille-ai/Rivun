...
1. Verify that `FleetDoctor` criteria checks, `IncidentCapturer` process/socket state collection, `SecretRedactor`, `.tar.gz` archive creation, and Prometheus metrics in `crates/rivun-telemetry`, `crates/rivun-node`, and `crates/rivun-cli` are genuinely implemented (zero hardcoded test outputs, facade logic, or cheating).
