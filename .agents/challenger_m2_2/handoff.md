...
Direct code examination and empirical analysis of Milestone 2 (`crates/rivun-store`, `crates/rivun-pack`, `crates/rivun-cli`) yielded the following observations:
...
1. **Corrupt Bundle Detection**:
   - `crates/rivun-store/src/bundle.rs`:
     ...
     - Lines 377-381: Magic header check correctly validates `&bytes[0..8] == ZPACK_MAGIC` and returns `RivunStoreError::InvalidDomainPackBundleFormat("invalid ZPACK magic header")` on mismatch.
     ...
     - Lines 100-146: `DomainPackBundleSignature::verify` validates `expected_bundle_sha256`, node ID derivation (`node_id_from_public_key`), and Ed25519 signature verification over domain `rivun-DOMAIN-PACK-BUNDLE-v1`.
     - Lines 149-181: `verify_against_trusted_keys` validates signer public key against non-empty `trusted_public_keys` array, returning `RivunStoreError::UntrustedDomainPackSigner` on mismatch.

2. **Dependency Resolution Edge Cases**:
   - `crates/rivun-store/src/resolver.rs`:
     - Lines 131-135: `visited_branch.contains(&dep.pack_id)` detects direct self-dependency loops and returns `RivunStoreError::CircularDomainPackDependency(dep.pack_id)`.
     - Lines 149-154: Unsatisfied non-optional dependencies return `RivunStoreError::UnsatisfiedDomainPackDependency { pack_id, requirement }`.
...
1. **[CRITICAL SECURITY] Path Traversal / Zip Slip in `DomainPackBundle::extract_to_dir`**:
   - **Location**: `crates/rivun-store/src/bundle.rs`, line 500:
     ```rust
     ...
     503:             .map_err(|e| RivunStoreError::IoError(e.to_string()))?;
     504:     }
...
2. **[MEDIUM] `audit_pack_dir` ignores `status` field in `pack.toml`**:
   - **Location**: `crates/rivun-store/src/audit.rs`, lines 25-94 vs 97-158.
   - **Observation**: `audit_pack_dir` reads `pack.toml` but only checks `[[capabilities]]`. It ignores `status = "revoked"` or `status = "deprecated"`. In contrast, `audit_bundle` checks `bundle.manifest.status` and sets `overall_risk = Critical` for revoked status. As a result, running `rivun pack audit` on an uncompiled directory of a revoked pack reports `overall_risk: Low` and `passed: true`.

3. **[MEDIUM] `rivun pack verify` returns success when `.sig` file is missing**:
   - **Location**: `crates/rivun-cli/src/main.rs`, lines 7636-7687:
     ```rust
...
4. **[LOW] Version Requirement Resolver Fall-through**:
   - **Location**: `crates/rivun-store/src/resolver.rs`, lines 40-67:
     ```rust
...
3. **CLI verification flaw**: `rivun pack verify` passing when signature file is missing.
...
2. **Verify Path Traversal Flaw**:
   Inspect `crates/rivun-store/src/bundle.rs` line 500. Confirm `target_dir.join(rel_path)` does not validate that `out_path.starts_with(target_dir)`.

3. **Verify Audit Status Flaw**:
   Inspect `crates/rivun-store/src/audit.rs` lines 25-94. Confirm `pack_toml.get("status")` is never queried or checked against `DomainPackStatus::Revoked`.

4. **Verify Pack Verify Flaw**:
   Inspect `crates/rivun-cli/src/main.rs` lines 7636-7687. Confirm `errors` is not populated when `sig_path.exists()` is false.
