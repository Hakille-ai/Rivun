...
### A. Secret Redaction (`crates/zap-telemetry/src/incident.rs:260-437`)
...
### B. POSIX Tar Archive & Gzip Compression (`crates/zap-telemetry/src/incident.rs:477-595`)
...
### C. Fleet Doctor Dynamic Diagnostics (`crates/zap-telemetry/src/doctor.rs`)
...
### D. Process and Socket State Collection (`crates/zap-telemetry/src/incident.rs:13-233`)
- Uses native OS APIs:
...
4. **Observability Parity**: The Prometheus exporter includes all 16 metrics, including dedicated `zap_replay_drops_total` and un-clamped `zap_peers_active`.
...
```bash
# 1. Run zap-telemetry adversarial and unit tests
cargo test -p zap-telemetry --all-targets
