...
1. **`crates/zap-crypto/src/lib.rs` (lines 19-35, 124-245, 448-696)**:
   ...
   - Domain separation constants present: `NODE_ID_DOMAIN = b"ZAP-NODE-ID-v1"`, `SIGN_HINT_DOMAIN = b"ZAP-SIGN-HINT-v1"`, `POA_DIGEST_DOMAIN = b"ZAP-POA-DIGEST-v1"`, `POA_SIGNATURE_DOMAIN = b"ZAP-POA-SIGNATURE-v1"`, `POA_VALIDATOR_SET_SIGNATURE_DOMAIN = b"ZAP-POA-VALIDATOR-SET-v1"`.
   - Node ID derivation: `node_id_from_public_key` derives UUIDv8 via Blake3 (`ZAP-NODE-ID-v1`).
   - `zap-crypto` currently does not expose `BlindedReceiptCommitment`, `BlindedCommitment`, `verify_batch_signatures`, or threshold multi-signature aggregation helpers.

2. **`crates/zap-ledger/src/lib.rs` (lines 6-7, 42-45, 1243-1302)**:
...
3. **`crates/zap-ledger/src/mmr.rs` (lines 28-62, 85-346)**:
   - Defines `MmrHash = [u8; 32]`, `hash_leaf(b"ZAP-MMR-LEAF-v1:")`, `hash_nodes(b"ZAP-MMR-NODE-v1:")`, `bag_peaks(b"ZAP-MMR-PEAK-BAG-v1:")`.
...
4. **`Cargo.toml` & `crates/zap-crypto/Cargo.toml` / `crates/zap-ledger/Cargo.toml`**:
   ...
   - Running `cargo test -p zap-crypto -p zap-ledger` passes with 47 tests passed, 0 failed (16 in `zap-crypto`, 25 in `zap-ledger`, 6 in `m1_challenger_stress`).
...
1. **Threshold Multi-Signatures & Swarm Quorum Seals (Obs 1, Obs 2)**:
   ...
   - Exposing `verify_batch_signatures` and `ThresholdSignatureSet` in `zap-crypto` allows `zap-ledger` to cleanly verify batch seals without duplicating dalek plumbing.
...
2. **Blinded Commitments & ZK Rollups (Obs 1, Obs 3)**:
   ...
   - `BlindedReceiptCommitment` combined with `ZkReceiptBatchProof` in `zap-ledger/src/zk.rs` enables compact proof of execution state transitions without payload disclosure.
...
1. **Cryptographic Foundations**: The existing `zap-crypto` and `zap-ledger` crates have all required dependencies (`ed25519-dalek` with `batch`, `blake3`, `rayon`, `rand_core`) and pass all 47 tests.
2. **Required Deliverables for Implementer**:
   - **`zap-crypto`**: Add `BLINDED_COMMITMENT_DOMAIN`, `BLINDED_RECEIPT_DOMAIN`, `BATCH_SEAL_DOMAIN`, `BlindedCommitment`, `BlindedReceiptCommitment`, and `verify_batch_signatures`.
   - **`zap-ledger/src/mmr.rs`**: Add `IncrementalMmr` ($O(\log N)$ peak accumulator with `.zmmr` persistence), `MmrBatchInclusionProof` (deduplicated sister DAG), and `MmrExclusionProof` (non-membership proofs).
   - **`zap-ledger/src/batch.rs`**: Add `ReceiptBatchSeal`, `BatchValidatorSignature`, and `SignedReceiptBatch` with threshold quorum validation.
   - **`zap-ledger/src/zk.rs`**: Add `ZkReceiptBatchProof` and `ZkRollupPublicInputs` with `generate_rollup` and `verify`.
