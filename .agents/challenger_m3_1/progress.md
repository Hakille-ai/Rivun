...
- [x] Inspect implementation files in `crates/zap-telemetry`, `crates/zap-node`, `crates/zap-cli`
- [x] Run existing tests in `zap-telemetry`, `zap-node`, `zap-cli`, and workspace
...
- [x] Empirically verify Prometheus metrics parity (including `zap_replay_drops_total` and escaping)
- [x] Verified `cargo clippy -p zap-telemetry -- -D warnings` runs cleanly
