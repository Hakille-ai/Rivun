...
### 1.1 `crates/rivun-agent` Existing State
- **File**: `crates/rivun-agent/src/lib.rs` (1190 lines).
  ...
  - Tests in `rivun-agent`: All 9 unit tests and 6 fixture tests pass cleanly (`cargo test --package rivun-agent` exits 0).
...
### 1.2 `crates/rivun-gateway` Non-Existence
- **Workspace Manifest**: `Cargo.toml` lines 1-28 does NOT contain `"crates/rivun-gateway"`.
- **Filesystem**: Search for `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\rivun\crates\rivun-gateway` failed with error: `search directory ... does not exist`.
- `rivun-gateway` must be newly created to fulfill R4 requirements as specified in `PROJECT.md` line 60 (`crates/rivun-agent/ & crates/rivun-gateway/: Agent protocol, MCP server, transport bridge`).

### 1.3 Missing Provenance Fields in `crates/rivun-ledger`
- **File**: `crates/rivun-ledger/src/lib.rs` lines 155-176 (`pub struct ActionReceipt`).
...
### 1.4 Missing CLI Subcommands in `crates/rivun-cli`
- **File**: `crates/rivun-cli/src/main.rs` lines 91-281 (`enum Commands`).
  ...
  - **Missing**: `Gateway` subcommand (`rivun gateway start`, `rivun gateway status`) and `Provenance` verification command (`rivun provenance verify` or `rivun receipts verify --provenance`).

### 1.5 Facades and Compilation Errors in `tests/e2e`
- **Compilation Failure**: `cargo test --workspace` fails in `tests/e2e/tests/e2e_suite.rs` with 71 errors due to type signature mismatches (e.g. `AgentIntent` struct fields in test mismatched with `@@rivun_HEADER@@agent::AgentIntent`, `ReceiptReplicationRequest` fields mismatched with `@@rivun_HEADER@@ledger`, `DriverManifest::new` parameter mismatches).
...
1. **Requirement R4 & Milestone M4 Scope** (from `ORIGINAL_REQUEST.md` and `PROJECT.md`):
   - M4 requires a production-ready AI Agent Gateway and Model Context Protocol (MCP) server connecting LLMs to rivun's deterministic policy, Proof-of-Action (PoA) consensus, WASM execution engine, and signed receipt ledger.
...
2. **Crate Boundary & Architectural Decomposition**:
   - `rivun-agent` is the contract layer (pure data structures, validation rules, JSON schema).
   - `rivun-gateway` must be the active transport and execution bridge crate, depending on `rivun-agent`, `rivun-core`, `rivun-crypto`, `rivun-policy`, `rivun-capability`, `rivun-runtime`, `rivun-ledger`, `rivun-memory`, `rivun-telemetry`, and `rivun-node`.
   - `rivun-gateway` architecture must provide:
     1. **`mcp` module**: Stdio and network JSON-RPC 2.0 protocol engine:
        ...
        - `tools/list` & `tools/call` for execution tools: `@@rivun_HEADER@@send_transaction`, `@@rivun_HEADER@@query_state`, `@@rivun_HEADER@@get_fleet_health`, `@@rivun_HEADER@@inspect_pack`, `@@rivun_HEADER@@intent_create`, `@@rivun_HEADER@@delegate`.
        - `resources/list` & `resources/read` for URIs: `rivun://ledger/receipts`, `rivun://memory/{namespace}/{subject}`.
...
3. **Receipt Ledger & ActionReceipt Integration**:
   - `crates/rivun-ledger` must be updated to include optional provenance fields on `ActionReceipt`:
...
4. **CLI Subcommand Integration**:
   - `crates/rivun-cli` must expose:
     - `rivun gateway start [--port <u16>] [--bind <addr>] [--config <rivun.toml>] [--mcp-stdio]`
     - `rivun gateway status [--port <u16>]`
     - `rivun provenance verify --receipt <file.json> / --chain <digest>`
...
1. **WASM Runtime Dependency**: Driver execution in `rivun-gateway` depends on `rivun-runtime` and `wasmtime`. In test environments where valid WASM bytecode is needed, minimal valid WAT (`(module (func (export "@@rivun_HEADER@@execute") ...))`) should be supplied.
...
1. `crates/rivun-agent` provides a solid, tested foundation of protocol contracts, but **Milestone 4 is currently 0% implemented in terms of runtime gateway, MCP JSON-RPC 2.0 handlers, multi-transport servers, and provenance chain verification**.
2. A new workspace crate **`crates/rivun-gateway`** must be created and registered in `Cargo.toml`.
3. `crates/rivun-ledger` must be extended with provenance fields in `ActionReceipt`.
4. `crates/rivun-cli` must be extended with `gateway` and `provenance` subcommands.
...
1. **Unit & Integration Tests in `rivun-gateway`**:
   ```powershell
   cargo test --package rivun-gateway
   ```
   Must verify:
   - MCP `initialize`, `tools/list`, `tools/call` for `@@rivun_HEADER@@send`, `@@rivun_HEADER@@query`, `@@rivun_HEADER@@get_fleet_health`, `@@rivun_HEADER@@inspect_pack`.
   - MCP `resources/list`, `resources/read` for `rivun://ledger/receipts` and `rivun://memory/...`.
...
2. **CLI Commands**:
   ```powershell
   cargo run --package rivun-cli --bin rivun -- gateway --help
   cargo run --package rivun-cli --bin rivun -- provenance --help
   ```

3. **E2E Test Suite Validation**:
   ```powershell
   cargo test --package rivun-e2e
   ```
