...
- **Path Traversal / Zip-Slip in `DomainPackBundle::extract_to_dir`**:
  - File: `crates/zap-store/src/bundle.rs`, lines 495–510:
    ```rust
...
- **Missing Dependency Verification in `zap pack install`**:
  - File: `crates/zap-cli/src/main.rs`, lines 7690–7785:
...
- **Incomplete Transitive Dependency Resolution**:
  - File: `crates/zap-store/src/resolver.rs`, lines 158–165:
    ```rust
...
- **Policy Validator Bypass via File Naming**:
  - File: `crates/zap-store/src/validator.rs`, lines 27, 40, 57:
    ```rust
...
- **Version Requirement Fallthrough & SemVer Caret Bug**:
  - File: `crates/zap-store/src/resolver.rs`, lines 40–67:
...
- **Public Key Format Inconsistency in `verify_against_trusted_keys`**:
  - File: `crates/zap-store/src/bundle.rs`, line 167:
    ```rust
...
1. **Path Traversal Vulnerability**:
   ...
   - *Deduction*: An attacker can build a `.zpack` bundle with `rel_path` equal to `../../../../tmp/evil` or `../system_file`, leading to arbitrary file write outside `target_dir` during `zap pack install`.
...
- Verification was conducted empirically via static analysis and by creating a dedicated Rust test harness file (`crates/zap-store/tests/adversarial_m2_tests.rs`).
...
2. **Missing Requirement**: Dependency resolution is omitted during `zap pack install`.
...
`crates/zap-store/tests/adversarial_m2_tests.rs`
