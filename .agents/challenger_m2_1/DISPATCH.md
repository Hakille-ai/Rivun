...
1. Verify `rivun pack` CLI commands (`init`, `build`, `sign`, `verify`, `install`, `audit`, `validate`, `inspect`, `list`).
2. Test `DomainPackBundle` container binary integrity, SHA-256 artifact verification, detached Ed25519 signature verification under domain `rivun-DOMAIN-PACK-BUNDLE-v1`.
