...
- **Observation**:
  - `FleetDoctor::evaluate()` now executes authentic evaluation logic across all 6 criteria categories:
    ...
    - **`pack_registry`** (`doctor.rs:202-219`, `433-515`): Inspects `registry.json` and `.zapstore/index.json`, parses `DomainPackRegistry` or `DriverRegistry`, and verifies signatures via `verify_signature()`.
...
- **Verification**: Verified via `cargo test -p zap-telemetry --test telemetry_tests test_fleet_doctor_evaluation_6_criteria` and `test_fleet_doctor_evaluation_corrupted_wal_and_manifests` (PASSED).

### 2. Secret Redactor Security & Format Preservation (`crates/zap-telemetry/src/incident.rs`)
- **Observation**:
...
- **Verification**: Verified via `cargo test -p zap-telemetry --test adversarial_m3_tests test_adversarial_secret_redactor_leaks` (PASSED).

### 3. Live Process & Socket Diagnostics (`crates/zap-telemetry/src/incident.rs`)
- **Observation**:
...
- **Verification**: Verified via `cargo test -p zap-telemetry --test adversarial_m3_tests test_adversarial_process_and_socket_state_hardcoding` (PASSED).

### 4. RFC 1952 Gzip Tarball Archives (`crates/zap-telemetry` & `crates/zap-cli`)
- **Observation**:
  ...
  - `zap-cli` (`crates/zap-cli/src/main.rs:3620-3657`) auto-detects gzip archive targets (`.tar.gz`, `.tgz`, `--format tar.gz`) and writes compressed archives.
- **Verification**: Verified via `cargo test -p zap-telemetry --test adversarial_m3_tests test_adversarial_tar_builder_unpacking_and_gzip` and `cargo test -p zap-cli tests::test_cli_incident_snapshot_tar_gz` (PASSED).

### 5. Prometheus Metrics Parity & Counter Accuracy (`crates/zap-node` & `crates/zap-telemetry`)
- **Observation**:
  - `replay_drops_total: u64` added to `ZapNodeMetricsSnapshot` (`metrics.rs:49`), `NodeMetricsCounters` (`zap-node/src/lib.rs:1514`), and exported as `zap_replay_drops_total` (`metrics.rs:203-208`).
  - In `ZapNode::metrics_snapshot()`, the fallback that reported configured peer count when active peers was 0 was eliminated; `peers_active` now accurately reflects `counters.peers_active` (`zap-node/src/lib.rs:1900`).
  - `ZapNode::record_replay_drop()` (`zap-node/src/lib.rs:2248-2253`) increments both `replay_drops_total` and `replay_rejections_total`.
- **Verification**: Verified via `cargo test -p zap-telemetry --test telemetry_tests test_metrics_parity_all_16_metrics` and `cargo test -p zap-node` (75 unit tests + 5 stress tests = 80 passed).
...
```bash
# 1. Verify zap-telemetry unit and adversarial tests
cargo test -p zap-telemetry

# 2. Verify zap-node unit and durable replay stress tests
cargo test -p zap-node

# 3. Verify zap-cli doctor and snapshot command tests
...
cargo test --workspace --exclude zap-e2e
```
