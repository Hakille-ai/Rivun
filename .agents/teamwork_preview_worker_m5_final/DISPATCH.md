...
Read Explorer Blueprint at c:\Users\Stagiaire\Documents\Amadou PGC\Prs\ZAP\.agents\explorer_m5\handoff.md
...
1. Go SDK Parity (`sdks/go/zapstore.go` & `protocol_test.go`): Add `ReceiptReplicationResponseBody`, `ReceiptSample`, receipt validation helpers, protocol constants, and signature message builder. Update `protocol_test.go`.
...
3. Fix CLI Gateway Status Test Race (`crates/zap-cli/tests/gateway_cli_tests.rs`): Add 50ms startup delay in `test_cli_gateway_status_query`.
...
5. Run Workspace Final Verification Commands:
   ...
   - Golden fixture verification across Rust, TS, Python, and Go SDK targets (`cargo run -p zap-cli -- fixtures verify --fixtures fixtures --sdk <path>`).
