...
2. **`crates/rivun-node/src/durable_replay.rs`**:
...
5. **`crates/rivun-ledger/src/lib.rs`**:
...
6. **`crates/rivun-journal/tests/m1_journal_stress.rs`**:
   - Fixed `clippy::manual_is_multiple_of` lint (`i % 2 == 0` -> `i.is_multiple_of(2)`).

7. **`crates/rivun-ledger/tests/m1_challenger_stress.rs`**:
...
- `cargo test -p rivun-net -p rivun-node -p rivun-journal -p rivun-ledger`: 65 tests passed, 0 failures.
- `cargo clippy -p rivun-net -p rivun-node -p rivun-journal -p rivun-ledger --all-targets -- -D warnings`: 0 errors, 0 warnings.
