...
- **`Cargo.toml` Workspace Members** (`Cargo.toml:1-28`):
  - Currently includes: `crates/zap-agent`, `crates/zap-capability`, `crates/zap-cli`, `crates/zap-core`, `crates/zap-crypto`, `crates/zap-driver-sdk`, `crates/zap-envelope`, `crates/zap-journal`, `crates/zap-ledger`, `crates/zap-machine`, `crates/zap-memory`, `crates/zap-net`, `crates/zap-node`, `crates/zap-ops`, `crates/zap-pact`, `crates/zap-pack`, `crates/zap-policy`, `crates/zap-router`, `crates/zap-runtime`, `crates/zap-schema`, `crates/zap-store`, `crates/zap-telemetry`, `examples`, `tools/xtask`, `tests/e2e`.
  - **`crates/zap-gateway` does NOT exist** on the filesystem and is missing from `Cargo.toml`.
- **`crates/zap-agent`** (`crates/zap-agent/src/lib.rs:1-1190`):
  ...
  - Supported subjects: `zap.agent.intent`, `zap.agent.session`, `zap.agent.delegation.request`, `zap.agent.delegation.response`, `zap.agent.capability_negotiation.request`, `zap.agent.capability_negotiation.response`, `zap.agent.status`, `zap.agent.result`, `zap.agent.error`.
...
- **Requirements from `PROJECT.md` & `ORIGINAL_REQUEST.md` (R4, F09)**:
  ...
  - Mandatory methods:
    1. `initialize`: Returns protocol version (`2024-11-05`), server capabilities (`tools`, `resources`, `prompts`), and server info (`name: "zap-gateway"`, `version: "0.1.0"`).
    2. `tools/list`: Exposes ZAP execution capabilities as callable tools:
       - `zap_send`: Send typed messages / envelopes to ZAP nodes.
       - `zap_query`: Query node status, capabilities, or journal.
       - `zap_agent_intent`: Submit an `AgentIntent` through the policy and driver pipeline.
       - `zap_receipts_verify`: Verify cryptographic receipts and provenance chains.
    ...
    4. `resources/list` & `resources/read`: Exposes read-only audit streams and URIs:
       - `zap://ledger/receipts`: Read recent action receipts.
       - `zap://node/status`: Read node health, peer count, and runtime stats.
       - `zap://fleet/topology`: Read discovered cluster peer nodes.
...
- **Status in Codebase**:
  - `zap_telemetry` (`metrics.rs:56, 261`) and `zap_node` (`lib.rs:1521, 2306`) declare counter `provenance_verification_failures_total`.
...
- **`tests/e2e/tests/e2e_suite.rs`**:
  ...
  - Several tests (`tc_f10_001`, `tc_rw_001`) use struct field patterns that do not match `crates/zap-agent/src/lib.rs` (e.g. `session_id: Some(Uuid)` vs `session_id: Uuid`, `IntentPriority` vs `Priority`).
...
1. **Requirement R4 & PROJECT.md Interface Contracts**:
   - `ORIGINAL_REQUEST.md` (R4) demands MCP and streaming/HTTP/WebSocket bridge interfaces connecting LLM agent frameworks to ZAP's deterministic policy, PoA, and signed receipt ledger, plus strict cryptographic provenance linking.
   - `PROJECT.md` Section "Interface Contracts" specifies:
     - `zap-agent ↔ zap-gateway (Provenance Chain)`: `ProvenanceChainDigest`: SHA256/Ed25519 chain ($H_{\text{intent}} \to H_{\text{negotiation}} \to H_{\text{policy}} \to H_{\text{driver}} \to H_{\text{poa}} \to H_{\text{receipt}}$).
     - `crates/zap-agent/` & `crates/zap-gateway/`: Agent protocol, MCP server, transport bridge.

2. **From Observations to Root Implementation Needs**:
   - **Step 1 — Create `zap-gateway` crate**: Create `crates/zap-gateway` (or module within workspace) and register in root `Cargo.toml` workspace members and dependencies.
   - **Step 2 — Implement Provenance Chain in `zap-agent` (or `zap-ledger`)**: Define `ProvenanceChainDigest`, `ProvenanceLink`, `ProvenanceStage`, step-by-step verification method `ProvenanceChainDigest::verify(...)`, and tamper detection logic.
   - **Step 3 — Implement JSON-RPC 2.0 MCP Server in `zap-gateway`**:
     ...
     - Connect `tools/call` to `zap-node` / `zap-policy` / `zap-ledger` execution pipelines.
   - **Step 4 — Implement Multi-Transport HTTP / SSE / WebSocket Gateway in `zap-gateway`**:
...
1. **Standalone Crate vs Internal Modules**:
   - `PROJECT.md` specifies `crates/zap-gateway` alongside `crates/zap-agent`. Creating `crates/zap-gateway` as a clean workspace crate ensures separation of concerns (protocol types in `zap-agent`, network/transport/server engines in `zap-gateway`).
...
Milestone 4 requires four primary deliverables across `crates/zap-agent`, `crates/zap-gateway`, and `tests/e2e`:

1. **`crates/zap-gateway` Creation**:
...
2. **`ProvenanceChainDigest` in `crates/zap-agent`**:
...
3. **Integration with `zap-node` and `zap-ledger`**:
...
1. **Unit Tests**:
   - `cargo test -p zap-agent --lib` — verifies agent protocol models, serialization, and `ProvenanceChainDigest` cryptographic link verification.
   - `cargo test -p zap-gateway --lib` — verifies MCP JSON-RPC 2.0 dispatch, HTTP REST request handlers, SSE framing, and WebSocket bridge.
2. **E2E Integration Tests**:
   - `cargo test --package zap-e2e --test e2e -- tc_f09` — verifies MCP server initialize, tools, resources, prompts.
   - `cargo test --package zap-e2e --test e2e -- tc_f10` — verifies REST intent submission, SSE event streaming, WebSocket bridge.
   - `cargo test --package zap-e2e --test e2e -- tc_f11` — verifies 6-step cryptographic provenance chain generation, link tampering detection, and verification.
   - `cargo test --package zap-e2e --test e2e -- tc_rw_001` — verifies full distributed AI agent workflow from intent to signed receipt.
