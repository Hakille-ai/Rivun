...
- `crates/zap-net`: Encrypted UDP transport, static peer management, Noise handshake.
- `crates/zap-node`: Daemon, event loop, frame dispatch, policy evaluation, runtime execution, security configs.
- `crates/zap-journal`: Low-level binary journal engine (`.zjseg`, `.zjidx`, `.zjmanifest.json`).
- `crates/zap-ledger`: Signed action receipts (`SignedActionReceipt`), receipt journal store (`ReceiptJournalStore`), segment manifests (`SignedReceiptSegmentManifest`), and segment indexes (`ReceiptSegmentIndex`).
- `crates/zap-cli`: CLI binary (`zap`), subcommands for node administration, keygen, trust, pact, policy, pack, registry, receipts, memory, etc.
- `crates/zap-store`: Registry index engine (`DriverRegistry`, `DomainPackRegistry`), driver manifests, publication, bundle manifests, install plans.
- Other member crates: `zap-agent`, `zap-capability`, `zap-core`, `zap-crypto`, `zap-driver-sdk`, `zap-envelope`, `zap-machine`, `zap-memory`, `zap-ops`, `zap-pact`, `zap-policy`, `zap-router`, `zap-runtime`, `zap-schema`.
...
3. **Journal Framing & Segment Manifests in `zap-journal`**:
   - File: `crates/zap-journal/src/lib.rs`
...
4. **Receipt Ledger & Manifest Signing in `zap-ledger`**:
   - File: `crates/zap-ledger/src/lib.rs`
   - `ReceiptJournalStore` (lines 435–584) wraps `JournalStore` for `JournalProfile::Receipts`.
   - `ReceiptSegmentManifest` (lines 587–677) and `SignedReceiptSegmentManifest` (lines 679–760) define segment manifest creation and Ed25519 signing over domain `ZAP-RECEIPT-SEGMENT-MANIFEST-v1`.
...
1. **Domain Pack CLI in `zap-cli`**:
   - File: `crates/zap-cli/src/main.rs` (lines 1066–1088, 7341–7588)
...
2. **Domain Pack Registry in `zap-store`**:
   - File: `crates/zap-store/src/lib.rs` (lines 485–568, 2066–2255)
   ...
   - `DomainPackRegistry::sign` and `verify_signature` implement operator signing/verification of the domain pack registry index over domain `ZAP-DOMAIN-PACK-REGISTRY-v1`.
...
1. **Replay Protection Durability Gap**:
   - *Observation*: Both `NonceReplayCache` (`zap-net`) and `ReplayGuard` (`zap-node`) store seen datagram nonces and frame fingerprints in standard memory datastructures (`HashSet`, `VecDeque`).
...
3. **Domain Pack Tooling & Lifecycle Gap**:
   - *Observation*: `zap pack` in `zap-cli` currently supports `validate`, `inspect`, and `list`.
   ...
   - *Conclusion*: 6 CLI subcommands (`zap pack init`, `build`, `sign`, `verify`, `install`, `audit`) are missing and must be implemented.

4. **ZapStore Registry & Bundle Resolution Gap**:
   - *Observation*: `zap-store` defines `DomainPackRegistry` structs and operator signing, while `DriverRegistry` handles WASM driver manifest publishing, install plans (`RegistryInstallPlan`), and driver bundle manifests (`RegistryBundleManifest`).
   ...
   - *Conclusion*: ZapStore integration for domain packs requires offline bundle verification (`DomainPackBundle`) and a dependency resolution engine.
...
- **Assumptions**: The presence of `SignedReceiptSegmentManifest` in `zap-ledger` and `DomainPackRegistry` in `zap-store` indicates these types were designed as foundation structures for R1 and R2 requirements.
...
### Summary Assessment
The codebase has clean, modular foundational structures for cryptographic operations (`zap-crypto`), binary journal storage (`zap-journal`), action receipt signing (`zap-ledger`), and driver registry management (`zap-store`). However, critical functional gaps exist for Requirements R1 and R2:

### Detailed Feature Gaps

#### Requirement R1 Gaps:
1. **Durable Replay Protection**:
   - Persistent disk backing for `NonceReplayCache` (`zap-net`) and `ReplayGuard` (`zap-node`).
...
#### Requirement R2 Gaps:
1. **Domain Pack CLI Tooling (`zap pack`)**:
   - `zap pack init`: Initialize domain pack directory structure with template `pack.toml`, policy, schema, and driver files.
   - `zap pack build`: Package domain pack directory into an archive bundle (`.zpk` / `.zap-pack.tar.gz`).
   - `zap pack sign`: Sign domain pack manifest and bundle with Ed25519 key.
   - `zap pack verify`: Verify domain pack signature, artifact hashes, and offline bundle integrity.
   - `zap pack install`: Install domain pack into ZAP node configuration/store with policy and route validation.
   - `zap pack audit`: Perform security audit of capability requirements, risk levels, and policy permissions.
2. **ZapStore Domain Pack Registry & Offline Verification**:
...
- Findings regarding segment sealing are invalidated if `JournalStore` auto-signing hooks exist elsewhere in `zap-ops` or `zap-node`.
