...
1. **`crates/zap-pact/src/lib.rs`**:
   ...
   - Lines 201–207: Signing uses single Ed25519 keypair domain `ZAP-PACT-v1`.
...
2. **`crates/zap-policy/src/lib.rs`**:
...
3. **`crates/zap-agent/src/provenance.rs`**:
...
4. **`crates/zap-cli/src/main.rs`**:
   - Lines 92–292: `enum Commands` defines 26 commands, including `Keygen`, `Run`, `Pact`, `Policy`, `Gateway`, `Poa`, `Bench`, `Provenance`, but completely lacks `Cluster` (`zap cluster`) and `Swarm` (`zap swarm`).
   - Lines 1773–1779: `BenchCommand` only supports `Parse { iterations: u64 }`.
5. **`crates/zap-telemetry` & `crates/zap-net`**:
   - `crates/zap-telemetry/src/metrics.rs` (lines 37–58) exports 17 Prometheus metrics for node internals, but lacks consensus throughput, latency histograms (p50/p95/p99), and swarm benchmarking metrics.
   - `crates/zap-net/src/gossip.rs` (lines 35–147) provides `VectorClock`, `QuorumProposal` (2/3 threshold), and partition detection, serving as a solid base for cluster simulation.
6. **Fixtures & SDKs**:
   - `fixtures/pact-record-v1.json` and `fixtures/pact-bundle-v1.json` lock the v1 single-party JSON schema and Ed25519 signature domain (`ZAP-PACT-v1`).
   - Multi-language SDKs (`sdks/python`, `sdks/go`, `sdks/typescript`, `sdks/rust`) implement and verify this schema in their respective test suites (e.g. `sdks/go/protocol_test.go:258`, `sdks/python/src/zap_sdk/zapstore.py:406`).
...
1. **R4 Architecture:** Ready for implementation by introducing `MultiPartyPact`, `PactEscrowLock`, `PactDisputeRecord`, `DisputeMediationResult` in `zap-pact`, `PolicySet::evaluate_dispute()` in `zap-policy`, and causal chain stage extensions in `zap-agent::provenance`.
2. **R5 Architecture:** Ready for implementation by adding `ClusterCommand` and `SwarmCommand` to `zap-cli`, extending `zap-telemetry` with benchmark metric collectors, building the chaos transport adapter in `zap-net`, and writing high-concurrency benchmarks validating 10,000+ ops/sec in `benches/` and `tests/e2e`.
3. Full technical details, equations, data structures, and CLI arguments are cataloged in `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\ZAP\.agents\explorer_survey_3\analysis.md`.
...
2. **Golden Protocol Fixture Verification:**
   ```powershell
   cargo test --package zap-agent --test fixtures
   cargo test --package zap-cli --test pack_cli_tests
   ```
