...
- **Interface contracts**: crates/zap-telemetry, crates/zap-node, crates/zap-cli, crates/zap-store
...
## Change Tracker
- `crates/zap-telemetry/src/doctor.rs`: Dynamic FleetDoctor checks across all 6 categories.
- `crates/zap-telemetry/src/incident.rs`: Process/socket collection, secret redactor, TarBuilder, gzip archive.
- `crates/zap-telemetry/src/metrics.rs`: `zap_replay_drops_total` export.
- `crates/zap-node/src/lib.rs`: Metrics parity cleanup, health status definitions.
- `crates/zap-cli/src/main.rs`: Gzip incident snapshot support and key loading fixes.
- `crates/zap-store`: Resolved compilation and clippy issues.

## Quality Status
- **Build/test result**: PASS (`cargo test -p zap-telemetry -p zap-node -p zap-cli` passed 156 tests).
- **Clippy status**: PASS (`-D warnings` with 0 warnings).
- **Tests added/modified**: `crates/zap-telemetry/tests/adversarial_m3_tests.rs` and `telemetry_tests.rs`.
