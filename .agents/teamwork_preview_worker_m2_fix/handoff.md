...
All 6 remediation steps specified in the Explorer roadmap have been fully implemented across `crates/rivun-store`, `crates/rivun-pack`, and `crates/rivun-cli`:
...
1. **Struct & Enum Alignment (`crates/rivun-store/src/lib.rs`)**:
   ...
   - `DomainPackCompatibility`: Added fields `@@rivun_HEADER@@version_req: String`, `abi_version_req: String`, `capabilities_required: Vec<String>`, `capabilities_provided: Vec<String>`.
   ...
   - Updated struct instantiations in `crates/rivun-cli/src/main.rs:7751-7775`, `crates/rivun-store/tests/pack_tests.rs:125-149`, and `crates/rivun-store/tests/adversarial_m2_tests.rs:103-143`.

2. **Zip Slip Security Fix (`crates/rivun-store/src/bundle.rs`)**:
...
3. **Public Key Parsing Fix (`crates/rivun-store/src/bundle.rs`)**:
...
4. **SemVer & Dependency Resolver Fixes (`crates/rivun-store/src/resolver.rs`)**:
...
5. **Policy Validator & Security Audit Enhancements (`crates/rivun-store/`)**:
