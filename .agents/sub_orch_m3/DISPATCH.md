...
1. Read `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\rivun\.agents\sub_orch_m3\SCOPE.md` and `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\rivun\.agents\explorer_survey_2\analysis.md`.
2. Follow the iteration loop:
   - Worker implements `crates/rivun-driver-sdk` (async traits, zero-copy pinned views) and `crates/rivun-runtime` (async execution engine, streaming ring-buffers, inter-driver IPC, `DriverPipeline`, aggregate fuel budgeting).
