# Handoff Report: Milestone 1 Explorer 1 (`rivun-net` Blueprint)

## 1. Observation
- **Existing `rivun-net` Codebase (`crates/rivun-net/src/lib.rs:1-1140`)**:
...
- **Existing Replay Storage (`crates/rivun-net/src/durable_replay.rs:1-215` & `tests/durable_replay_stress.rs:1-264`)**:
  - `DurableNonceStore` writes 36-byte binary records to a WAL journal (`ZAPNONC1`).
  - Executed `cargo test -p rivun-net`: all 22 unit tests and 5 stress tests passed (`test result: ok. 22 passed; 0 failed`, stress tests passed in 37.30s).
...
- **Downstream Usages**:
  - `rivun-cli`, `rivun-node`, `rivun-agent`, and `tests/e2e` import `Peer`, `TransportKey`, `ZapEndpoint`, `ZapEndpointConfig`, `InboundZap`, `MAX_DATAGRAM_SIZE`.

## 2. Logic Chain
1. **Preserving Backward Compatibility**: Because `ZapEndpoint` is imported across `rivun-cli`, `rivun-node`, and `tests/e2e`, all root public exports in `crates/rivun-net/src/lib.rs` must maintain identical signatures and behavior while delegating or exposing new capabilities.
...
## 3. Caveats
- `rivun-net/Cargo.toml` must add `ed25519-dalek = { workspace = true, features = ["batch", "rand_core"] }` and `rivun-crypto = { path = "../rivun-crypto" }` to enable batch signature verification.
