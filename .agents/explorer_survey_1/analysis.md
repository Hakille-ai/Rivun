...
**Document Reference**: `rivun-SURVEY-R1-2026`  
**Working Directory**: `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\rivun\.agents\explorer_survey_1`  
**Target Crates**: `crates/rivun-net`, `crates/rivun-agent`, `crates/rivun-node`, with cross-cutting integration in `crates/rivun-core`, `crates/rivun-crypto`, `crates/rivun-router`, `crates/rivun-ledger`  
...
The rivun Next-Gen Frontier objective is to transform rivun into an autonomous, hyper-scalable, cross-cluster decentralized execution and verification fabric. 
...
| **Discovery Handling** | Point-to-point query/announce | `rivun.discovery.query`, `rivun.discovery.response`, `rivun.discovery.announce` caching `SignedDiscoveryAdvertisement`. | Passive announcement cache; does not actively gossip advertisements across multi-hop topologies. |
| **Routing** | `RouteTable` (`rivun-router`) | Subject pattern matching (`*`, `prefix.*`), peer unicast, local driver dispatch, drop. | Static routing decisions; no dynamic multi-peer failover, no path quality metrics (RTT, packet loss). |

### 2.4 `crates/rivun-crypto` & `crates/rivun-core`

- **Wire Header (`ZapHeader`)**: 64 bytes fixed big-endian header with magic `@@rivun_HEADER@@`, version 1, flags (`ENCRYPTED`, `PRIORITY`, `REQUIRES_CONSENSUS`, `SIGNED`, `BROADCAST`), source/target UUIDs, timestamp (micros), payload length, and 8-byte `@@rivun_HEADER@@sign` hint.
- **Trailers**:
...
To achieve autonomous swarm self-organization, `rivun-net` must implement a two-tier decentralized P2P gossip layer combining **HyParView / Plumtree style epidemic dissemination** with **Kademlia-inspired peer distance metric** and **SWIM-based peer sampling**.
...
```
       ...
       |  (crates/rivun-net/gossip)  |                     |   (crates/rivun-net/mesh)   |
       +-------------+-------------+                     +-------------+-------------+
                     |                                                 |
   +-----------------+-----------------+             +-----------------+-----------------+
   |                 |                 |             |                 |                 |
...
4. **Anti-Entropy Synchronization**:
   - Periodic digest reconciliation: Every $T_{\text{sync}}$ (default 5s), nodes exchange an **Invertible Bloom Lookup Table (IBLT)** or **Merkle State Digest** of recent state/consensus hashes over topic channels (`rivun.gossip.sync`).
...
1. **Capability Advertisement Gossip**:
   - When a node registers or modifies WASM drivers or policies, it generates a `SignedCapabilityAdvertisement` and broadcasts it on topic `rivun.swarm.capabilities`.
...
### 3.4 Rust Data Structures for Gossip Protocol (`crates/rivun-net/src/gossip/`)

```rust
// Proposed in crates/rivun-net/src/gossip/mod.rs

pub const GOSSIP_MAGIC: [u8; 4] = *b"ZGSP";
pub const GOSSIP_VERSION: u8 = 1;

pub const GOSSIP_TOPIC_CONSENSUS: &str = "rivun.gossip.consensus";
pub const GOSSIP_TOPIC_STATE: &str = "rivun.gossip.state";
pub const GOSSIP_TOPIC_CAPABILITIES: &str = "rivun.gossip.capabilities";
pub const GOSSIP_TOPIC_HEARTBEAT: &str = "rivun.gossip.heartbeat";
pub const GOSSIP_TOPIC_MEMBERSHIP: &str = "rivun.gossip.membership";
...
The rivun Swarm BFT consensus engine provides deterministic, state-machine-replicated decision making across $N$ autonomous nodes under partial synchrony.
...
1. **Phase 1 — Propose (Round $r$, View $v$)**:
   ...
   - Broadcast via `rivun.gossip.consensus`.
...
5. **Phase 5 — Execution & Receipt Attachment**:
   - The frame is executed in WASM runtime, and the `SwarmCommitCertificate` is attached to the action receipt and stored in `rivun-ledger`.
...
### 4.5 Rust Data Structures for Consensus Engine (`crates/rivun-net/src/consensus/`)

```rust
// Proposed in crates/rivun-net/src/consensus/mod.rs
...
Nodes broadcast lightweight encrypted heartbeat probes (`rivun.p2p.heartbeat`) to active peers.
...
Rather than binary up/down timeouts, rivun implements the **Phi Accrual Failure Detector** (Hayashibara et al.):
...
## 6. Integration Architecture with `rivun-agent` and `rivun-node`

### 6.1 `rivun-agent` Integration
...
### 6.2 `rivun-node` Runtime Daemon Refactoring
...
2. **Node Configuration Extensions (`rivun.toml`)**:
...
### Phase 1: Gossip Subsystem & Dynamic Mesh Foundation (`rivun-net`)
1. Implement `crates/rivun-net/src/gossip/` module: `GossipEnvelope`, message deduplication cache, peer exchange (PEX), epidemic fanout dispatcher.
2. Implement `crates/rivun-net/src/mesh/` module: `MeshHeartbeat`, Phi Accrual Failure Detector, partition detector, and dynamic 2-hop relay routing.
3. Unit and property test gossip fanout under simulated packet drops.

### Phase 2: Byzantine Swarm Consensus & Threshold Signatures (`rivun-net`, `rivun-crypto`, `rivun-core`)
1. Define `SwarmConsensusTrailer` (`ZSC1`) in `rivun-core` and threshold signature verification routines in `rivun-crypto`.
2. Implement BFT consensus state machine (`SwarmConsensusEngine`) in `rivun-net/src/consensus/` (Propose $\to$ Prevote $\to$ Precommit $\to$ Finalize).
3. Implement dynamic validator set transitions, bitmask signature encoding, and batch Ed25519 verification.

### Phase 3: Agent & Node Daemon Integration (`rivun-agent`, `rivun-node`, `rivun-cli`)
1. Integrate Swarm Coordinator with `rivun-agent`, updating `ProvenanceChainEngine` stage `Consensus`.
2. Refactor `ZapNode` daemon into concurrent Tokio actor tasks (`GossipTask`, `ConsensusTask`, `MeshTask`, `ExecutionTask`).
3. Add CLI cluster & swarm simulation commands (`rivun cluster up`, `rivun swarm bench`, `rivun swarm partition-test`).
...
---
*Report compiled by Explorer 1 — rivun Next-Gen Frontier Survey Phase.*
