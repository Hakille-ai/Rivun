...
- [x] Inspect `crates/zap-net` (UDP transport, Noise handshake, nonces, durable replay)
- [x] Inspect `crates/zap-agent` (Agent message protocols, provenance chain engine)
- [x] Inspect `crates/zap-node` (ZapNode daemon, handle_once, discovery, PoA verification, routing, durability)
- [x] Inspect related crates (`zap-core`, `zap-crypto`, `zap-router`, `zap-ledger`, etc.)
...
- [x] Deep-dive analysis on R1:
  ...
  - Integration points with `zap-agent` and `zap-node`
