...
Survey Analysis: c:\Users\Stagiaire\Documents\Amadou PGC\Prs\rivun\.agents\explorer_survey_2\analysis.md

MANDATORY INTEGRITY WARNING:
DO NOT CHEAT. All implementations must be genuine. DO NOT hardcode test results, create dummy/facade implementations, or circumvent the intended task. A teamwork_preview_auditor will independently verify your work. Integrity violations WILL be detected and your work WILL be rejected.

Your Mission:
Implement the complete, production-ready, genuine Milestone 3 features across `crates/rivun-driver-sdk` and `crates/rivun-runtime`:

1. `crates/rivun-driver-sdk`:
   ...
   - Comprehensive unit tests in `crates/rivun-driver-sdk`.

2. `crates/rivun-runtime`:
   - `async_engine.rs`: Non-blocking asynchronous WASM driver host execution on Tokio tasks (`AsyncWasmExecutor`), fuel budget tracking, async host function bindings (`rivun.async_stream_read`, `rivun.async_stream_write`, `rivun.async_device_call`), and compiled async module caching.
...
3. Testing and Verification:
   - Run `cargo test -p rivun-driver-sdk` and `cargo test -p rivun-runtime`.
   ...
   - Document all changes and verification results in `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\rivun\.agents\worker_m3_r3\handoff.md`.
