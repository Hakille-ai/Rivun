...
2. Action 1: Update Go SDK (`sdks/go/zapstore.go` & `sdks/go/protocol_test.go`), run `go test ./...`.
3. Action 2: Update Rust SDK (`sdks/rust/src/lib.rs`), run `cargo test -p zap-sdk`.
4. Action 3: Fix CLI gateway status test race (`crates/zap-cli/tests/gateway_cli_tests.rs`).
