...
1. **FleetDoctor Health Checks (`crates/rivun-telemetry/src/doctor.rs`)**:
   - Replaced static/hardcoded `FleetDoctorStatus::Passed` check evaluations with genuine verification logic:
     ...
     - `check_certificate_and_quorum`: Reads configured node keypairs via `@@rivun_HEADER@@crypto::Keypair::from_key_file_toml`, verifies node identity matching, checks quorum threshold satisfiability ($T \le N$), and evaluates active validator peer counts against $T$.
...
4. **Gzip Tarball Archives (`crates/rivun-telemetry` & `crates/rivun-cli`)**:
   ...
   - Updated `crates/rivun-cli/src/main.rs` `incident_snapshot()` to automatically invoke `build_tar_gz_archive()` when format or output extension is `.tar.gz` or `.tgz`.

5. **Prometheus Metrics Parity & Replay Drops Counter (`crates/rivun-node` & `crates/rivun-telemetry`)**:
   - Added `replay_drops_total: u64` to `ZapNodeMetricsSnapshot` and `NodeMetricsCounters`.
   - Exported `@@rivun_HEADER@@replay_drops_total` metric in `ZapNodeMetricsSnapshot::to_prometheus_text()`.
...
- **Step 2**: Genuine evaluation logic was built by linking each doctor check to its domain counterpart: WAL files are checked against `rivun-node`'s `DURABLE_FRAME_MAGIC` (`b"ZAPFRM01"`), journal manifests against `rivun-ledger`'s `SignedReceiptSegmentManifest`, and pack registries against `rivun-store`'s `DomainPackRegistry` / `DriverRegistry`.
...
- **Step 6**: The missing `replay_drops_total` metric was added to `rivun-node` and `rivun-telemetry` to achieve 100% parity across all 16 Prometheus metrics.
...
1. `cargo test -p rivun-telemetry --test telemetry_tests test_fleet_doctor_evaluation_6_criteria`
2. `cargo test -p rivun-telemetry --test adversarial_m3_tests`
3. `cargo test -p rivun-telemetry` (5 unit tests + 3 adversarial tests = 8 passed)
4. `cargo test -p rivun-node` (75 tests passed)
5. `cargo test -p rivun-cli` (78 tests passed)
6. `cargo test --workspace --exclude rivun-e2e` (all 23 workspace crates passed)
7. `cargo clippy --workspace --all-targets --exclude rivun-e2e -- -D warnings` (0 warnings)
