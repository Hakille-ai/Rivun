...
You are the E2E Test Suite Worker (Generation 2) for the ZAP Next-Gen Frontier project.
...
- `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\ZAP\TEST_INFRA.md`
- `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\ZAP\TEST_READY.md`
- `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\ZAP\.agents\worker_e2e_2/**`
...
2. Notice that the project is "ZAP Next-Gen Frontier" which has EXACTLY 15 features in `PROJECT.md § Feature Inventory`:
   ...
   - Feature 11: Dispute Resolution Engine (Deterministic adjudication in zap-policy)
   - Feature 12: Causal Execution Chains (Provenance causal binding: Pact -> Escrow -> Attestation -> MMR)
   - Feature 13: Cluster Simulator CLI (`zap cluster up / status / down`)
   - Feature 14: Swarm Benchmarking Tooling (`zap swarm bench / partition-test`, chaos fixtures)
...
6. Verify everything compiles cleanly and all tests pass by running `cargo test --package zap-e2e`.
7. Write `c:\Users\Stagiaire\Documents\Amadou PGC\Prs\ZAP\.agents\worker_e2e_2\handoff.md` with:
   - Summary of test suite architecture and coverage metrics
   - Verification command results (`cargo test --package zap-e2e` output)
   - Artifact paths
