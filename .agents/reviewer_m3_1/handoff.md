...
Direct inspection of code, tests, and build artifacts across `crates/rivun-telemetry`, `crates/rivun-node`, and `crates/rivun-cli` confirms:

1. **FleetDoctor Verification Logic (`crates/rivun-telemetry/src/doctor.rs:95-255`)**:
   ...
   - `check_pack_registry` (lines 433-515): Inspects `registry.json` and `.RivunStore/index.json` candidates. Parses as `DomainPackRegistry` or `DriverRegistry` and validates signatures with `verify_signature()`. Missing signatures produce `Warning`; invalid signatures or corrupted JSON produce `Failed`.
   - `check_certificate_and_quorum` (lines 517-593): Parses node configuration TOML and key file via `@@rivun_HEADER@@crypto::Keypair::from_key_file_toml(&key_content)`. Validates node ID against derived keypair node ID. Evaluates quorum threshold $T = (N \times 2/3) + 1$, verifying $T \le N$ and active peer count.
...
5. **Prometheus Metrics Parity & Replay Drops Counter (`crates/rivun-node/src/lib.rs:1514,1884,2250` & `crates/rivun-telemetry/src/metrics.rs:49,207`)**:
   - `ZapNodeMetricsSnapshot` contains dedicated `replay_drops_total: u64` alongside `replay_rejections_total: u64`.
   - `to_prometheus_text` exports `@@rivun_HEADER@@replay_drops_total` with full Prometheus metric metadata.
...
6. **Automated Test Results**:
   ...
   - `cargo clippy --workspace --all-targets --exclude rivun-e2e -- -D warnings`: 0 warnings.
...
- **Step 5 (Code Cleanliness)**:
  - Clippy and test suites across `rivun-telemetry`, `rivun-node`, and `rivun-cli` executed with zero errors and zero warnings.
