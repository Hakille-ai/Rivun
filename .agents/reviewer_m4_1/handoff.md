...
### 1.1 `crates/zap-agent` Cryptographic Provenance Chain
- **`crates/zap-agent/src/provenance.rs`**:
  ...
  - Merkle root hash is computed across all stages ($H_{\text{root}} = \text{SHA256}(\sum \text{stage}_i : H_i ;)$) and signed using the node's Ed25519 key over domain `ZAP-PROVENANCE-CHAIN-v1\0{root_hash}`.
...
### 1.2 `crates/zap-gateway` Architecture & Transports
- **MCP Server (`crates/zap-gateway/src/mcp/`)**:
  ...
  - `tools.rs`: Full execution handlers for `zap_send`, `zap_send_transaction`, `zap_query`, `zap_query_state`, `zap_agent_intent`, `zap_receipts_verify`, `zap_verify_provenance`, `zap_get_fleet_health`, `zap_inspect_pack`, and `zap_delegate`, interacting directly with `PolicySet`, `ZapNode`, and `ReceiptJournalStore`.
  - `resources.rs`: Providers for `zap://ledger/receipts`, `zap://node/status`, `zap://fleet/topology`, `zap://fleet/status`, `zap://memory/status`, and `zap://packs/installed`.
...
- **Multi-Transport Router (`crates/zap-gateway/src/transports/`)**:
...
- Running `cargo test -p zap-agent -p zap-gateway --all-targets`:
  - `zap-agent` unit tests: 12 passed, 0 failed
  - `zap-agent` fixture tests: 6 passed, 0 failed
  - `zap-gateway` unit tests: 1 passed, 0 failed
  - `zap-gateway` adversarial challenger tests: 10 passed, 0 failed
  - `zap-gateway` adversarial stress tests: 9 passed, 0 failed
  - `zap-gateway` integration gateway tests: 9 passed, 0 failed
...
1. **Specification Alignment**:
   - `ORIGINAL_REQUEST.md` (R4) requires MCP (Model Context Protocol) and streaming/HTTP/WebSocket bridge interfaces connecting LLM agent frameworks to ZAP's deterministic policy, PoA, and signed receipt ledger, with strict cryptographic provenance linking.
...
- All 47 tests across `zap-agent` and `zap-gateway` pass with 0 failures.
