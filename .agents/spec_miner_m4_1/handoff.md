...
1. **E2E Test Specifications for Milestone 4 (`tests/e2e/tests/e2e_suite.rs`)**:
   - **Feature 9: MCP Server Implementation** (lines 635–700):
     ...
     - `tc_f09_002_tools_list`: Requests `"tools/list"`, expects list of callable rivun execution drivers.
     - `tc_f09_003_tools_call`: Calls `"tools/call"` with params `{"name": "@@rivun_HEADER@@send", "arguments": {"target": "<uuid>", "payload": "hello"}}`.
     - `tc_f09_004_resources_read`: Reads `"resources/read"` with params `{"uri": "rivun://ledger/receipts"}`.
...
2. **Test Infrastructure Expectations (`TEST_INFRA.md:96–116, 191–211, 237–247`)**:
   - **F09 MCP Server**:
     - `TC-F09-001` to `TC-F09-005`: stdio initialization, tools list, tools call (`@@rivun_HEADER@@send`, `@@rivun_HEADER@@query`), resources list/read (`rivun://ledger/receipts`), prompts list/get.
   ...
   - **F11 Provenance**:
     - `TC-F11-001` to `TC-F11-005`: 6 linked hashes ($H_{\text{intent}} \to H_{\text{negotiation}} \to H_{\text{policy}} \to H_{\text{driver}} \to H_{\text{poa}} \to H_{\text{receipt}}$), hash linking equations, PoA link, receipt root embedding, CLI `rivun receipts verify --provenance`.
...
3. **Data Contracts in `crates/rivun-agent/src/lib.rs` & `crates/rivun-ledger/src/lib.rs`**:
   ...
   - `ZapNode` telemetry counters in `crates/rivun-node/src/lib.rs`: `record_agent_gateway_request(transport, status)`, `inc_agent_session()`, `dec_agent_session()`, `record_provenance_verification_failure()`.
...
1. **MCP Server Interface Specification**:
   ...
   - Handlers required:
     ...
     - `tools/list`: Returns array of `Tool` definitions. Built-in tools include `@@rivun_HEADER@@send`, `@@rivun_HEADER@@query`, `@@rivun_HEADER@@get_fleet_health`, `@@rivun_HEADER@@inspect_pack`, `@@rivun_HEADER@@delegate`, `@@rivun_HEADER@@verify_provenance`, plus dynamically registered WASM drivers.
     ...
     - `resources/list`: Returns array of `Resource` definitions for accessible rivun URIs (`rivun://ledger/receipts`, `rivun://memory/{namespace}/{subject}`, `rivun://fleet/status`, `rivun://packs/{name}`).
...
3. **Cryptographic Provenance Chain Specification**:
   ...
   - Verification Rules:
     ...
     5. Any mismatch at any stage triggers an explicit error identifying the exact tampered stage index (0 to 5) and increments the Prometheus counter `@@rivun_HEADER@@provenance_verification_failures_total`.
   - CLI verification: `rivun provenance verify --chain <path> --key <key_file>` and `rivun receipts verify --dir <journal_dir> --provenance`.
...
| 5 | MCP JSON-RPC | `resources/read` | Reads content of a resource URI (`rivun://ledger/receipts`, `rivun://memory/*`, `rivun://fleet/status`) | `ResourceReadParams` (uri string) | `ResourceReadResult` (contents array: uri, mimeType, text) | Returns `-32602` if URI invalid, `-32000` / error if resource not found | `TEST_INFRA.md:100`, `tests/e2e:679` |
...
| 10 | Multi-Transport Gateway | HTTP REST Session Management | Creates, retrieves, and updates `AgentSession` state | `POST /v1/agent/sessions`, `GET /v1/agent/sessions/{id}` | HTTP 200/201 with `AgentSession` JSON body | HTTP 404 if session not found, 400 on invalid session state | `TEST_INFRA.md:104`, `rivun-agent:310` |
| 11 | Multi-Transport Gateway | HTTP REST Negotiation & Delegation | Dispatches multi-agent capability negotiations and task delegations | `POST /v1/agent/negotiate`, `POST /v1/agent/delegate` | HTTP 200 with `CapabilityNegotiationResponse` or `DelegationResponse` | HTTP 400 on invalid payload or rejected delegation without reason | `TEST_INFRA.md:104`, `rivun-agent:364,477` |
...
| 24 | CLI & Telemetry | Gateway CLI Subcommands | Manages agent gateway daemon lifecycle via CLI | `rivun gateway start`, `rivun gateway status` | Daemon start / JSON status output | Clear error if port bound or config invalid | `explorer_m4:26`, `TEST_INFRA.md:104` |
| 25 | CLI & Telemetry | Provenance CLI Verification | Verifies standalone or journal-embedded provenance chains via CLI | `rivun provenance verify --chain <file> --key <key>`, `rivun receipts verify --provenance` | Success output or JSON verification report | Exits non-zero with failure report on corrupted chain | `TEST_INFRA.md:115`, `explorer_m4:28` |
| 26 | CLI & Telemetry | Gateway & Provenance Prometheus Metrics | Exposes real-time gateway requests, active sessions, and provenance error metrics | Prometheus HTTP scrape `GET /metrics` | `@@rivun_HEADER@@agent_gateway_requests_total`, `@@rivun_HEADER@@agent_sessions_active`, `@@rivun_HEADER@@provenance_verification_failures_total` | Accurately tracks counts and gauge increments/decrements | `TEST_INFRA.md:93-94`, `crates/rivun-node:2285` |
...
| 3 | MCP JSON-RPC | Missing required parameter (e.g. `@@rivun_HEADER@@send` without `target`) | Server returns standard JSON-RPC `-32602` Invalid Params error detailing missing field (`TC-B09-003`). |
...
| 13 | Provenance Chain | Tampered intent step hash (1 byte flipped $0\text{xAA}$ in $H_{\text{intent}}$) | Verifier detects mismatch at Stage 0 ($H_{\text{intent}}$), fails verification, and increments `@@rivun_HEADER@@provenance_verification_failures_total` (`tc_b11_001`, `TC-B11-001`). |
...
| 18 | Agent Delegation | `DelegationResponse` with `decision: accepted` but `assigned_agent: None` | Validation rejects struct with `ZapAgentError::AcceptedDelegationMissingAssignee` (`crates/rivun-agent/src/lib.rs:448`). |
| 19 | Agent Delegation | `DelegationResponse` with `decision: rejected` but `reason: None` | Validation rejects struct with `ZapAgentError::RejectedDelegationMissingReason` (`crates/rivun-agent/src/lib.rs:451`). |
| 20 | Agent Negotiation | `CapabilityNegotiationRequest` with empty capabilities and empty intents | Validation rejects request with `ZapAgentError::EmptyCapabilityNegotiation` (`crates/rivun-agent/src/lib.rs:509`). |
| 21 | Agent Result | `AgentResult` with non-terminal status (`running`) or failed status without error | Validation rejects non-terminal status with `ResultStatusNotTerminal`, failed without error with `FailedResultMissingError` (`crates/rivun-agent/src/lib.rs:748-752`). |
| 22 | Agent Session | `AgentSession` with `updated_at_micros < created_at_micros` | Validation rejects inverted timestamps with `ZapAgentError::InvalidTimestampOrder` (`crates/rivun-agent/src/lib.rs:930`). |
| 23 | Agent Error Info | Deeply nested error cause chain exceeding 8 levels ($>8$) | Validation rejects recursion depth with `ZapAgentError::ErrorCauseTooDeep { max: 8 }` (`crates/rivun-agent/src/lib.rs:702`). |

---

## 5. Caveats

1. **No External Heavy Web Frameworks**: In line with the rivun architecture, all HTTP REST, SSE, and WebSocket networking is implemented natively using `tokio::net` and standard library primitives rather than pulling in large frameworks like `axum` or `actix-web`.
...
- **CLI & Telemetry**: Covers `rivun gateway start`, `rivun gateway status`, `rivun provenance verify`, and Prometheus metrics (`@@rivun_HEADER@@agent_gateway_requests_total`, `@@rivun_HEADER@@agent_sessions_active`, `@@rivun_HEADER@@provenance_verification_failures_total`).
...
2. **Inspect Agent Protocol Contracts**:
   - `crates/rivun-agent/src/lib.rs`: inspect lines 84–970 (validation rules for `AgentIntent`, `AgentSession`, `DelegationRequest`, `CapabilityNegotiationRequest`, `AgentResult`, `AgentErrorInfo`).
   - `crates/rivun-agent/tests/fixtures.rs`: verify schema version, declared subjects, and fixture shapes.
3. **Execute Test Commands (when ready)**:
   ```bash
   cargo test --package rivun-agent
