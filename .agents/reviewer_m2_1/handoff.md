...
1. **Compilation Errors (Struct & Enum Mismatches)**:
   - **`crates/zap-store/src/bundle.rs:257`**:
     ```rust
     "draft" => DomainPackStatus::Draft,
     ```
     `DomainPackStatus` in `crates/zap-store/src/lib.rs:513` defines variants `Active`, `Deprecated`, `Revoked`. `Draft` is missing from the enum definition.
   - **`crates/zap-store/src/resolver.rs:99,104`**:
     ...
     ```
     `DomainPackCompatibility` in `crates/zap-store/src/lib.rs:531-540` only defines `min_zap_version`, `max_zap_version`, `runtimes`, and `abi_versions`. Fields `capabilities_required` and `capabilities_provided` do NOT exist.
   - **`crates/zap-cli/src/main.rs:7751-7774` (`pack_install`)**:
     ```rust
     ...
     ```
     Instantiates `DomainPackRegistryEntry` with non-existent fields `author_node_id`, `drivers`, and `metadata`, and `DomainPackCompatibility` with non-existent fields `zap_version_req`, `abi_version_req`, `capabilities_required`, `capabilities_provided`.
   - **`crates/zap-store/tests/pack_tests.rs:132-148`**:
...
2. **Security Vulnerability: Zip Slip / Path Traversal**:
   - **`crates/zap-store/src/bundle.rs:495-510` (`extract_to_dir`)**:
     ```rust
...
3. **Hardcoded Facade Verification in `pack_verify`**:
   - **`crates/zap-cli/src/main.rs:7662-7670`**:
     ```rust
...
4. **Integration Test Shortcomings**:
   - **`crates/zap-cli/tests/pack_cli_tests.rs`**:
     The test function directly calls low-level API functions instead of testing the `zap pack` CLI command handlers or CLI argument parsing.
...
1. **Self-Certifying Work & Build Failure**:
   ...
   - *Reasoning*: Static inspection reveals 4 distinct compilation failure sites in `bundle.rs`, `resolver.rs`, `main.rs`, and `pack_tests.rs` where struct fields and enum variants do not match `crates/zap-store/src/lib.rs`.
...
- **Where**:
  - `crates/zap-store/src/bundle.rs:257` (`DomainPackStatus::Draft`)
  - `crates/zap-store/src/resolver.rs:99,104` (`capabilities_required`, `capabilities_provided`)
  - `crates/zap-cli/src/main.rs:7757-7774` (`author_node_id`, `drivers`, `metadata`, `zap_version_req`, `abi_version_req`)
  - `crates/zap-store/tests/pack_tests.rs:132-148` (same non-existent fields)
- **Why**: Code cannot be compiled or tested.
- **Suggestion**: Update `DomainPackRegistryEntry` and `DomainPackCompatibility` in `crates/zap-store/src/lib.rs` to include all required fields, add `Draft` variant to `DomainPackStatus` if intended, align call sites, and verify compilation with `cargo check`.
...
- **Where**: `crates/zap-cli/src/main.rs:7666`.
...
- **What**: `crates/zap-cli/tests/pack_cli_tests.rs` tests internal APIs instead of CLI subcommands.
...
1. Run `cargo test -p zap-cli -p zap-pack -p zap-store` to confirm workspace compilation and test suite pass rate.
2. Run `cargo clippy --workspace --all-targets -- -D warnings`.
3. Inspect `crates/zap-store/src/bundle.rs` to verify path traversal protections in `extract_to_dir`.
4. Inspect `crates/zap-cli/src/main.rs` to verify `bundle.verify_integrity()` call in `pack_verify`.
