...
- **Core Milestone 4 Functional Features (APPROVED)**:
  ...
  6. Unit & integration test suites in `rivun-agent` and `rivun-gateway`: **47 of 47 tests PASSED**.

- **Required Changes (BLOCKING ACCEPTANCE CRITERIA)**:
  1. **Clippy Errors in `crates/rivun-agent/src/provenance.rs`**: 2 warnings break `cargo clippy --workspace --all-targets -- -D warnings`.
  2. **Compilation Failures in `tests/e2e/tests/e2e_suite.rs`**: Outdated test stubs in later tiers prevent `cargo test --package rivun-e2e --test e2e` from compiling cleanly.
...
1. **`cargo test -p rivun-agent -p rivun-gateway --all-targets`**:
   ...
   - `rivun-agent` fixtures (6 passed, 0 failed)
   - `rivun-gateway` unit tests (1 passed: `transports::ws::tests::test_rfc6455_accept_calculation`)
   - `rivun-gateway` adversarial challenger suite (`adversarial_challenger_m4_2.rs`, 10 passed, 0 failed):
   ...
   - `rivun-gateway` adversarial stress tests (`adversarial_stress_tests.rs`, 9 passed, 0 failed)
   - `rivun-gateway` integration tests (`gateway_tests.rs`, 9 passed, 0 failed)
   - **Total Passed**: 47 tests.

2. **`cargo clippy --workspace --all-targets -- -D warnings` Failure**:
   ```
   error: the borrowed expression implements the required traits
      --> crates\rivun-agent\src\provenance.rs:331:28
       |
   ...
   error: this `if` statement can be collapsed
      --> crates\rivun-agent\src\provenance.rs:579:17
       |
...
3. **`cargo test --package rivun-e2e --test e2e` Compilation Failure**:
...
1. **Cryptographic Provenance Linking ($H_0 \to H_5$)**:
   ...
   - The Merkle root $H_{\text{root}} = \text{SHA256}(\sum \text{stage}:H_k)$ is signed with Ed25519 using domain separator `rivun-PROVENANCE-CHAIN-v1\0`.
...
5. **Acceptance Criteria Discrepancies**:
   - While the functional implementation of Milestone 4 is solid and bug-free, `cargo clippy --workspace --all-targets -- -D warnings` and `cargo test --package rivun-e2e --test e2e` fail. In accordance with the project integrity rules, these must be resolved before final approval.
...
### Required Fixes:
1. **Fix Clippy Warnings in `crates/rivun-agent/src/provenance.rs`**:
...
2. **Clean up / Fix compilation in `tests/e2e/tests/e2e_suite.rs`**:
   - Align the outdated type signatures in `tier3_cross_feature` and `tier4_real_world_workflows` with current crate APIs or feature-gate them so `cargo test --package rivun-e2e --test e2e` compiles cleanly.
...
```bash
# 1. Run rivun-agent and rivun-gateway tests (including adversarial challenger suite)
