...
- [x] Run required commands:
  - [x] `cargo test -p zap-agent -p zap-gateway --all-targets` (47 passed, 0 failed)
  - [x] `cargo test --package zap-e2e --test e2e` (Ran & diagnosed compilation failures in unadapted Tier 3 / Tier 4 test stubs)
  - [x] `cargo clippy --workspace --all-targets -- -D warnings` (Ran & diagnosed 2 clippy warnings in `crates/zap-agent/src/provenance.rs`)
