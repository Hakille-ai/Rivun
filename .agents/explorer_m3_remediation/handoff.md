...
Direct code inspection of `crates/rivun-telemetry`, `crates/rivun-cli`, `crates/rivun-node`, and existing unit/adversarial tests (`crates/rivun-telemetry/tests/adversarial_m3_tests.rs`) revealed four distinct implementation defects:

1. **Hardcoded Facade in `FleetDoctor` Health Criteria (`crates/rivun-telemetry/src/doctor.rs:156-199`)**:
...
2. **Dummy Mock Values in `IncidentCapturer` (`crates/rivun-telemetry/src/incident.rs:146-147`)**:
...
3. **Secret Redactor Bypasses, Leaks & JSON Corruption (`crates/rivun-telemetry/src/incident.rs:65-114`)**:
...
4. **Uncompressed POSIX Tar Bytes in `.tar.gz` Files (`crates/rivun-cli/src/main.rs:3634-3650` & `crates/rivun-telemetry/src/incident.rs:186-259`)**:
   - `TarBuilder` generates raw POSIX ustar tar bytes without gzip compression.
   - `rivun-cli` writes raw uncompressed tar bytes directly to files specified with `.tar.gz` or `.tgz` extensions.
   - Standard archive utilities (`tar -xzf`, `gzip -d`) fail with `not in gzip format`.

5. **Prometheus Metrics Anomalies (`crates/rivun-node/src/lib.rs:1822-1826` & `crates/rivun-telemetry/src/metrics.rs:207`)**:
   ...
   - `@@rivun_HEADER@@replay_drops_total` duplicates `self.replay_rejections_total` instead of emitting a dedicated drop counter.
...
1. **Requirement Integrity**:
   - Requirement R3 and Gate Criteria require strict production `rivun fleet doctor` health verification and live `rivun incident snapshot` diagnostic collection.
...
### Step 1: Real `FleetDoctor` Health Checks (`crates/rivun-telemetry/src/doctor.rs`)
1. In `FleetDoctor::evaluate`, replace hardcoded `FleetDoctorStatus::Passed` responses with genuine checks for all 4 missing categories:
   ...
   - **`pack_registry` check**: Look in `config_path.parent()` / working directory for RivunStore index files (`registry.json`, `.RivunStore/index.json`). Check signature validity (`registry_signature_valid == 1`). If missing or signature invalid, set status to `Warning` / `Failed`. Merge status with `overall_status.merge(...)`.
...
### Step 2: Real System Process & Socket Collection (`crates/rivun-telemetry/src/incident.rs`)
...
### Step 3: Comprehensive `SecretRedactor` (`crates/rivun-telemetry/src/incident.rs`)
...
### Step 4: Gzip Tarball Archive Compression (`crates/rivun-telemetry` & `crates/rivun-cli`)
1. Add `flate2 = "1.0"` to workspace dependencies in `Cargo.toml` and `crates/rivun-telemetry/Cargo.toml` & `crates/rivun-cli/Cargo.toml`.
2. In `crates/rivun-telemetry/src/incident.rs`, implement:
   ```rust
   ...
   ```
3. In `crates/rivun-cli/src/main.rs`:
...
1. In `crates/rivun-node/src/lib.rs:1822`, remove `if counters.peers_active > 0 { ... } else { self.peers.len() as u64 }` fallback so `peers_active` accurately reflects connected peers.
2. In `crates/rivun-telemetry/src/metrics.rs`, add `pub replay_drops_total: u64` to `ZapNodeMetricsSnapshot` and use `self.replay_drops_total` for `@@rivun_HEADER@@replay_drops_total`.
...
1. **Verify `FleetDoctor` Checks**:
   ...
   - Verify that corrupted WAL files, missing manifests, invalid RivunStore index signatures, or invalid node keypairs cause `FleetDoctor::evaluate` to return `Warning` or `Failed`.
...
3. **Verify Gzip Decompression**:
   - Run `cargo run -p rivun-cli -- incident snapshot --out snapshot.tar.gz --force`.
