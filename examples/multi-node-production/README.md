...
This example shows the operator sequence for a small two-node Rivun deployment
...
```bash
cargo run -p rivun-cli -- registry init --out examples/multi-node-production/registry.index.toml
...
cargo run -p rivun-cli -- registry publication create \
  --registry examples/multi-node-production/registry.index.toml \
  --publisher-key .rivun/node-a.key \
...
```bash
cargo run -p rivun-cli -- doctor --config examples/multi-node-production/node-a.toml --strict --json
cargo run -p rivun-cli -- doctor --config examples/multi-node-production/node-b.toml --strict --json
cargo run -p rivun-cli -- capability cache refresh --config examples/multi-node-production/node-a.toml --strict --json
```
...
```bash
RUST_LOG=info cargo run -p rivun-cli -- run --config examples/multi-node-production/node-a.toml
RUST_LOG=info cargo run -p rivun-cli -- run --config examples/multi-node-production/node-b.toml
```

In another terminal, send a typed action:

```bash
cargo run -p rivun-cli -- send \
...
Use the assets in `crates/rivun-ops/config` for production-style monitoring:

- import `grafana/rivun-production-dashboard.json`;
- load `prometheus/rivun-rules.yml`;
