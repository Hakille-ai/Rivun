# ZAP Thermostat WASM Driver Example

This folder contains a WebAssembly Text (WAT) driver showing how to return a static status payload from a sandboxed ZAP driver.
...
When `zap_execute` is called, the driver returns this pre-loaded response.
...
3. **Start the daemon and send an action**:
   ...
   ```bash
   cargo run -p zap-cli -- send --config client.toml --target <gateway-uuid> --action thermostat.setpoint --payload '{"temp": 22}'
   ```
