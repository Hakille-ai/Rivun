...
REGISTRY_INDEX_CONTENT_TYPE = "application/rivun-registry-index+json"
REGISTRY_BUNDLE_MANIFEST_CONTENT_TYPE = "application/rivun-registry-bundle-manifest+json"
REGISTRY_INDEX_REQUEST_SUBJECT = "rivun.registry.index.request"
REGISTRY_INDEX_RESPONSE_SUBJECT = "rivun.registry.index.response"
REGISTRY_BUNDLE_MANIFEST_REQUEST_SUBJECT = "rivun.registry.bundle.manifest.request"
REGISTRY_BUNDLE_MANIFEST_RESPONSE_SUBJECT = "rivun.registry.bundle.manifest.response"

NIL_UUID = UUID(int=0)


class RivunMessageKind(IntEnum):
    DATA = 1
    EVENT = 2
    COMMAND = 3
    QUERY = 4
    ...
    def protocol_name(self) -> str:
        return {
            RivunMessageKind.DATA: "data",
            RivunMessageKind.EVENT: "event",
            RivunMessageKind.COMMAND: "command",
            RivunMessageKind.QUERY: "query",
            RivunMessageKind.RESPONSE: "response",
            RivunMessageKind.STREAM_CHUNK: "stream_chunk",
            RivunMessageKind.ACTION: "action",
            RivunMessageKind.CONTROL: "control",
        }[self]

    def requires_subject(self) -> bool:
        return self is not RivunMessageKind.DATA
...
class RivunEnvelope:
    ...
    @classmethod
    def decode(cls, payload: bytes) -> "RivunEnvelope":
        ...
        try:
            kind = RivunMessageKind(kind_value)
...
class ControlFrame:
    ...
    def to_envelope(self) -> RivunEnvelope:
        return RivunEnvelope(
    ...
    def decode(cls, payload: bytes) -> "ControlFrame":
        envelope = RivunEnvelope.decode(payload)
        if envelope.kind is not RivunMessageKind.CONTROL:
...
class RivunUdpClient:
    ...
    def send_envelope(self, envelope: RivunEnvelope, target: tuple[str, int]) -> int:
    ...
    def recv_envelope(self, max_bytes: int = 65535) -> tuple[RivunEnvelope, tuple[str, int]]:
        payload, addr = self.socket.recvfrom(max_bytes)
        return RivunEnvelope.decode(payload), addr
    ...
    def __enter__(self) -> "RivunUdpClient":
        return self
