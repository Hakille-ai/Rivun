...
RECEIPT_REPLICATION_CONTENT_TYPE = "application/rivun-receipts+json"
RECEIPT_REPLICATION_REQUEST_SUBJECT = "rivun.receipts.request"
RECEIPT_REPLICATION_RESPONSE_SUBJECT = "rivun.receipts.response"
RECEIPT_SIGNATURE_DOMAIN = b"rivun-ACTION-RECEIPT-v1"
AGENT_CONTENT_TYPE = "application/rivun-agent+json"
AGENT_INTENT_SUBJECT = "rivun.agent.intent"
AGENT_STATUS_SUBJECT = "rivun.agent.status"
AGENT_RESULT_SUBJECT = "rivun.agent.result"
PACT_SCHEMA_VERSION = 1
PACT_CONTENT_TYPE = "application/rivun-pact+json"
PACT_RECORD_SUBJECT = "rivun.pact.record"
PACT_VERIFY_SUBJECT = "rivun.pact.verify"
PACT_REVOKE_SUBJECT = "rivun.pact.revoke"
PACT_BUNDLE_SUBJECT = "rivun.pact.bundle"
...
def artifact_hash(data: bytes) -> str:
    try:
        ...
        raise MissingCryptoBackend(
            "canonical Rivun artifact hashes use BLAKE3; install the optional 'blake3' Python package "
            "or verify hashes with rivun-cli"
        ) from exc
...
def verify_pact(pact: Mapping[str, Any], now_micros: int | None = None) -> bool:
    ...
    return verify_ed25519_signature(
        rivun_domain_message(PACT_SIGNATURE_DOMAIN, pact_canonical_signing_bytes(pact)),
        signature,
        public_key,
    )
...
def verify_signature_placeholder(kind: str) -> SignatureVerificationStatus:
    return SignatureVerificationStatus(
        supported=False,
        reason=(
            f"{kind} signatures are Ed25519 signatures over Rivun domain-separated payloads. "
            "Build the exact canonical message and call verify_ed25519_signature(), "
            "or use rivun-cli/Rust for canonical registry verification."
        ),
    )


def rivun_domain_message(domain: bytes, message: bytes) -> bytes:
...
def verify_ed25519_signature(message: bytes, signature: str, public_key: str) -> bool:
    try:
        ...
        raise MissingCryptoBackend(
            ...
            "or verification with rivun-cli"
        ) from exc
