...
const (
	...
	PactSchemaVersion                 = 1
	PactContentType                   = "application/rivun-pact+json"
	PactRecordSubject                 = "rivun.pact.record"
	PactVerifySubject                 = "rivun.pact.verify"
	PactRevokeSubject                 = "rivun.pact.revoke"
	PactBundleSubject                 = "rivun.pact.bundle"
...
type RivunPactStatus string

const (
	RivunPactStatusDraft   RivunPactStatus = "draft"
	RivunPactStatusActive  RivunPactStatus = "active"
	RivunPactStatusExpired RivunPactStatus = "expired"
	RivunPactStatusRevoked RivunPactStatus = "revoked"
	RivunPactStatusInvalid RivunPactStatus = "invalid"
)

type RivunPact struct {
	...
	Status          RivunPactStatus `json:"status,omitempty"`
}

type RivunPactBundle struct {
	SchemaVersion uint8          `json:"schema_version"`
	Pact          RivunPact        `json:"pact"`
	Verifications []any          `json:"verifications,omitempty"`
	Revocations   []any          `json:"revocations,omitempty"`
	Metadata      map[string]any `json:"metadata,omitempty"`
}

type rivunPactSigningPayload struct {
	PactID          UUID    `json:"pact_id"`
	Actor           string  `json:"actor"`
	Target          string  `json:"target"`
	Intent          string  `json:"intent"`
...
type RivunStoreClient struct{}

func (RivunStoreClient) RegistryIndexRequest(requireSignature bool) (ControlFrame, error) {
	return RegistryIndexRequestFrame(requireSignature)
}

func (RivunStoreClient) RegistryBundleManifestRequest(requirePublication bool, requireDrivers bool) (ControlFrame, error) {
...
func PactCanonicalSigningBytes(pact RivunPact) ([]byte, error) {
	if err := ValidatePactShape(pact); err != nil {
		return nil, err
	}
	payload := rivunPactSigningPayload{
		PactID:          pact.PactID,
		Actor:           pact.Actor,
		Target:          pact.Target,
		Intent:          pact.Intent,
...
func PactHash(pact RivunPact) (string, error) {
...
func ValidatePactShape(pact RivunPact) error {
...
func VerifyPact(pact RivunPact, nowMicros *uint64) (bool, error) {
	if err := ValidatePactShape(pact); err != nil {
		return false, err
	}
	if pact.Status == RivunPactStatusRevoked {
		return false, nil
	}
	...
	return VerifyEd25519Signature(RivunDomainMessage([]byte(PactSignatureDomain), message), pact.Signature, pact.ActorPublicKey)
}

func VerifyPactBundle(bundle RivunPactBundle, nowMicros *uint64) (bool, error) {
...
func RivunDomainMessage(domain []byte, message []byte) []byte {
