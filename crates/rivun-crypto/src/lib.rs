//! Rivun identity and signature primitives.
//!
//! The 64-byte Rivun-Wire header keeps an 8-byte `ZAP_SIGN` field for fast
...
use uuid::Uuid;
use rivun_core::{
    ED25519_SIGNATURE_LEN, PoaAttestation, PoaTrailer, RivunError as CoreError, RivunFlags, RivunFrame,
};

const KEY_FILE_SCHEMA_VERSION: u8 = 1;
const SECRET_KEY_LEN: usize = 32;
const PUBLIC_KEY_LEN: usize = 32;
const NODE_ID_DOMAIN: &[u8] = b"Rivun-NODE-ID-v1";
const SIGN_HINT_DOMAIN: &[u8] = b"Rivun-SIGN-HINT-v1";
const POA_DIGEST_DOMAIN: &[u8] = b"Rivun-POA-DIGEST-v1";
const POA_SIGNATURE_DOMAIN: &[u8] = b"Rivun-POA-SIGNATURE-v1";
const POA_VALIDATOR_SET_SIGNATURE_DOMAIN: &[u8] = b"Rivun-POA-VALIDATOR-SET-v1";
...
pub const POA_VALIDATOR_SET_CONTENT_TYPE: &str = "application/rivun-poa-validator-set+json";
pub const POA_VALIDATOR_SET_REQUEST_SUBJECT: &str = "rivun.poa.validator_set.request";
pub const POA_VALIDATOR_SET_RESPONSE_SUBJECT: &str = "rivun.poa.validator_set.response";

#[derive(Debug, Error)]
pub enum RivunCryptoError {
    #[error(transparent)]
    Core(#[from] CoreError),
...
pub type Result<T> = std::result::Result<T, RivunCryptoError>;
...
impl Keypair {
    ...
    pub fn to_key_file(&self) -> RivunKeyFile {
    ...
    pub fn from_key_file(file: &RivunKeyFile) -> Result<Self> {
        if file.schema_version != KEY_FILE_SCHEMA_VERSION {
            return Err(RivunCryptoError::UnsupportedKeyFileVersion(
                file.schema_version,
            ));
        }
        ...
        if verifying.to_bytes() != public {
            return Err(RivunCryptoError::PublicKeyMismatch);
        }
        if verifying.node_id() != file.node_id {
            return Err(RivunCryptoError::NodeIdMismatch);
        }
    ...
    pub fn from_key_file_toml(input: &str) -> Result<Self> {
        let file: RivunKeyFile = toml::from_str(input)?;
...
impl PublicKey {
    ...
    pub fn verify_domain_message(
        ...
        self.verifying_key
            ...
            .map_err(|_| RivunCryptoError::InvalidSignature)
    }
}
...
pub fn verify_batch_signatures(
    ...
    if messages.len() != signatures.len() || signatures.len() != public_keys.len() {
        return Err(RivunCryptoError::InvalidKeyLength {
...
impl PoaValidatorSetRequest {
    pub fn validate(&self) -> Result<()> {
        if self.schema_version != POA_VALIDATOR_SET_SCHEMA_VERSION {
            return Err(RivunCryptoError::UnsupportedPoaValidatorSetVersion(
                self.schema_version,
            ));
        }
        Ok(())
...
impl PoaValidatorSet {
    pub fn validate_static(&self) -> Result<()> {
        ...
        if self.epoch == 0 {
            return Err(RivunCryptoError::InvalidPoaValidatorSetEpoch);
        }
        if self.required_threshold == 0 {
            return Err(RivunCryptoError::InvalidPoaValidatorSetThreshold);
        }
        if self.required_threshold as usize > self.validators.len() {
            return Err(RivunCryptoError::PoaValidatorSetThresholdTooHigh {
        ...
        {
            return Err(RivunCryptoError::InvalidPoaValidatorSetValidityWindow);
        }
        let mut seen = HashSet::with_capacity(self.validators.len());
        for validator in &self.validators {
            if !seen.insert(validator.node_id) {
                return Err(RivunCryptoError::DuplicatePoaValidatorSetValidator(
                    validator.node_id,
                ));
            }
            ...
            if derived != validator.node_id {
                return Err(RivunCryptoError::PoaValidatorSetNodeMismatch {
...
impl SignedPoaValidatorSet {
    pub fn verify(&self, expected_authority: Option<&PublicKey>) -> Result<()> {
        ...
        if derived != self.authority_node {
            return Err(RivunCryptoError::PoaValidatorSetAuthorityMismatch {
        ...
        if let Some(expected_authority) = expected_authority {
            ...
            if expected != self.authority_node {
                return Err(RivunCryptoError::PoaValidatorSetUnexpectedAuthority {
                    expected,
...
pub fn sign_frame(keypair: &Keypair, frame: &RivunFrame) -> Result<RivunFrame> {
    let key_node = keypair.node_id();
    if frame.header.source_node != key_node {
        return Err(RivunCryptoError::SourceNodeMismatch {
    ...
    signed.header.flags |= RivunFlags::SIGNED;
    signed.header.rivun_sign = [0_u8; 8];
    signed.auth = None;
    signed.poa = None;
...
pub fn verify_frame(public_key: &PublicKey, frame: &RivunFrame) -> Result<()> {
    let key_node = public_key.node_id();
    if frame.header.source_node != key_node {
        return Err(RivunCryptoError::VerifyingNodeMismatch {
            frame_node: frame.header.source_node,
            key_node,
        });
    }
    if !frame.header.flags.contains(RivunFlags::SIGNED) {
        return Err(RivunCryptoError::MissingSignature);
    }

    let auth = frame.auth.ok_or(RivunCryptoError::MissingSignature)?;
    if signature_hint(&auth.signature) != frame.header.rivun_sign {
        return Err(RivunCryptoError::SignatureHintMismatch);
    }
...
pub fn certify_frame(frame: &RivunFrame, threshold: u16, validators: &[Keypair]) -> Result<RivunFrame> {
    if !frame.header.flags.contains(RivunFlags::SIGNED) {
        return Err(RivunCryptoError::MissingSignature);
    }
    if !frame.header.flags.contains(RivunFlags::REQUIRES_CONSENSUS) {
        return Err(RivunCryptoError::PoaNotRequired);
    }
    if validators.len() < threshold as usize {
        return Err(RivunCryptoError::PoaThresholdNotMet {
            required: threshold,
    ...
    for validator in validators {
        ...
        if !seen.insert(validator_node) {
            return Err(RivunCryptoError::DuplicatePoaValidator(validator_node));
        }
...
pub fn sign_poa_attestation_request(
    ...
    if request.schema_version != POA_ATTESTATION_SCHEMA_VERSION {
        return Err(RivunCryptoError::UnsupportedPoaAttestationVersion(
...
pub fn verify_poa_attestation_response(
    ...
    if response.validator_node != validator.node_id() {
        return Err(RivunCryptoError::UnknownPoaValidator(response.validator_node));
    }
    let digest = decode_poa_digest(&response.frame_digest)?;
    if &digest != expected_digest {
        return Err(RivunCryptoError::PoaResponseDigestMismatch);
    }
    ...
    validator
        ...
        .map_err(|_| RivunCryptoError::InvalidPoaSignature(response.validator_node))?;
    Ok(PoaAttestation {
...
pub fn verify_poa_certificate(
    ...
    let poa = frame
        .poa
        .as_ref()
        .ok_or(RivunCryptoError::MissingPoaCertificate)?;
    ...
    if poa.frame_digest != poa_frame_digest(frame) {
        return Err(RivunCryptoError::PoaDigestMismatch);
    }
    ...
    for attestation in &poa.attestations {
        if !seen.insert(attestation.validator_node) {
            return Err(RivunCryptoError::DuplicatePoaValidator(
        ...
        let public_key = validators
            ...
            .ok_or(RivunCryptoError::UnknownPoaValidator(
                attestation.validator_node,
            ))?;
        ...
        public_key
            ...
            .map_err(|_| RivunCryptoError::InvalidPoaSignature(attestation.validator_node))?;
...
pub fn poa_frame_digest(frame: &RivunFrame) -> [u8; 32] {
    let mut hasher = blake3::Hasher::new();
...
fn decode_poa_digest(encoded: &str) -> Result<[u8; 32]> {
    let decoded = STANDARD_NO_PAD.decode(encoded)?;
    if decoded.len() != 32 {
        return Err(RivunCryptoError::InvalidPoaDigestLength(decoded.len()));
    }
    Ok(decoded.try_into().unwrap())
}
...
mod tests {
    ...
    fn unsigned_frame(keypair: &Keypair) -> RivunFrame {
        RivunFrame::with_timestamp(
            keypair.node_id(),
    ...
    fn unsigned_consensus_frame(keypair: &Keypair) -> RivunFrame {
        RivunFrame::with_timestamp(
            keypair.node_id(),
            Uuid::from_bytes([7_u8; 16]),
    ...
    fn signed_frame_verifies() {
        ...
        assert!(frame.header.flags.contains(RivunFlags::SIGNED));
    ...
    fn payload_tampering_fails() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::InvalidSignature)
        ));
    }
    ...
    fn signature_hint_tampering_fails_fast() {
        let keypair = Keypair::generate();
        let mut frame = sign_frame(&keypair, &unsigned_frame(&keypair)).unwrap();
        frame.header.rivun_sign[0] ^= 0xFF;

        assert!(matches!(
            verify_frame(&keypair.verifying_key(), &frame),
            Err(RivunCryptoError::SignatureHintMismatch)
        ));
    }
    ...
    fn domain_messages_are_signed_and_verified() {
        let keypair = Keypair::generate();
        let signature = keypair.sign_domain_message(b"Rivun-TEST-v1", b"payload");

        keypair
            .verifying_key()
            .verify_domain_message(b"Rivun-TEST-v1", b"payload", &signature)
            .unwrap();
        assert!(matches!(
            keypair
                .verifying_key()
                .verify_domain_message(b"Rivun-OTHER-v1", b"payload", &signature),
            Err(RivunCryptoError::InvalidSignature)
        ));
        assert!(matches!(
            keypair
                .verifying_key()
                .verify_domain_message(b"Rivun-TEST-v1", b"tampered", &signature),
            Err(RivunCryptoError::InvalidSignature)
        ));
    }
    ...
    fn wrong_public_key_fails_on_source_node() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::VerifyingNodeMismatch { .. })
        ));
    }
    ...
    fn refuses_to_sign_for_a_different_source_node() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::SourceNodeMismatch { .. })
        ));
    }
    ...
    fn poa_attestation_response_rejects_wrong_digest() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::PoaResponseDigestMismatch)
        ));
    }
    ...
    fn poa_validator_set_signs_and_verifies() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::PoaValidatorSetUnexpectedAuthority { .. })
        ));
    }
    ...
    fn poa_certificate_detects_payload_tampering() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::PoaDigestMismatch)
        ));
    }
    ...
    fn poa_certificate_rejects_unknown_validator() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::UnknownPoaValidator(_))
        ));
    }
    ...
    fn poa_certificate_enforces_required_threshold() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::PoaThresholdNotMet {
                required: 2,
                actual: 1
            })
        ));
    ...
    fn batch_signature_verification() {
        ...
        assert!(matches!(
            ...
            Err(RivunCryptoError::InvalidKeyLength { .. })
        ));
