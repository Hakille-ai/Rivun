...
use uuid::Uuid;
use rivun_agent::{AgentId, AgentIntent, IntentKind, Validate};
use rivun_core::{RivunFlags, RivunFrame, now_micros};
use rivun_crypto::{Keypair, PublicKey};
use rivun_ledger::{ReceiptJournalStore, SignedActionReceipt};
use rivun_memory::{MemoryJournalStore, MemoryStore};
use rivun_node::RivunNode;
...
pub fn list_tools() -> Vec<ToolDescriptor> {
    vec![
        ToolDescriptor {
            name: "rivun_send".to_string(),
            description: "Send typed messages, commands, or envelopes to Rivun nodes.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolDescriptor {
            name: "rivun_send_transaction".to_string(),
            description: "Execute an action or transaction through Rivun deterministic policy and receipt journal.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolDescriptor {
            name: "rivun_query".to_string(),
            description: "Query Rivun deterministic auditable memory journal and receipts.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolDescriptor {
            name: "rivun_query_state".to_string(),
            description: "Query Rivun deterministic state and receipts.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "namespace": { "type": "string", "description": "Memory namespace" },
        ...
        ToolDescriptor {
            name: "rivun_agent_intent".to_string(),
        ...
        ToolDescriptor {
            name: "rivun_receipts_verify".to_string(),
        ...
        ToolDescriptor {
            name: "rivun_verify_provenance".to_string(),
            description: "Alias for rivun_receipts_verify for step-by-step cryptographic provenance verification.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "chain": { "type": "object", "description": "ProvenanceChainDigest JSON object" },
        ...
        ToolDescriptor {
            name: "rivun_get_fleet_health".to_string(),
        ...
        ToolDescriptor {
            name: "rivun_inspect_pack".to_string(),
...
pub async fn execute_tool(
    ...
    match params.name.as_str() {
        "rivun_send" | "rivun_send_transaction" => {
            let payload = params
                .arguments
                .get("payload")
                .and_then(|p| p.as_str())
                ...
                (&ctx.journal, &ctx.node_keypair)
            {
                ...
                .map_err(|e| RivunGatewayError::Internal(e.to_string()))?;
        ...
        "rivun_query" | "rivun_query_state" => {
            ...
            let records = if let Some(mem_lock) = &ctx.memory {
                let mem = mem_lock
                    .lock()
                    .map_err(|e| RivunGatewayError::Internal(e.to_string()))?;
                let q = rivun_memory::MemoryQuery {
        ...
        "rivun_agent_intent" => {
            let intent: AgentIntent = if let Some(intent_obj) = params.arguments.get("intent") {
                serde_json::from_value(intent_obj.clone()).map_err(|e| {
                    RivunGatewayError::jsonrpc_invalid_params(format!("Invalid intent: {e}"))
                })?
            } else {
                ...
                let source_agent = AgentId::new(source_agent_str)
                    .map_err(|e| RivunGatewayError::jsonrpc_invalid_params(e.to_string()))?;
                let kind_str = params
                    .arguments
                    .get("kind")
                    .and_then(|k| k.as_str())
        ...
        "rivun_receipts_verify" | "rivun_verify_provenance" => {
            let chain_val = params
                .arguments
                .get("chain")
                .or_else(|| params.arguments.get("provenance_chain"))
                .ok_or_else(|| {
                    RivunGatewayError::jsonrpc_invalid_params("Missing `chain` argument")
                })?;

            let chain: ProvenanceChainDigest =
                serde_json::from_value(chain_val.clone()).map_err(|e| {
                    RivunGatewayError::jsonrpc_invalid_params(format!("Invalid chain JSON: {e}"))
                })?;

            let pk =
                if let Some(pk_str) = params.arguments.get("public_key").and_then(|p| p.as_str()) {
                    let bytes = hex::decode(pk_str).map_err(|_| {
                        RivunGatewayError::jsonrpc_invalid_params("Invalid hex for public_key")
                    })?;
                    if bytes.len() != 32 {
                        return Err(RivunGatewayError::jsonrpc_invalid_params(
        ...
        other => Err(RivunGatewayError::jsonrpc_method_not_found(format!(
