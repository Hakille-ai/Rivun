...
async fn test_empirical_6_stage_provenance_causal_chain_integrity() {
    ...
    let chain = ProvenanceChainBuilder::new(session_id, intent_id)
        ...
        .with_negotiation(
            &serde_json::json!({
                "protocol": "rivun.agent.negotiation.v1",
...
Sec-WebSocket-Version: 13\r\n\r\n",
    ...
    assert_eq!(
        ack_frame.opcode,
        rivun_gateway::transports::ws::WS_OPCODE_TEXT
    );
    ...
    assert_eq!(
        pong_frame.opcode,
        rivun_gateway::transports::ws::WS_OPCODE_PONG
    );
...
async fn test_empirical_full_e2e_ai_agent_workflow() {
    ...
    let neg_req = CapabilityNegotiationRequest {
        ...
        required_capabilities: BTreeSet::from([rivun_capability::CapabilityId::new(
