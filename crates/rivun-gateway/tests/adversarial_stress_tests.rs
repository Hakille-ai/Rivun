...
async fn challenge_mcp_invalid_params_32602() {
    ...
    let req5 = json!({
        ...
        "id": 105,
        "method": "resources/read",
        "params": { "uri": "rivun://nonexistent/uri" }
    });
...
async fn challenge_mcp_all_registered_tools_execute() {
    ...
    let send_res = send_resp.result.expect("rivun_send tool must succeed");
    ...
    // Call rivun_get_fleet_health
    let fleet_req = json!({
        "jsonrpc": "2.0",
        "id": 203,
        "method": "tools/call",
        "params": {
            "name": "rivun_get_fleet_health",
            "arguments": {}
        }
    });
...
Sec-WebSocket-Version: 13\r\n\r\n",
    ...
    let valid_frame = WsFrame::text("Hello Rivun Gateway within limit");
    ws_client_handler
