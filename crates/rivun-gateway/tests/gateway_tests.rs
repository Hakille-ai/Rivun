...
async fn test_mcp_initialize() {
    ...
    assert_eq!(result["serverInfo"]["name"], "rivun-gateway");
}
...
async fn test_mcp_tools_list_and_call() {
    ...
    assert!(tool_names.contains(&"rivun_send"));
    assert!(tool_names.contains(&"rivun_query"));
    assert!(tool_names.contains(&"rivun_agent_intent"));
    assert!(tool_names.contains(&"rivun_receipts_verify"));
...
async fn test_mcp_resources_and_prompts() {
    ...
    assert!(uris.contains(&"rivun://ledger/receipts"));
    assert!(uris.contains(&"rivun://node/status"));
    assert!(uris.contains(&"rivun://fleet/topology"));
...
async fn test_http_rest_sessions_and_negotiate_and_delegate() {
    ...
    let neg_req = CapabilityNegotiationRequest {
        ...
        required_capabilities: std::collections::BTreeSet::from([
            rivun_capability::CapabilityId::new("driver.execute:test").unwrap(),
        ]),
