...
"#;
    ...
    assert_eq!(bundle.manifest.pack_id, "com.rivun.finance");
    ...
    assert_eq!(loaded_bundle.manifest.pack_id, "com.rivun.finance");
...
fn test_dependency_resolver() {
    ...
    let deps = vec![DomainPackDependencySpec {
        pack_id: "com.rivun.core".to_string(),
        version_req: "^1.0.0".to_string(),
        optional: false,
    }];

    let plan = resolver.resolve("com.rivun.app", "0.1.0", &deps).unwrap();
    assert_eq!(plan.install_order.len(), 1);
    assert_eq!(plan.install_order[0].id, "com.rivun.core");
