...
use crate::{DomainPackStatus, RivunStoreError};
...
impl DomainPackBundleSignature {
    pub fn sign(
        ...
        let payload = BundleSigningPayload {
            domain: "Rivun-DOMAIN-PACK-BUNDLE-v1",
            pack_id,
            pack_version,
            bundle_sha256,
            signer_node_id,
    ...
    pub fn verify(&self, expected_bundle_sha256: &str) -> Result<(), RivunStoreError> {
        if self.bundle_sha256 != expected_bundle_sha256 {
            return Err(RivunStoreError::DomainPackBundleDigestMismatch {
        ...
        let verifying_key = VerifyingKey::from_bytes(&pub_key_bytes)
            .map_err(|_| RivunStoreError::InvalidDomainPackBundleSignature)?;
    ...
    pub fn verify_against_trusted_keys(
        ...
        if !trusted_public_keys.is_empty() {
            ...
            if !matched {
                return Err(RivunStoreError::UntrustedDomainPackSigner {
...
fn parse_public_key_str(s: &str) -> Result<[u8; 32], RivunStoreError> {
    let s = s.trim();
    ...
    Err(RivunStoreError::InvalidDomainPackBundleSignature)
}
...
impl DomainPackBundle {
    pub fn build_from_dir(pack_dir: &Path) -> Result<Self, RivunStoreError> {
        ...
        let pack_id = pack_toml
            .get("id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| RivunStoreError::EmptyDomainPackId)?
            .to_string();
        ...
        fn walk_dir(
            ...
            let entries =
                fs::read_dir(current_dir).map_err(|e| RivunStoreError::IoError(e.to_string()))?;
            for entry in entries {
                let entry = entry.map_err(|e| RivunStoreError::IoError(e.to_string()))?;
                let path = entry.path();
                ...
                if path.is_dir() {
                    ...
                    let rel_path = path
                        ...
                        .map_err(|_| {
                            RivunStoreError::InvalidDomainPackArtifactPath(path.display().to_string())
                        })?
                        .to_string_lossy()
                        .replace('\\', "/");

                    let content =
                        fs::read(&path).map_err(|e| RivunStoreError::IoError(e.to_string()))?;
    ...
    pub fn decode_bytes(bytes: &[u8]) -> Result<Self, RivunStoreError> {
        if bytes.len() < 16 || &bytes[0..8] != ZPACK_MAGIC {
            return Err(RivunStoreError::InvalidDomainPackBundleFormat(
    ...
    pub fn open_from_file(bundle_path: &Path) -> Result<Self, RivunStoreError> {
        let bytes = fs::read(bundle_path).map_err(|e| {
            RivunStoreError::IoError(format!(
    ...
    pub fn write_to_file(&self, output_path: &Path) -> Result<(), RivunStoreError> {
        if let Some(parent) = output_path.parent() {
            fs::create_dir_all(parent).map_err(|e| RivunStoreError::IoError(e.to_string()))?;
        }
