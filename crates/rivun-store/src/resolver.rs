...
use crate::{DomainPackRegistry, DomainPackRegistryEntry, DriverVersion, RivunStoreError};
...
impl<'a> DomainPackDependencyResolver<'a> {
    ...
    fn resolve_dep(
        ...
        if visited_branch.contains(&dep.pack_id) {
            return Err(RivunStoreError::CircularDomainPackDependency(
                dep.pack_id.clone(),
            ));
        }
        ...
        let entry = match matching_entry {
            ...
            None => {
                if dep.optional {
                    ...
                    return Err(RivunStoreError::UnsatisfiedDomainPackDependency {
