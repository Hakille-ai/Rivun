//! Rivun PACT profile contracts.
//!
//! A PACT is a portable signed action record intended to travel inside a
//! `ZENV` envelope. The profile reuses Rivun identity, BLAKE3 hashing, and
...
use thiserror::Error;
use uuid::Uuid;
use rivun_crypto::{Keypair, PublicKey, RivunCryptoError};

pub const PACT_SCHEMA_VERSION: u8 = 1;
pub const PACT_CONTENT_TYPE: &str = "application/rivun-pact+json";
pub const PACT_RECORD_SUBJECT: &str = "rivun.pact.record";
pub const PACT_VERIFY_SUBJECT: &str = "rivun.pact.verify";
pub const PACT_REVOKE_SUBJECT: &str = "rivun.pact.revoke";
pub const PACT_BUNDLE_SUBJECT: &str = "rivun.pact.bundle";
...
pub type Result<T> = std::result::Result<T, RivunPactError>;

#[derive(Debug, Error)]
pub enum RivunPactError {
...
impl Validate for RivunPactProof {
    fn validate(&self) -> Result<()> {
...
pub struct RivunPact {
    ...
    #[serde(default)]
    pub status: RivunPactStatus,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub verification: Option<RivunPactVerification>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub revocation: Option<RivunPactRevocation>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub timeline: Vec<RivunPactTimelineEntry>,
}
...
impl RivunPact {
    ...
    pub fn sign(&mut self, keypair: &Keypair) -> Result<()> {
        ...
        self.status = RivunPactStatus::Active;
        Ok(())
    }

    pub fn verify(&self, now_micros: Option<u64>) -> Result<RivunPactVerification> {
        self.validate()?;
        if self.status == RivunPactStatus::Revoked || self.revocation.is_some() {
            return Err(RivunPactError::Revoked);
        }
        if self
            .expires_at_micros
            .zip(now_micros)
        ...
        {
            return Err(RivunPactError::Expired);
        }
        let expected_hash = self.canonical_hash()?;
        let actual_hash = self.hash.as_ref().ok_or(RivunPactError::MissingField {
            entity: "pact",
            field: "hash",
        })?;
        if actual_hash != &expected_hash {
            return Err(RivunPactError::HashMismatch {
        ...
        let signature = decode_fixed::<ED25519_SIGNATURE_LEN>(
            self.signature
                .as_deref()
                .ok_or(RivunPactError::MissingField {
                    entity: "pact",
                    field: "signature",
                })?,
            "signature",
        ...
        public_key
            ...
            .map_err(|_| RivunPactError::InvalidSignature)?;
        Ok(RivunPactVerification {
            schema_version: PACT_SCHEMA_VERSION,
    ...
    fn signing_payload_ordered(&self) -> RivunPactSigningPayload<'_> {
        RivunPactSigningPayload {
            pact_id: self.pact_id,
...
struct RivunPactSigningPayload<'a> {
    pact_id: Uuid,
    actor: &'a str,
    target: &'a str,
    intent: &'a str,
...
impl Validate for RivunPact {
    fn validate(&self) -> Result<()> {
        ...
        {
            return Err(RivunPactError::InvalidField {
                entity: "pact",
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct RivunPactVerification {
    pub schema_version: u8,
    pub pact_id: Uuid,
    pub valid: bool,
    pub status: RivunPactStatus,
...
impl Validate for RivunPactVerification {
    fn validate(&self) -> Result<()> {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct RivunPactRevocation {
    pub schema_version: u8,
    pub pact_id: Uuid,
...
impl Validate for RivunPactRevocation {
    fn validate(&self) -> Result<()> {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct RivunPactTimelineEntry {
    pub at_micros: u64,
    pub status: RivunPactStatus,
    pub note: String,
}

impl Validate for RivunPactTimelineEntry {
    fn validate(&self) -> Result<()> {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct RivunPactBundle {
    pub schema_version: u8,
    pub pact: RivunPact,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub verifications: Vec<RivunPactVerification>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub revocations: Vec<RivunPactRevocation>,
    #[serde(default)]
    pub metadata: BTreeMap<String, Value>,
}

impl RivunPactBundle {
    pub fn new(pact: RivunPact) -> Self {
        Self {
            schema_version: PACT_SCHEMA_VERSION,
            pact,
...
impl Validate for RivunPactBundle {
    fn validate(&self) -> Result<()> {
...
fn validate_schema_version(entity: &'static str, version: u8) -> Result<()> {
    if version != PACT_SCHEMA_VERSION {
        return Err(RivunPactError::UnsupportedSchemaVersion { entity, version });
    }
    Ok(())
}
...
fn validate_text(entity: &'static str, field: &'static str, value: &str) -> Result<()> {
    if value.trim().is_empty() {
        return Err(RivunPactError::EmptyField { entity, field });
    }
    if value.len() > MAX_TEXT_BYTES {
        return Err(RivunPactError::FieldTooLong {
            entity,
            field,
            max: MAX_TEXT_BYTES,
        });
...
fn decode_fixed<const N: usize>(input: &str, field: &'static str) -> Result<[u8; N]> {
    let decoded = STANDARD_NO_PAD
        .decode(input)
        .map_err(|source| RivunPactError::Base64 { field, source })?;
    decoded
        .try_into()
        .map_err(|bytes: Vec<u8>| RivunPactError::InvalidField {
            entity: "pact",
            field,
...
mod tests {
    ...
    fn sample_pact() -> RivunPact {
        let mut pact = RivunPact::new("agent.alpha", "driver.valve", "open_valve", 1_700_000);
    ...
    fn canonical_payload_excludes_mutable_fields() {
        ...
        pact.status = RivunPactStatus::Revoked;
        pact.hash =
        ...
        pact.verification = Some(RivunPactVerification {
            schema_version: PACT_SCHEMA_VERSION,
            pact_id: pact.pact_id,
            valid: false,
            status: RivunPactStatus::Invalid,
        ...
        pact.timeline.push(RivunPactTimelineEntry {
            at_micros: 2,
            status: RivunPactStatus::Revoked,
            note: "revoked".into(),
        });
    ...
    fn tampered_terms_fail_verification() {
        ...
        assert!(matches!(
            ...
            Err(RivunPactError::HashMismatch { .. })
        ));
    }
