...
use crate::{AgentIntent, Result, Validate, RivunAgentError};

pub const PROVENANCE_SCHEMA_VERSION: u8 = 1;
pub const PROVENANCE_SIGNATURE_DOMAIN: &[u8] = b"Rivun-PROVENANCE-CHAIN-v1";
...
impl ProvenanceChainBuilder {
    ...
    pub fn with_negotiation(
        ...
        let prev = self
            .intent_step
            .as_ref()
            .ok_or(RivunAgentError::MissingStep(ProvenanceStage::Intent))?;
    ...
    pub fn with_policy(
        ...
        let prev_hash = if let Some(neg) = &self.negotiation_step {
            ...
            return Err(RivunAgentError::MissingStep(ProvenanceStage::Intent));
        };
...
impl ProvenanceChainDigest {
    ...
    pub fn verify_step(&self, stage: ProvenanceStage) -> Result<()> {
        let (idx, step) = self
            ...
            .ok_or(RivunAgentError::MissingStep(stage))?;

        if idx == 0 {
            if step.stage != ProvenanceStage::Intent {
                return Err(RivunAgentError::StepVerificationFailed {
                    stage,
            ...
            let declared_prev =
                step.previous_hash
                    .as_ref()
                    .ok_or(RivunAgentError::StepVerificationFailed {
                        stage,
