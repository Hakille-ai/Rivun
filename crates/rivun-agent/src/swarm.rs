...
impl SwarmAgentCoordinator {
    ...
    pub fn submit_intent(&mut self, intent: AgentIntent, deadline_micros: u64) -> Result<Uuid> {
        intent.validate()?;
        let canonical_bytes =
            serde_json::to_vec(&intent).map_err(|e| RivunAgentError::InvalidIdentifier {
                entity: "agent_intent",
                field: "json",
                value: e.to_string(),
            })?;
    ...
    pub fn mark_proposed(&mut self, proposal_id: Uuid) -> Result<&SwarmIntentProposal> {
        let record = self.active_intents.get_mut(&proposal_id).ok_or_else(|| {
            RivunAgentError::InvalidIdentifier {
                entity: "swarm_intent",
                field: "proposal_id",
    ...
    pub fn finalize_intent_with_provenance(
        ...
        let cert = record.commit_certificate.as_ref().ok_or_else(|| {
            RivunAgentError::MissingStep(crate::provenance::ProvenanceStage::Consensus)
        })?;
