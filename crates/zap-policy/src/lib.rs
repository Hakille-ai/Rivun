//! Deterministic policy evaluation for typed ZAP messages.
//!
...
pub type Result<T> = std::result::Result<T, ZapPolicyError>;
...
impl PolicySet {
    ...
    pub fn from_toml_str(input: &str) -> Result<Self> {
        let set: Self =
            toml::from_str(input).map_err(|error| ZapPolicyError::Toml(error.to_string()))?;
        set.validate()?;
        Ok(set)
    }
...
impl PolicyRule {
    pub fn validate(&self, fallback_name: &str) -> Result<()> {
        ...
        {
            return Err(ZapPolicyError::EmptyKind(name));
        }
        if self
            .subject
            .as_deref()
        ...
        {
            return Err(ZapPolicyError::EmptySubjectPattern(name));
        }
        if self
            .content_type
            .as_deref()
        ...
        {
            return Err(ZapPolicyError::EmptyContentType(name));
        }
        if self.decision == PolicyDecision::RequireGrant && self.required_capability.is_none() {
            return Err(ZapPolicyError::MissingRequiredCapability(name));
        }
        Ok(())
    }
