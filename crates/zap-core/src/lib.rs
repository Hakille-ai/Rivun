//! Core ZAP-Wire protocol types.
//!
//! The PDF specification fixes the first 64 bytes of every ZAP discharge.
...
bitflags! {
    /// ZAP-Wire frame flags. Unknown bits are rejected in v1 so malformed or
    /// future frames cannot silently downgrade behavior.
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
    pub struct ZapFlags: u16 {
        const ENCRYPTED = 1 << 0;
        const PRIORITY = 1 << 1;
...
pub enum ZapError {
    ...
    InvalidMagic(u32),
    #[error("unsupported ZAP version {0}")]
    UnsupportedVersion(u16),
    #[error("unknown ZAP flag bits 0x{0:04X}")]
    UnknownFlags(u16),
...
impl TryFrom<u16> for SignatureAlgorithm {
    ...
    fn try_from(value: u16) -> Result<Self> {
        match value {
            1 => Ok(Self::Ed25519),
            other => Err(ZapError::UnsupportedSignatureAlgorithm(other)),
        }
    }
}
...
impl AuthTrailer {
    ...
    pub fn parse(input: &[u8]) -> Result<Self> {
        if input.len() < AUTH_TRAILER_LEN {
            return Err(ZapError::Truncated {
                expected: AUTH_TRAILER_LEN,
                actual: input.len(),
            });
        }
        if input[0..4] != AUTH_TRAILER_MAGIC {
            return Err(ZapError::InvalidAuthTrailerMagic);
        }
        ...
        if signature_len as usize != ED25519_SIGNATURE_LEN {
            return Err(ZapError::InvalidSignatureLength(signature_len));
        }
...
impl PoaTrailer {
    pub fn new(
        ...
        if threshold == 0 {
            return Err(ZapError::InvalidPoaThreshold(threshold));
        }
        if attestations.len() > MAX_POA_ATTESTATIONS {
            return Err(ZapError::TooManyPoaAttestations(attestations.len() as u16));
        }
        Ok(Self {
            threshold,
            frame_digest,
    ...
    pub fn parse(input: &[u8]) -> Result<Self> {
        ...
        if input[0..4] != POA_TRAILER_MAGIC {
            return Err(ZapError::InvalidPoaTrailerMagic);
        }

        let version = u16::from_be_bytes([input[4], input[5]]);
        if version != POA_TRAILER_VERSION {
            return Err(ZapError::UnsupportedPoaTrailerVersion(version));
        }
        ...
        if attestation_count as usize > MAX_POA_ATTESTATIONS {
            return Err(ZapError::TooManyPoaAttestations(attestation_count));
        }

        let expected = POA_TRAILER_HEADER_LEN
            .checked_add(attestation_count as usize * POA_ATTESTATION_LEN)
            .ok_or(ZapError::TooManyPoaAttestations(attestation_count))?;
        if input.len() != expected {
            return Err(ZapError::PoaTrailerLengthMismatch {
                expected,
                actual: input.len(),
            });
        }
...
impl ZapHeader {
    pub fn new(
        ...
        if zap_len > MAX_PAYLOAD_LEN {
            return Err(ZapError::PayloadTooLarge(zap_len));
        }
    ...
    pub fn parse(input: &[u8]) -> Result<Self> {
        ...
        if magic != MAGIC_NUMBER {
            return Err(ZapError::InvalidMagic(magic));
        }
        ...
        let flags = ZapFlags::from_bits(flags_bits).ok_or_else(|| {
            let unknown = flags_bits & !ZapFlags::all().bits();
            ZapError::UnknownFlags(unknown)
        })?;
        ...
        let mut zap_sign = [0_u8; 8];
        zap_sign.copy_from_slice(&input[ZAP_SIGN_OFFSET..HEADER_LEN]);
    ...
    pub fn write_to(&self, out: &mut [u8; HEADER_LEN]) {
        ...
        out[ZAP_LEN_OFFSET..ZAP_SIGN_OFFSET].copy_from_slice(&self.zap_len.to_be_bytes());
        out[ZAP_SIGN_OFFSET..HEADER_LEN].copy_from_slice(&self.zap_sign);
    }
...
impl ZapFrame {
    ...
    pub fn decode(input: &[u8]) -> Result<Self> {
        let header = ZapHeader::parse(input)?;
        let payload_len = usize::try_from(header.zap_len)
            .map_err(|_| ZapError::PayloadTooLarge(header.zap_len))?;
        let payload_end = HEADER_LEN
            .checked_add(payload_len)
            .ok_or(ZapError::PayloadTooLarge(header.zap_len))?;
        ...
        let auth = if header.flags.contains(ZapFlags::SIGNED) {
            let auth_end = offset
                .checked_add(AUTH_TRAILER_LEN)
                .ok_or(ZapError::PayloadTooLarge(header.zap_len))?;
            if input.len() < auth_end {
                return Err(ZapError::MissingAuthTrailer);
            }
        ...
        {
            return Err(ZapError::UnexpectedAuthTrailer);
        } else {
            None
        };

        let poa = if input.len() > offset {
            if !header.flags.contains(ZapFlags::REQUIRES_CONSENSUS) {
                return Err(ZapError::UnexpectedPoaTrailer);
            }
        ...
        match (header.flags.contains(ZapFlags::SIGNED), auth.is_some()) {
            (true, false) => Err(ZapError::MissingAuthTrailer),
            (false, true) => Err(ZapError::UnexpectedAuthTrailer),
            _ => Ok(Self {
                header,
                payload,
                auth,
    ...
    pub fn set_auth(&mut self, signature: [u8; ED25519_SIGNATURE_LEN], hint: [u8; 8]) {
        self.header.flags |= ZapFlags::SIGNED;
        self.header.zap_sign = hint;
        self.auth = Some(AuthTrailer::ed25519(signature));
    }

    pub fn set_poa(&mut self, poa: PoaTrailer) {
        self.header.flags |= ZapFlags::REQUIRES_CONSENSUS;
        self.poa = Some(poa);
    }
}
...
pub fn now_micros() -> Result<u64> {
    let duration = SystemTime::now()
        ...
        .map_err(|_| ZapError::ClockBeforeUnixEpoch)?;
...
mod tests {
    ...
    fn header_offsets_are_big_endian_and_stable() {
        let mut header = ZapHeader::new(
            ZapFlags::ENCRYPTED | ZapFlags::PRIORITY,
            source(),
            target(),
        ...
        assert_eq!(ZapHeader::parse(&bytes).unwrap(), header);
    }

    #[test]
    fn rejects_invalid_magic_version_flags_and_payload_len() {
        let mut bytes = ZapHeader::new(ZapFlags::empty(), source(), target(), 1, 0)
            .unwrap()
            .to_bytes();

        bytes[0] = 0;
        assert!(matches!(
            ZapHeader::parse(&bytes),
            Err(ZapError::InvalidMagic(_))
        ));
        ...
        assert_eq!(
            ZapHeader::parse(&bytes),
            Err(ZapError::UnsupportedVersion(2))
        );
        ...
        assert_eq!(
            ZapHeader::parse(&bytes),
            Err(ZapError::UnknownFlags(0x8000))
        );
        ...
        assert_eq!(
            ZapHeader::parse(&bytes),
            Err(ZapError::PayloadTooLarge(MAX_PAYLOAD_LEN + 1))
        );
    }
    ...
    fn frame_round_trip_without_auth() {
        ...
        let decoded = ZapFrame::decode(&encoded).unwrap();
    ...
    fn signed_flag_requires_auth_trailer() {
        ...
        assert_eq!(
            ZapFrame::decode(&frame.encode()),
            Err(ZapError::MissingAuthTrailer)
        );
    }

    #[test]
    fn auth_trailer_requires_signed_flag() {
        let mut frame = ZapFrame::with_timestamp(
            source(),
            target(),
            ZapFlags::empty(),
            42,
        ...
        assert_eq!(
            ZapFrame::decode(&frame.encode()),
            Err(ZapError::UnexpectedAuthTrailer)
        );
    }
    ...
    fn poa_trailer_requires_consensus_flag() {
        ...
        assert_eq!(
            ZapFrame::decode(&frame.encode()),
            Err(ZapError::UnexpectedPoaTrailer)
        );
    }
    ...
    fn unsigned_frame_golden_vector_is_stable() {
        ...
        .unwrap();
        frame.header.zap_sign = [0xAA; 8];

        let expected =
            hex::decode(include_str!("../tests/golden/zap_frame_v1_unsigned.hex").trim()).unwrap();
        let encoded = frame.encode();
        assert_eq!(encoded.as_ref(), expected.as_slice());
        assert_eq!(ZapFrame::decode(&expected).unwrap(), frame);
    }
}
