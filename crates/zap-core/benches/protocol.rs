...
use uuid::Uuid;
use zap_core::{PoaAttestation, PoaTrailer, ZapFlags, ZapFrame, ZapHeader};
...
fn protocol(c: &mut Criterion) {
    ...
    let mut certified_frame = ZapFrame::with_timestamp(
        Uuid::from_bytes([1; 16]),
        Uuid::from_bytes([2; 16]),
    ...
    c.bench_function("decode_signed_poa_frame", |b| {
        b.iter(|| ZapFrame::decode(&certified_bytes).unwrap())
    });
}
