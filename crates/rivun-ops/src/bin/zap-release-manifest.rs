...
fn usage() -> &'static str {
    "usage: rivun-release-manifest --version <semver> --channel <nightly|preview|stable|security> --git-sha <sha> --out <path> <artifact>...\n\
     usage: rivun-release-manifest --check <manifest.json>"
}
...
fn target_from_archive_name(version: &str, file_name: &str) -> String {
    let prefix = format!("rivun-{version}-");
    file_name
        .strip_prefix(&prefix)
        .unwrap_or(file_name)
        .strip_suffix(".tar.gz")
