...
#[test]
fn test_zap_pack_cli_lifecycle() {
...
"#;
    ...
    // Build bundle
    let bundle = zap_store::DomainPackBundle::build_from_dir(&pack_dir).unwrap();
    bundle.write_to_file(&bundle_file).unwrap();

    // Sign bundle
    let sig = zap_store::DomainPackBundleSignature::sign(
    ...
    let install_dir = store_dir.join("packs/com.zap.cli-test/0.1.0");
    bundle.extract_to_dir(&install_dir).unwrap();
    assert!(install_dir.join("pack.toml").exists());

    // Audit
    let audit_report = zap_store::audit_bundle(&bundle, None).unwrap();
