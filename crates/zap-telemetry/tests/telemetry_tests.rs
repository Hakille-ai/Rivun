...
use uuid::Uuid;
use zap_telemetry::{
    ActionCounter, FleetDoctor, FleetDoctorStatus, FleetNodeHealth, FleetNodeState, FleetTopology, IncidentCapturer,
    PeerCounter, PeerTrustGauge, PrometheusExporter, ReasonCounter, SecretRedactor, TransportCounter,
...
fn test_metrics_parity_all_16_metrics() {
    ...
    let snapshot = ZapNodeMetricsSnapshot {
        node_id,
        frames_sent_total: vec![PeerCounter { peer: peer_id, value: 42 }],
        frames_received_total: vec![PeerCounter { peer: peer_id, value: 100 }],
        frames_rejected_total: vec![ReasonCounter { reason: "invalid_signature".to_string(), value: 3 }],
        driver_execution_errors_total: vec![ActionCounter { action: "exec_wasm".to_string(), value: 1 }],
        peer_trust_status: vec![PeerTrustGauge { peer: peer_id, status: "trusted".to_string(), value: 1 }],
        ...
        agent_gateway_requests_total: vec![TransportCounter { transport: "mcp_stdio".to_string(), status: "200".to_string(), value: 15 }],
...
"#;
    ...
    let node_id = Uuid::new_v4();
    let metrics = "zap_replay_rejections_total 1\n";

    let snapshot = IncidentCapturer::capture(node_id, metrics, Some(&config_path));

    assert_eq!(snapshot.node_id, node_id);
    let cfg_redacted = snapshot.config_summary.get("config_content_redacted").unwrap();
    assert!(!cfg_redacted.contains("0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"));
...
fn test_fleet_doctor_evaluation_corrupted_wal_and_manifests() {
    ...
    let replay_check = report.checks.iter().find(|c| c.category == "replay_guard").unwrap();
    assert_eq!(replay_check.status, FleetDoctorStatus::Failed);

    let journal_check = report.checks.iter().find(|c| c.category == "journal").unwrap();
