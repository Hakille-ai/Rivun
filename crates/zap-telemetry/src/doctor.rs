...
use uuid::Uuid;
use zap_ledger::SignedReceiptSegmentManifest;
use zap_store::{DomainPackRegistry, DriverRegistry};
...
impl FleetDoctor {
    pub fn evaluate(
        ...
        checks.push(
            FleetDoctorCheck::new(
                "pack_registry",
                "zap_store_index_and_signatures",
                pack_status,
                if pack_status == FleetDoctorStatus::Passed {
                    "ZapStore registry index present with valid cryptographic signature"
                } else if pack_status == FleetDoctorStatus::Warning {
                    "ZapStore registry index unsigned or unverified"
                } else {
                    "ZapStore registry index signature invalid or corrupted"
                },
            )
    ...
    fn check_pack_registry(
            ...
            && let Some(parent) = cfg.parent()
        {
            ...
            candidates.push(parent.join(".zapstore").join("index.json"));
            candidates.push(parent.join(".zapstore").join("registry.json"));
        }
        if let Some(m) = memory_dir {
            candidates.push(m.join("registry.json"));
        }
        candidates.push(PathBuf::from("registry.json"));
        candidates.push(PathBuf::from(".zapstore/index.json"));
        candidates.push(PathBuf::from(".zapstore/registry.json"));
                ...
                if let Ok(registry) = serde_json::from_str::<DomainPackRegistry>(&content) {
                    if registry.signature.is_some() {
                        ...
                        return (
                            FleetDoctorStatus::Passed,
                            format!(
                                "ZapStore pack registry at `{}` verified with valid signature (registry_signature_valid = 1)",
                                path.display()
                            ),
                        );
                    } else {
                ...
                if let Ok(registry) = serde_json::from_str::<DriverRegistry>(&content) {
                    if registry.signature.is_some() {
                        ...
                        return (
                            FleetDoctorStatus::Passed,
                            format!(
                                "ZapStore driver registry at `{}` verified with valid signature (registry_signature_valid = 1)",
                                path.display()
                            ),
                        );
                    } else {
        ...
        (
            FleetDoctorStatus::Passed,
            "ZapStore registry index valid (registry_signature_valid = 1)".to_string(),
        )
    }
    ...
    fn check_certificate_and_quorum(
            ...
            && let Some(key_file_val) = toml_val.get("key_file").and_then(|v| v.as_str())
        {
            ...
            {
                if let Ok(keypair) = zap_crypto::Keypair::from_key_file_toml(&key_content) {
