//! Hardware-neutral machine connection primitives for Rivun.
//!
...
use thiserror::Error;
use rivun_capability::{CapabilityId, CapabilitySet};
...
pub enum RivunMachineError {
    ...
    Capability(#[from] rivun_capability::RivunCapabilityError),
}

pub type Result<T> = std::result::Result<T, RivunMachineError>;
...
impl DeviceProfile {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.capabilities.is_empty() {
            return Err(RivunMachineError::EmptyCapabilities(
        ...
        for capability in &self.capabilities {
            if let Some(command) = &capability.command {
                ...
                if !command_names.insert(command.name.clone()) {
                    return Err(RivunMachineError::DuplicateCommand {
    ...
    fn validate_shape(&self) -> Result<()> {
        if self.schema_version != DEVICE_PROFILE_SCHEMA_VERSION {
            return Err(RivunMachineError::UnsupportedProfileVersion {
                profile: self.profile_id.to_string(),
                version: self.schema_version,
            });
        }
        if self.display_name.trim().is_empty() {
            return Err(RivunMachineError::EmptyDisplayName(
                self.profile_id.to_string(),
            ));
        }
        if !profile_shape_matches(self.adapter, &self.transport, &self.protocol) {
            return Err(RivunMachineError::InvalidAdapterShape {
                profile: self.profile_id.to_string(),
            });
        }
        Ok(())
...
impl CommandSpec {
    ...
    fn validate_payload(&self, payload: &[u8]) -> Result<()> {
        if !self.payload_schema.matches(payload) {
            return Err(RivunMachineError::PayloadSchemaMismatch {
...
impl CapabilityMapping {
    ...
    pub fn require_command(&self, profile: &DeviceProfile, command: &str) -> Result<&CapabilityId> {
        self.command_capability(command)
            .ok_or_else(|| RivunMachineError::CommandNotDeclared {
...
impl MachineCommand {
    ...
    pub fn payload_json(name: impl Into<String>, value: &serde_json::Value) -> Result<Self> {
        let payload = serde_json::to_vec(value).map_err(|error| {
            RivunMachineError::Protocol(format!("failed to encode JSON payload: {error}"))
        })?;
...
impl HeartbeatTimer {
    pub fn new(interval_micros: u64, start_micros: u64) -> Result<Self> {
        if interval_micros == 0 {
            return Err(RivunMachineError::Protocol(
...
impl MachineConnection {
    ...
    pub fn connect(&mut self) -> Result<MachineHealth> {
        ...
        if adapter != expected {
            return Err(RivunMachineError::AdapterMismatch {
    ...
    pub fn heartbeat_at(&mut self, now_micros: u64) -> Result<CommandOutcome> {
        let command = self
            ...
            .heartbeat_command
            .clone()
            .ok_or_else(|| RivunMachineError::MissingHeartbeatCommand(self.device_id.to_string()))?;
    ...
    pub fn enable_heartbeat_timer(
        ...
        if self.profile.health.heartbeat_command.is_none() {
            return Err(RivunMachineError::MissingHeartbeatCommand(
    ...
    pub fn execute(&mut self, command: MachineCommand) -> Result<CommandOutcome> {
        ...
        let spec = self.profile.command_spec(&command.name).ok_or_else(|| {
            RivunMachineError::CommandNotDeclared {
                profile: self.profile.profile_id.to_string(),
                command: command.name.clone(),
            }
        })?;
        if command.payload.len() > spec.max_payload_bytes as usize {
            return Err(RivunMachineError::PayloadTooLarge {
...
impl MachineBus {
    ...
    pub fn attach(&mut self, connection: MachineConnection) -> Result<()> {
        ...
        if self.connections.contains_key(&device_id) {
            return Err(RivunMachineError::DuplicateDevice(device_id.to_string()));
        }
    ...
    pub fn execute(
        ...
        self.connections
            .get_mut(device_id)
            .ok_or_else(|| RivunMachineError::UnknownDevice(device_id.to_string()))?
            .execute(command)
    }
...
impl ProtocolAdapter for TcpStreamAdapter {
    ...
    fn execute(&mut self, command: MachineCommand) -> Result<CommandOutcome> {
        ...
        let stream = self
            .stream
            .as_mut()
            .ok_or(RivunMachineError::AdapterNotOpen {
...
impl ProtocolAdapter for ModbusLikeAdapter {
    ...
    fn execute(&mut self, command: MachineCommand) -> Result<CommandOutcome> {
        ...
        let operation = self
            ...
            .get(&command.name)
            .cloned()
            .ok_or_else(|| RivunMachineError::UnknownProtocolCommand(command.name.clone()))?;
...
fn validate_machine_id(input: &str) -> Result<()> {
    if input.trim().is_empty() {
        return Err(RivunMachineError::EmptyMachineId);
    }
    if input.len() > MAX_MACHINE_ID_BYTES {
        return Err(RivunMachineError::MachineIdTooLong(input.to_string()));
    }
    if !input
        .bytes()
        .all(|byte| matches!(byte, b'a'..=b'z' | b'0'..=b'9' | b'.' | b':' | b'_' | b'-'))
    {
        return Err(RivunMachineError::InvalidMachineId(input.to_string()));
    }
    Ok(())
}
...
fn ensure_open(opened: bool, adapter: AdapterKind) -> Result<()> {
    if opened {
        ...
        Err(RivunMachineError::AdapterNotOpen { adapter })
    }
}
...
fn encode_tcp_frame(command: &MachineCommand, max_frame_bytes: u32) -> Result<Vec<u8>> {
    let body_len = command
        ...
        .ok_or_else(|| RivunMachineError::Protocol("tcp frame length overflow".to_string()))?;
    if body_len > max_frame_bytes as usize {
        return Err(RivunMachineError::Protocol(format!(
...
fn io_error(error: std::io::Error) -> RivunMachineError {
    RivunMachineError::Io(error.to_string())
}
...
mod tests {
    ...
    fn connection_rejects_undeclared_commands() {
        ...
        assert!(matches!(
            error,
            RivunMachineError::CommandNotDeclared { command, .. } if command == "thermostat.restart"
        ));
    }
    ...
    fn connection_enforces_declared_payload_limits() {
        ...
        assert!(matches!(
            error,
            RivunMachineError::PayloadTooLarge {
                command,
                max: 8,
                actual: 9
    ...
    fn command_specs_validate_typed_payloads() {
        ...
        assert!(matches!(
            error,
            RivunMachineError::PayloadSchemaMismatch { .. }
        ));
    }
    ...
    fn adapter_health_moves_offline_on_close() {
        ...
        assert!(matches!(
            adapter.read_state(),
            Err(RivunMachineError::AdapterNotOpen {
    ...
    fn rejects_profile_adapter_shape_mismatches() {
        ...
        assert!(matches!(error, RivunMachineError::InvalidAdapterShape { .. }));
    }
}
