//! Operational contracts for running ZAP in production.
//!
...
pub type Result<T> = std::result::Result<T, ZapOpsError>;
...
impl ObservabilityConfig {
    pub fn validate(&self) -> Result<()> {
        if self.schema_version != OBSERVABILITY_SCHEMA_VERSION {
            return Err(ZapOpsError::OutOfRange {
    ...
    pub fn from_toml_str(input: &str) -> Result<Self> {
        let config: Self =
            toml::from_str(input).map_err(|error| ZapOpsError::TomlDecode(error.to_string()))?;
        config.validate()?;
        Ok(config)
    }
}
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for check in &self.checks {
            ...
            if !names.insert(check.name.clone()) {
                return Err(ZapOpsError::Duplicate {
                    kind: "health check",
                    id: check.name.clone(),
                });
            }
...
impl GovernanceConfig {
        ...
        for group in &self.groups {
            ...
            for member in &group.members {
                if !operator_ids.contains(member) {
                    return Err(ZapOpsError::Unknown {
                        kind: "operator",
                        id: member.clone(),
                    });
                }
            ...
            if group.min_approvals as usize > eligible {
                return Err(ZapOpsError::ApprovalThresholdTooHigh {
        ...
        for policy in &self.policies {
            ...
            for quorum in &policy.required_groups {
                if !group_ids.contains(&quorum.group_id) {
                    return Err(ZapOpsError::PolicyReferencesUnknownGroup {
    ...
    pub fn evaluate_approvals(
        ...
        let policy = self
            ...
            .ok_or_else(|| ZapOpsError::Unknown {
                kind: "approval policy",
...
pub fn verify_audit_chain(entries: &[AuditEntry]) -> Result<()> {
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        if entry.sequence != expected_sequence {
            return Err(ZapOpsError::AuditSequenceGap {
                expected: expected_sequence,
                actual: entry.sequence,
            });
        }
        if entry.previous_hash != previous_hash {
            return Err(ZapOpsError::AuditPreviousHashMismatch {
                sequence: entry.sequence,
            });
        }
        let computed = entry.compute_hash();
        if computed != entry.entry_hash {
            return Err(ZapOpsError::AuditHashMismatch {
                sequence: entry.sequence,
            });
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for signature in &self.signatures {
            ...
            if !artifact_names.contains(&signature.artifact_name) {
                return Err(ZapOpsError::UnknownSignatureArtifact(
...
impl ReleasePackagePlan {
    pub fn expected_artifact_names(&self) -> Result<Vec<String>> {
        ...
        for target in &self.targets {
            ...
            names.push(format!("zap-{}-{target}", self.version));
        }
        if self.include_registry_bundle {
            names.push(format!("zapstore-bundle-{}", self.version));
        }
        if self.include_sbom {
            names.push(format!("zap-sbom-{}.spdx.json", self.version));
        }
        Ok(names)
    }
}
...
fn validate_semver(version: &str) -> Result<()> {
    ...
    if valid {
        ...
        Err(ZapOpsError::InvalidVersion(version.to_string()))
    }
}
...
fn validate_git_sha(sha: &str) -> Result<()> {
    if sha.len() == 40 && sha.chars().all(is_lower_hex) {
        ...
        Err(ZapOpsError::InvalidGitSha(sha.to_string()))
    }
}
...
fn validate_checksum(name: &str, field: &'static str, checksum: &str) -> Result<()> {
    if checksum.len() == 64 && checksum.chars().all(is_lower_hex) {
        ...
        Err(ZapOpsError::InvalidChecksum {
            name: name.to_string(),
            field,
        })
    }
...
mod tests {
    ...
    fn governance_config() -> GovernanceConfig {
        GovernanceConfig {
            ...
            audit: AuditConfig {
                log_path: ".zap/audit.jsonl".to_string(),
                hash_chained: true,
                retain_days: 365,
            },
        }
    ...
    fn observability_validation_rejects_duplicate_health_checks() {
        let config = ObservabilityConfig {
            ...
            service: ServiceIdentity {
                service_name: "zap-node".to_string(),
    ...
    fn audit_chain_detects_mutation() {
        ...
        assert_eq!(
            ...
            Err(ZapOpsError::AuditHashMismatch { sequence: 1 })
        );
    }
    ...
    fn release_manifest_validates_artifacts_and_signatures() {
        ...
        let manifest = ReleaseManifest {
            ...
            artifacts: vec![ReleaseArtifact {
                name: "zap-0.1.0-x86_64-unknown-linux-gnu".to_string(),
                kind: ArtifactKind::CliBinary,
                target: "x86_64-unknown-linux-gnu".to_string(),
                path: "dist/zap-0.1.0-linux.tar.gz".to_string(),
                size_bytes: 100,
                sha256: checksum.clone(),
                blake3: checksum,
            }],
            signatures: vec![ArtifactSignature {
                artifact_name: "zap-0.1.0-x86_64-unknown-linux-gnu".to_string(),
    ...
    fn release_plan_names_expected_artifacts() {
        ...
        assert_eq!(
            ...
            vec![
                "zap-0.2.0-rc.1-x86_64-unknown-linux-gnu",
                "zapstore-bundle-0.2.0-rc.1",
                "zap-sbom-0.2.0-rc.1.spdx.json"
            ]
        );
    }
}
