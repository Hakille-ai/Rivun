...
use rivun_agent::{AgentId, AgentIntent, IntentKind};
use rivun_crypto::Keypair;
use rivun_gateway::{AgentGatewayServer, GatewayConfig, ProvenanceChainBuilder};
use rivun_ledger::ReceiptJournalStore;
...
fn test_cli_receipts_verify_with_provenance_flag() {
    ...
    let frame = rivun_core::RivunFrame::new(
        sender.node_id(),
        key.node_id(),
        rivun_core::RivunFlags::empty(),
        bytes::Bytes::from_static(b"payload"),
    )
    .unwrap();
    let signed = rivun_crypto::sign_frame(&sender, &frame).unwrap();
    let receipt = rivun_ledger::SignedActionReceipt::new(
        &key,
        &signed,
        "echo",
        Some(b"ok"),
