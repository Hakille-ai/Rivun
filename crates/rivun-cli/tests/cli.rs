...
use rivun_node::{PeerTrustStatus, RivunNode, RivunNodeConfig};
use rivun_store::{
...
async fn send_uses_configured_bind_address_and_sends_signed_frame() {
    ...
    let receiver_endpoint = RivunEndpoint::bind(RivunEndpointConfig::new(
...
"#,
    ...
    let binary = env!("CARGO_BIN_EXE_rivun");
...
fn send_rejects_invalid_config_before_network_send() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "send",
            "--config",
...
fn schema_export_includes_protocol_constants_and_fixture_catalog() {
    ...
    assert_eq!(json["agent"]["content_type"], "application/rivun-agent+json");
    assert_eq!(json["pact"]["content_type"], "application/rivun-pact+json");
    assert!(
        json["agent"]["subjects"]
            .as_array()
            .unwrap()
            .iter()
            .any(|subject| subject == "rivun.agent.intent")
    );
    assert!(
        json["pact"]["subjects"]
            .as_array()
            .unwrap()
            .iter()
            .any(|subject| subject["subject"] == "rivun.pact.record" && subject["kind"] == "action")
    );
    assert!(
        json["controls"]["receipts"]
            ...
            .unwrap()
            .iter()
            .any(|control| control["subject"] == "rivun.receipts.request")
    );
    assert!(
        json["fixtures"]
            .as_array()
...
"#,
    ...
    let denied = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "policy",
            "evaluate",
            "--policy",
    ...
    let allowed = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "policy",
            "evaluate",
            "--policy",
...
"#,
    ...
    let json: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(json["valid"], true);
    assert_eq!(json["id"], "rivun-pack-agentic-dev");
...
"#,
    ...
    let good = packs
        .iter()
        .find(|pack| pack["id"] == "rivun-pack-agentic-dev")
        .unwrap();
    ...
    let bad = packs
        .iter()
        .find(|pack| pack["id"] == "rivun-pack-bad")
        .unwrap();
...
"#,
    ...
    assert!(errors.iter().any(|error| error.contains("rivun-policy")));
    assert!(
        errors
            .iter()
...
fn driver_manifest_create_and_verify_round_trip() {
    ...
    let create = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "driver-manifest",
            "create",
            "--driver",
    ...
    let verify = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "driver-manifest",
            "verify",
            "--driver",
...
fn registry_init_add_list_and_verify_round_trip() {
    ...
    let manifest = DriverManifest::new(
        ...
        rivun_runtime::DriverPermissions::none(),
        None,
        &author,
    )
    .unwrap();
    ...
    let init = Command::new(env!("CARGO_BIN_EXE_rivun"))
    ...
    let add = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "add",
            "--registry",
    ...
    let list = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "list",
            "--registry",
...
fn registry_commands_manage_signed_manifest_index() {
    ...
    let sign = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "sign",
            "--registry",
    ...
    let verify_signature = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "verify-signature",
            "--registry",
...
fn registry_revoke_marks_entry_and_clears_signature() {
    ...
    assert!(
        Command::new(env!("CARGO_BIN_EXE_rivun"))
            .args(["registry", "init", "--out", registry_path.to_str().unwrap()])
            .output()
            .unwrap()
            .status
    ...
    let revoke = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "revoke",
            "--registry",
...
fn registry_deprecate_marks_entry_and_resolve_skips_it() {
    ...
    let deprecate = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "deprecate",
            "--registry",
    ...
    let resolve = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "resolve",
            "--registry",
...
fn agent_schema_and_validate_accept_agent_messages() {
    ...
    let schema = Command::new(env!("CARGO_BIN_EXE_rivun"))
    ...
    assert_eq!(
        schema_json["x-rivun"]["content_type"],
        "application/rivun-agent+json"
    );

    let validate = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "agent",
            "validate",
            "--input",
    ...
    assert_eq!(report["subject"], "rivun.agent.intent");
}
...
fn pact_cli_create_sign_verify_and_bundle_verify() {
    ...
    let bundle_export = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "pact",
            "bundle",
            "export",
    ...
    let bundle_verify = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "pact",
            "bundle",
            "verify",
...
fn agent_status_builds_valid_message_file() {
    ...
    let report: serde_json::Value = serde_json::from_slice(&validate.stdout).unwrap();
    assert_eq!(report["subject"], "rivun.agent.status");
}
...
fn agent_delegate_builds_request_and_response_messages() {
    let request = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "agent",
            "delegate",
            "--session-id",
    ...
    let response = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "agent",
            "delegate",
            "--response",
...
fn registry_resolve_selects_highest_compatible_active_entry() {
    ...
    let abi_filtered = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "resolve",
            "--registry",
    ...
    let missing = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "resolve",
            "--registry",
...
"#,
    ...
    .unwrap();
    let receiver_config = RivunNodeConfig::from_path(&receiver_config_path).unwrap();
    let receiver_node = RivunNode::from_config(receiver_config).await.unwrap();
...
async fn registry_bundle_pull_manifest_fetches_peer_bundle_manifest() {
    ...
    let pulled_manifest_path = dir.path().join("pulled-rivunstore.bundle.json");
    ...
    std::fs::write(
        bundle_path.join("rivunstore.bundle.json"),
...
"#,
    ...
    .unwrap();
    let receiver_node =
        RivunNode::from_config(RivunNodeConfig::from_path(&receiver_config_path).unwrap())
            .await
            .unwrap();
    let sender_config = write_sender_config(
        &dir,
...
"#,
    ...
    let receiver_one_node =
        RivunNode::from_config(RivunNodeConfig::from_path(&receiver_one_config_path).unwrap())
            .await
            .unwrap();
    let receiver_two_node =
        RivunNode::from_config(RivunNodeConfig::from_path(&receiver_two_config_path).unwrap())
            .await
            .unwrap();
...
fn registry_bundle_export_verify_and_import_round_trip() {
    ...
    let export = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "bundle",
            "export",
    ...
    let import = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "registry",
            "bundle",
            "import",
    ...
    assert!(imported_path.join("rivunstore.bundle.json").exists());
...
fn receipts_import_verify_export_and_compact_journal() {
    ...
    let compact = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "receipts",
            "compact",
            "--dir",
...
fn receipts_import_rejects_tampered_jsonl() {
    ...
    let failed = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "receipts",
            "import-jsonl",
            "--in",
...
fn poa_validator_set_create_verify_and_apply_config() {
    ...
    assert_eq!(report["required_threshold"], 2);
    let applied =
        RivunNodeConfig::from_toml_str(&std::fs::read_to_string(&applied_config_path).unwrap())
            .unwrap();
...
async fn send_with_action_builds_universal_action_envelope() {
    ...
    let inbound = timeout(Duration::from_secs(2), receiver_endpoint.recv())
        ...
        .unwrap();
    verify_frame(&sender.verifying_key(), &inbound.frame).unwrap();
    let envelope = RivunEnvelopeRef::parse(&inbound.frame.payload).unwrap();
...
async fn send_with_kind_event_builds_universal_envelope() {
    ...
    verify_frame(&sender.verifying_key(), &inbound.frame).unwrap();
    let envelope = RivunEnvelopeRef::parse(&inbound.frame.payload).unwrap();
    assert_eq!(envelope.kind(), RivunMessageKind::Event);
...
async fn send_requires_consensus_attaches_poa_certificate() {
    ...
    let envelope = RivunEnvelopeRef::parse(&safety.frame.payload).unwrap();
    assert_eq!(envelope.kind(), RivunMessageKind::Action);
    ...
    assert!(
        safety
            ...
            .header
            .flags
            .contains(RivunFlags::REQUIRES_CONSENSUS)
    );
...
"#,
    ...
    .unwrap();
    let validator_node =
        RivunNode::from_config(RivunNodeConfig::from_path(&validator_config_path).unwrap())
            .await
            .unwrap();
...
fn peer_rotate_and_revoke_update_config() {
    ...
    let rotated =
        RivunNodeConfig::from_toml_str(&std::fs::read_to_string(&rotated_path).unwrap()).unwrap();
    ...
    let revoked =
        RivunNodeConfig::from_toml_str(&std::fs::read_to_string(&revoked_path).unwrap()).unwrap();
...
"#,
    ...
    let check = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "check-config",
            "--config",
            config_path.to_str().unwrap(),
    ...
    let doctor = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "doctor",
            "--config",
            config_path.to_str().unwrap(),
...
fn memory_put_query_and_verify_round_trip() {
    ...
    let put = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "memory",
            "put",
            "--dir",
    ...
    let query = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "memory",
            "query",
            "--dir",
...
fn memory_export_evidence_and_incident_snapshot_are_payload_free() {
    ...
    let evidence = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "memory",
            "export-evidence",
            "--dir",
...
"#,
    ...
    let snapshot = Command::new(env!("CARGO_BIN_EXE_rivun"))
        .args([
            "incident",
            "snapshot",
            "--config",
...
fn check_config_rejects_invalid_driver_abi() {
    ...
    assert!(
        stderr.contains("missing required export `rivun_execute`"),
        "stdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        stderr
    );
