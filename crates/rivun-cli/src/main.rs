...
use rivun_core::{ED25519_SIGNATURE_LEN, PoaAttestation, RivunFlags, RivunFrame, RivunHeader, now_micros};
use rivun_crypto::{
...
use rivun_envelope::{
    DEFAULT_CONTENT_TYPE as DEFAULT_ENVELOPE_CONTENT_TYPE, RivunEnvelope, RivunEnvelopeRef,
    RivunMessageKind,
};
use rivun_gateway::{AgentGatewayServer, GatewayConfig, ProvenanceChainDigest};
use rivun_ledger::{
...
use rivun_memory::{MEMORY_SCHEMA_VERSION, MemoryJournalStore, MemoryPut, MemoryQuery, MemoryStore};
use rivun_net::{Peer, TransportKey, RivunEndpoint, RivunEndpointConfig};
use rivun_node::{
    ...
    RivunNode, RivunNodeConfig, build_discovery_advertisement, describe_capabilities,
    sign_discovery_advertisement,
};
use rivun_pact::{
    PACT_BUNDLE_SUBJECT, PACT_CONTENT_TYPE, PACT_RECORD_SUBJECT, PACT_REVOKE_SUBJECT,
    PACT_SCHEMA_VERSION, PACT_VERIFY_SUBJECT, Validate as RivunPactValidate, RivunPact, RivunPactBundle,
    RivunPactRevocation, pact_json_schema,
};
use rivun_policy::{PolicyDecision, PolicyInput, PolicySet};
use rivun_router::{RouteMessage, RouteTable};
use rivun_schema::{MessageContract, MessageParts};
use rivun_store::{
...
#[command(
    name = "rivun",
    version,
    about = "Universal low-latency Rivun protocol toolkit"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    Keygen {
        #[arg(long, default_value = ".rivun/node.key")]
        out: PathBuf,
        /// Overwrite an existing key file.
        #[arg(long)]
        force: bool,
    },
    /// Run a Rivun node daemon from a TOML config file.
    Run {
        #[arg(long, default_value = "rivun.toml")]
        config: PathBuf,
    ...
    /// Inspect an encoded Rivun frame file.
    Inspect {
        frame: PathBuf,
        #[arg(long)]
    ...
    /// Inspect and query Rivun capabilities.
    Capability {
        #[command(subcommand)]
    ...
    /// Create, sign, verify, revoke, and bundle Rivun PACT records.
    Pact {
        #[command(subcommand)]
        command: PactCommand,
    },
    ...
    /// Create or verify signed RivunStore driver manifests.
    #[command(name = "driver-manifest")]
    DriverManifest {
        #[command(subcommand)]
        command: DriverManifestCommand,
    },
    /// Manage local RivunStore registry index files.
    Registry {
        #[command(subcommand)]
...
enum CapabilityCacheCommand {
    ...
    List {
        #[arg(long, default_value = ".rivun/capabilities.jsonl")]
        path: PathBuf,
        #[arg(long)]
        peer: Option<Uuid>,
        #[arg(long)]
...
enum PactCommand {
    ...
    /// Sign a PACT record with an existing Rivun node key.
    Sign {
        #[arg(long)]
        input: PathBuf,
        #[arg(long)]
...
enum MemoryCommand {
    ...
    Put {
        #[arg(long, default_value = ".rivun/memory")]
        dir: PathBuf,
        ...
        #[arg(long)]
        payload_file: Option<PathBuf>,
        #[arg(long, default_value_t = rivun_memory::DEFAULT_MEMORY_MAX_RECORD_BYTES)]
...
enum DriverManifestCommand {
    ...
    Create {
        ...
        #[arg(long, default_value_t = rivun_capability::DEFAULT_MAX_HOST_CALL_BYTES)]
...
enum RegistryCommand {
    /// Create an empty local RivunStore registry index.
    Init {
    ...
    /// Pull a remote peer's RivunStore registry index over Rivun control messages.
    Pull {
        #[arg(long, default_value = "rivun.toml")]
        config: PathBuf,
        #[arg(long)]
    ...
    /// Export, verify, or import filesystem RivunStore bundles.
    Bundle {
        #[command(subcommand)]
...
enum RegistryBundleCommand {
    ...
    /// Pull a peer's RivunStore bundle manifest over Rivun control messages.
    PullManifest {
        #[arg(long, default_value = "rivun.toml")]
        config: PathBuf,
        #[arg(long)]
    ...
    /// Verify a RivunStore bundle directory.
    Verify {
        #[arg(long)]
        bundle: PathBuf,
        #[arg(long)]
    ...
    /// Import a verified RivunStore bundle into a local directory.
    Import {
        #[arg(long)]
        bundle: PathBuf,
        #[arg(long)]
...
enum ReceiptsCommand {
    /// Pull signed receipts from a configured peer over Rivun control messages.
    Pull {
        #[arg(long, default_value = "rivun.toml")]
        config: PathBuf,
        #[arg(long)]
...
enum GatewayCommand {
    ...
    Start {
        /// Optional config path (e.g. rivun.toml) to load node configuration.
        #[arg(long)]
        config: Option<PathBuf>,
...
fn main() -> Result<()> {
    std::thread::Builder::new()
        ...
        .context("failed to spawn rivun-cli main thread")?
        .join()
        .map_err(|_| anyhow!("rivun-cli main thread panicked"))?
}
...
async fn run(config_path: &Path, strict: bool) -> Result<()> {
    let config = RivunNodeConfig::from_path(config_path)?;
    if strict {
    ...
    let observability_http_bind = config
        ...
        .transpose()?;
    let node = Arc::new(RivunNode::from_config(config).await?);
...
fn doctor(config_path: &Path, json: bool, strict: bool) -> Result<()> {
    let report = match RivunNodeConfig::from_path(config_path).and_then(|config| config.validate()) {
...
fn build_doctor_report(
    config_path: &Path,
    report: &rivun_node::ConfigValidationReport,
) -> DoctorReport {
    ...
    checks.push(if let Some(addr) = report.observability_http_bind {
        ...
        DoctorCheck::warn(
            "observability HTTP",
            "observability.http_bind is not configured; /metrics and /healthz will not be served by rivun run",
        )
    });
...
fn driver_provenance_check(report: &rivun_node::ConfigValidationReport) -> DoctorCheck {
...
fn registry_policy_check(report: &rivun_node::ConfigValidationReport) -> DoctorCheck {
...
fn peer_trust_check(report: &rivun_node::ConfigValidationReport) -> DoctorCheck {
...
fn capability_policy_check(report: &rivun_node::ConfigValidationReport) -> DoctorCheck {
...
fn message_policy_check(report: &rivun_node::ConfigValidationReport) -> DoctorCheck {
...
fn message_schema_check(report: &rivun_node::ConfigValidationReport) -> DoctorCheck {
...
fn peer_grant_routes_check(report: &rivun_node::ConfigValidationReport) -> DoctorCheck {
...
async fn send(options: SendOptions<'_>) -> Result<()> {
    ...
    let endpoint = RivunEndpoint::bind(RivunEndpointConfig::new(bind, keypair.node_id())).await?;
    ...
    for message in messages {
        let mut flags = RivunFlags::ENCRYPTED;
        if message.requires_consensus {
            flags |= RivunFlags::REQUIRES_CONSENSUS;
        }
...
async fn pull_poa_validator_set(options: PoaValidatorSetPullOptions<'_>) -> Result<()> {
    ...
    let response = loop {
        ...
        let envelope = RivunEnvelopeRef::parse(&inbound.frame.payload)
...
async fn pull_receipts(options: ReceiptPullOptions<'_>) -> Result<()> {
    ...
    let request = ReceiptReplicationRequest {
        schema_version: rivun_ledger::RECEIPT_REPLICATION_SCHEMA_VERSION,
        after_processed_at_micros,
        until_processed_at_micros,
        limit: Some(limit),
        kind,
...
async fn gateway_start(opts: GatewayStartOptions<'_>) -> Result<()> {
    let (node, keypair, policy_set, journal, memory) = if let Some(cfg_path) = opts.config_path {
        let node_config = RivunNodeConfig::from_path(cfg_path)?;
        ...
        let node = Arc::new(RivunNode::from_config(node_config.clone()).await?);
        ...
        let keypair = if Path::new(".rivun/node.key").exists() {
            Keypair::from_key_file_toml(&fs::read_to_string(".rivun/node.key")?)
                .ok()
                .map(Arc::new)
        } else {
...
fn provenance_verify(
    ...
    let public_key = if let Some(hex_str) = public_key_hex {
        ...
        kp.verifying_key()
    } else if Path::new(".rivun/node.key").exists() {
        let key_str = fs::read_to_string(".rivun/node.key")?;
        let kp = Keypair::from_key_file_toml(&key_str)?;
        kp.verifying_key()
    } else {
...
fn fleet_doctor(
    ...
    let (node_id, receipts_dir, memory_dir) = if config_path.exists() {
        if let Ok(config) = rivun_node::RivunNodeConfig::from_path(config_path) {
            let key =
                load_keypair(&config.key_file).unwrap_or_else(|_| rivun_crypto::Keypair::generate());
            (
                key.node_id(),
    ...
    let mut topology = rivun_telemetry::FleetTopology::new(node_id, "default");
    if let Some(peer_id) = peer {
        topology.register_node(rivun_telemetry::FleetNodeState {
            node_id: peer_id,
            addr: None,
            trust_status: "trusted".to_string(),
            health_status: rivun_telemetry::FleetNodeHealth::Healthy,
    ...
    if json {
        ...
        println!("=== Rivun Fleet Doctor Report ===");
...
fn incident_snapshot(options: IncidentSnapshotOptions<'_>) -> Result<()> {
    let node_id = if options.config_path.exists() {
        if let Ok(config) = RivunNodeConfig::from_path(options.config_path) {
    ...
    let metrics_text = "# HELP rivun_replay_rejections_total Total replay rejections\n# TYPE rivun_replay_rejections_total counter\nrivun_replay_rejections_total 0\n";
    let live_snapshot =
        rivun_telemetry::IncidentCapturer::capture(node_id, metrics_text, Some(options.config_path));
    ...
    if is_tar {
        let archive_bytes = if is_gz {
            rivun_telemetry::IncidentCapturer::build_tar_gz_archive(&live_snapshot)?
        } else {
            rivun_telemetry::IncidentCapturer::build_tar_archive(&live_snapshot)?
        };
    ...
    let loaded = RivunNodeConfig::from_path(options.config_path);
...
async fn certify_frame_with_network_poa(
    ...
    for (validator_node, _) in &validator_keys {
        ...
        let request_frame = RivunFrame::new(
            keypair.node_id(),
            *validator_node,
            RivunFlags::ENCRYPTED,
            envelope.encode(),
    ...
    while attestations.len() < threshold as usize && started.elapsed() < timeout_budget {
        ...
        verify_frame(public_key, &inbound.frame)?;
        let envelope = match RivunEnvelopeRef::parse(&inbound.frame.payload) {
...
fn build_messages(options: BuildMessageOptions) -> Result<Vec<OutboundMessage>> {
    ...
    if let Some(kind) = kind {
        ...
        let kind: RivunMessageKind = kind.parse()?;
        ...
        let mut envelope = RivunEnvelope::new(kind, subject.clone(), content_type, payload_bytes)?;
...
fn select_refresh_peers(config: &RivunNodeConfig, selected: &[Uuid]) -> Result<Vec<Uuid>> {
...
fn parse_discovery_services(
    specs: &[String],
    capability_advertisement: &rivun_capability::CapabilityAdvertisement,
...
fn default_discovery_addr(config: &RivunNodeConfig) -> Option<String> {
    config
        .bind
        .parse::<SocketAddr>()
        .ok()
...
fn configured_peer_for_control<'a>(
    config: &'a RivunNodeConfig,
    target: Uuid,
    operation: &str,
) -> Result<&'a rivun_node::PeerConfig> {
    config
        .peers
        .iter()
        .find(|peer| peer.node_id == target)
...
const EVIDENCE_MANIFEST_SIGNATURE_DOMAIN: &[u8] = b"Rivun-EVIDENCE-BUNDLE-MANIFEST-v1";
...
fn memory_export_evidence(options: MemoryExportEvidenceOptions<'_>) -> Result<()> {
    ...
    let bundle = EvidenceBundle {
        ...
        limitations: vec![
            ...
            "re-verify the referenced memory and receipt journals with `rivun memory verify` and `rivun receipts verify`".to_string(),
        ],
    };
...
fn memory_record_summary(record: &rivun_memory::MemoryRecord) -> EvidenceMemoryRecordSummary {
...
struct TrustEnrollDocument {
    peers: Vec<rivun_node::PeerConfig>,
}
...
const PEER_INVITE_SIGNATURE_DOMAIN: &[u8] = b"Rivun-PEER-INVITE-v1";
...
fn peer_invite(options: PeerInviteOptions<'_>) -> Result<()> {
    let config = RivunNodeConfig::from_path(options.config_path)?;
    let keypair = load_keypair(&config.key_file)?;
...
fn load_raw_node_config(path: &Path) -> Result<RivunNodeConfig> {
    let input = fs::read_to_string(path)
        .with_context(|| format!("failed to read node config {}", path.display()))?;
    RivunNodeConfig::from_toml_str(&input)
        .with_context(|| format!("failed to parse node config {}", path.display()))
}

fn ensure_peer_absent(config: &RivunNodeConfig, node_id: Uuid) -> Result<()> {
...
fn pact_revoke(options: PactRevokeOptions<'_>) -> Result<()> {
    ...
    pact.status = rivun_pact::RivunPactStatus::Revoked;
    write_json_output(&pact, options.out, options.force)
}
...
fn pact_bundle_verify(input: &Path, now_micros: Option<u64>, json: bool) -> Result<()> {
    let bundle: RivunPactBundle = load_json_file(input)?;
...
fn load_pact(path: &Path) -> Result<RivunPact> {
    let pact: RivunPact = load_json_file(path)?;
    pact.validate()?;
    Ok(pact)
}
...
fn schema_validate(contract_path: &Path, envelope_path: &Path, json: bool) -> Result<()> {
    ...
    let envelope = RivunEnvelopeRef::parse(&envelope_bytes).context("invalid ZENV envelope")?;
...
fn pack_build(pack_dir: &Path, out: Option<PathBuf>, json: bool) -> Result<()> {
    let bundle = rivun_store::DomainPackBundle::build_from_dir(pack_dir)?;
...
fn pack_sign(bundle_path: &Path, key_path: &Path, out: Option<PathBuf>, json: bool) -> Result<()> {
    let bundle = rivun_store::DomainPackBundle::open_from_file(bundle_path)?;

    let keypair = read_keypair_file(key_path)?;

    let signature = rivun_store::DomainPackBundleSignature::sign(
...
fn read_keypair_file(key_path: &Path) -> Result<rivun_crypto::Keypair> {
    ...
    if raw.len() == 32 {
        ...
        return Ok(rivun_crypto::Keypair::from_secret_bytes(arr));
    }
    if raw.len() == 64 {
        let mut arr = [0u8; 32];
...
fn pack_verify(
    ...
    if sig_path.exists() {
        match fs::read_to_string(&sig_path) {
            Ok(sig_json) => {
                match serde_json::from_str::<rivun_store::DomainPackBundleSignature>(&sig_json) {
                    Ok(sig) => {
    ...
    if !no_policy_check {
        let val_res = rivun_store::DomainPackPolicyValidator::validate_bundle_policies(&bundle);
        if !val_res.valid {
            policy_ok = false;
...
fn pack_install(
    ...
    if sig_path.exists() {
        ...
        let sig: rivun_store::DomainPackBundleSignature = serde_json::from_str(&sig_json)?;
        ...
        && let Some(deps_arr) = pack_toml.get("dependencies").and_then(|v| v.as_array())
    {
        for dep in deps_arr {
            ...
            let optional = dep
                ...
                .unwrap_or(false);
            if !pack_id.is_empty() {
                declared_deps.push(rivun_store::DomainPackDependencySpec {
                    pack_id,
                    version_req,
                    optional,
                });
    ...
    let mut registry = if registry_file.exists() {
        ...
        serde_json::from_str::<rivun_store::DomainPackRegistry>(&json_str).unwrap_or_else(|_| {
            rivun_store::DomainPackRegistry {
                schema_version: 1,
                generated_by: None,
                channel: None,
    ...
    let resolver = rivun_store::DomainPackDependencyResolver::new(&registry);
    ...
    let entry = rivun_store::DomainPackRegistryEntry {
        id: bundle.manifest.pack_id.clone(),
        name: bundle.manifest.name.clone(),
        version: bundle.manifest.version.clone(),
        status: bundle.manifest.status,
        risk: rivun_store::DomainPackRisk::Low,
        description: None,
        deprecated_reason: None,
        revoked_reason: None,
        author_node_id: Uuid::nil(),
        compatibility: rivun_store::DomainPackCompatibility {
            min_rivun_version: None,
            max_rivun_version: None,
            runtimes: Vec::new(),
            abi_versions: Vec::new(),
            rivun_version_req: ">=0.1.0".to_string(),
...
fn pack_audit(pack_path: &Path, max_risk: Option<String>, json: bool) -> Result<()> {
    let max_risk_enum = if let Some(r) = max_risk.as_deref() {
        match r.to_lowercase().as_str() {
            "critical" => Some(rivun_store::DomainPackRisk::Critical),
            "high" => Some(rivun_store::DomainPackRisk::High),
            "medium" => Some(rivun_store::DomainPackRisk::Medium),
            "low" => Some(rivun_store::DomainPackRisk::Low),
    ...
    let report = if pack_path.is_file() || pack_path.extension().is_some_and(|ext| ext == "zpack") {
        let bundle = rivun_store::DomainPackBundle::open_from_file(pack_path)?;
        rivun_store::audit_bundle(&bundle, max_risk_enum)?
    } else {
        rivun_store::audit_pack_dir(pack_path, max_risk_enum)?
    };
            ...
            match fs::read_to_string(&resolved) {
                Ok(input) => {
                    if let Err(error) = PolicySet::from_toml_str(&input) {
                        errors.push(format!(
                            "{label} failed policy validation through rivun-policy: {error}"
                        ));
                    }
                }
                Err(error) => {
...
fn verify_agent_message_fixture(value: &serde_json::Value, errors: &mut Vec<String>) {
        ...
        .and_then(|bytes| AgentMessage::from_json_slice(&bytes).ok())
    {
        ...
        None => errors.push("body_json must parse as a rivun-agent message".to_string()),
    }
}
...
fn verify_pact_bundle_fixture(value: &serde_json::Value, errors: &mut Vec<String>) {
    ...
    match serde_json::from_value::<RivunPactBundle>(body.clone()) {
        Ok(bundle) => {
            if let Err(error) = bundle.verify(None) {
                errors.push(format!(
                    "body_json must verify as a rivun-pact bundle: {error}"
                ));
            }
        }
        Err(error) => errors.push(format!(
            "body_json must parse as a rivun-pact bundle: {error}"
        )),
    }
}
...
fn verify_pact_record_body(body: &serde_json::Value, errors: &mut Vec<String>) {
    match serde_json::from_value::<RivunPact>(body.clone()) {
        Ok(pact) => {
            if let Err(error) = pact.verify(None) {
                errors.push(format!(
                    "body_json must verify as a rivun-pact record: {error}"
                ));
            }
        }
        Err(error) => errors.push(format!(
            "body_json must parse as a rivun-pact record: {error}"
        )),
    }
}
...
fn verify_sdk_fixture_coverage(sdk_dir: &Path) -> SdkFixtureVerification {
    ...
    if sdk_dir.join("package.json").exists() && sdk_dir.join("test").is_dir() {
        ...
        verify_sdk_file_contains(sdk_dir, "pyproject.toml", &["rivun-sdk"], &mut sdk);
        ...
        verify_sdk_file_contains(
            ...
            &[
                ...
                "artifact_hash_uses_canonical_rivun_store_blake3",
            ],
            &mut sdk,
        );
        verify_sdk_file_contains(
...
async fn fetch_registry_index_from_peer(
    ...
    let request = RegistryIndexRequest {
        schema_version: rivun_store::REGISTRY_INDEX_SYNC_SCHEMA_VERSION,
...
async fn pull_registry_bundle_manifest(
    ...
    let request = RegistryBundleManifestRequest {
        schema_version: rivun_store::REGISTRY_BUNDLE_SCHEMA_VERSION,
        require_publication,
        require_drivers,
    };
...
fn registry_mirror_targets(config: &RivunNodeConfig, requested: &[Uuid]) -> Result<Vec<Uuid>> {
...
fn export_registry_bundle(options: RegistryBundleExportOptions<'_>) -> Result<()> {
    ...
    let registry_hash = rivun_store::registry_hash(&registry)?;
    ...
    let manifest = RegistryBundleManifest::new(
        Some(format!("rivun-cli {}", env!("CARGO_PKG_VERSION"))),
        registry_rel,
        registry_hash,
        publication_rel,
        publication_hash,
    ...
    manifest.validate()?;
    let manifest_rel = "rivunstore.bundle.json";
    write_text_file(
...
fn verify_registry_bundle(
    ...
    let registry = load_driver_registry(&registry_path)?;
    registry.verify_signature()?;
    let actual_registry_hash = rivun_store::registry_hash(&registry)?;
...
fn import_registry_bundle(options: RegistryBundleImportOptions<'_>) -> Result<()> {
    ...
    copy_bundle_file(bundle, "rivunstore.bundle.json", out, force)?;
...
fn create_registry(out: &Path, force: bool) -> Result<()> {
    ...
    let registry = DriverRegistry::empty(Some(format!("rivun-cli {}", env!("CARGO_PKG_VERSION"))));
...
fn load_registry_bundle_manifest(bundle: &Path) -> Result<RegistryBundleManifest> {
    let path = bundle.join("rivunstore.bundle.json");
...
fn verify_manifest_matches_registry_entry(
    entry: &rivun_store::DriverRegistryEntry,
...
fn verify_bundle_entry_matches_registry(
    ...
    registry_entry: &rivun_store::DriverRegistryEntry,
) -> Result<()> {
...
fn inspect(
    ...
    let bytes = fs::read(frame_path)
        .with_context(|| format!("failed to read frame {}", frame_path.display()))?;
    let frame = RivunFrame::decode(&bytes)?;
    let mut verified = None;
    let envelope = RivunEnvelopeRef::parse(&frame.payload).ok();
    ...
    println!(
        ...
        serde_json::to_string_pretty(&serde_json::json!({
            ...
            "payload_len": frame.header.rivun_len,
            "signature_hint": hex::encode(frame.header.rivun_sign),
...
async fn cluster(command: ClusterCommand) -> Result<()> {
    match command {
        ClusterCommand::Up {
            ...
            println!(
                "==> Spawning in-memory Rivun cluster topology (nodes={nodes}, base_port={base_port})..."
            );
            ...
            // Create mutual mesh
            let mut meshes: Vec<rivun_net::GossipMesh> = mesh_nodes
                .iter()
                .map(|(id, ep)| rivun_net::GossipMesh::new(*id, ep))
                .collect();
...
async fn swarm(command: SwarmCommand) -> Result<()> {
    match command {
        SwarmCommand::Bench {
            ...
            let mut mesh = rivun_net::GossipMesh::new(Uuid::new_v4(), "127.0.0.1:9000");
        ...
        SwarmCommand::PartitionTest {
            ...
            let now = 20_000_000;
            let clk = rivun_net::VectorClock::new();
