//! High-level agent protocol contracts for ZAP.
//!
...
pub const AGENT_CONTENT_TYPE: &str = "application/zap-agent+json";

pub const AGENT_INTENT_SUBJECT: &str = "zap.agent.intent";
pub const AGENT_SESSION_SUBJECT: &str = "zap.agent.session";
pub const AGENT_DELEGATION_REQUEST_SUBJECT: &str = "zap.agent.delegation.request";
pub const AGENT_DELEGATION_RESPONSE_SUBJECT: &str = "zap.agent.delegation.response";
pub const AGENT_CAPABILITY_NEGOTIATION_REQUEST_SUBJECT: &str =
    "zap.agent.capability_negotiation.request";
pub const AGENT_CAPABILITY_NEGOTIATION_RESPONSE_SUBJECT: &str =
    "zap.agent.capability_negotiation.response";
pub const AGENT_STATUS_SUBJECT: &str = "zap.agent.status";
pub const AGENT_RESULT_SUBJECT: &str = "zap.agent.result";
pub const AGENT_ERROR_SUBJECT: &str = "zap.agent.error";
...
pub type Result<T> = std::result::Result<T, ZapAgentError>;

#[derive(Debug, Error)]
pub enum ZapAgentError {
    ...
    fn validate(&self) -> Result<()> {
        ...
        match self.decision {
            DelegationDecision::Accepted if self.assigned_agent.is_none() => {
                Err(ZapAgentError::AcceptedDelegationMissingAssignee)
            }
            DelegationDecision::Rejected if self.reason_is_empty() => {
                Err(ZapAgentError::RejectedDelegationMissingReason)
            }
            _ => {
...
impl Validate for CapabilityNegotiationRequest {
    fn validate(&self) -> Result<()> {
        ...
        {
            return Err(ZapAgentError::EmptyCapabilityNegotiation {
...
impl Validate for AgentStatusUpdate {
    fn validate(&self) -> Result<()> {
        ...
        if let Some(progress) = self.progress_per_mille
            && progress > 1000
        {
            return Err(ZapAgentError::InvalidIdentifier {
...
impl AgentErrorInfo {
    ...
    fn validate_with_depth(&self, depth: usize) -> Result<()> {
        if depth > MAX_ERROR_DEPTH {
            return Err(ZapAgentError::ErrorCauseTooDeep {
                max: MAX_ERROR_DEPTH,
            });
        }
...
impl Validate for AgentResult {
    fn validate(&self) -> Result<()> {
        ...
        if !self.status.is_terminal() {
            return Err(ZapAgentError::ResultStatusNotTerminal);
        }
        if self.status == AgentStatus::Failed && self.error.is_none() {
            return Err(ZapAgentError::FailedResultMissingError);
        }
...
fn validate_uuid(entity: &'static str, field: &'static str, value: Uuid) -> Result<()> {
    if value.is_nil() {
        return Err(ZapAgentError::NilUuid { entity, field });
    }
    Ok(())
}
...
fn validate_timestamp_order(entity: &'static str, created: u64, updated: u64) -> Result<()> {
    if updated < created {
        return Err(ZapAgentError::InvalidTimestampOrder { entity });
    }
    Ok(())
}
...
fn validate_non_empty(entity: &'static str, field: &'static str, value: &str) -> Result<()> {
    if value.trim().is_empty() {
        return Err(ZapAgentError::EmptyField { entity, field });
    }
    Ok(())
}
...
fn validate_max_len(
    ...
    if value.len() > max {
        return Err(ZapAgentError::FieldTooLong { entity, field, max });
    }
    Ok(())
}
...
mod tests {
    ...
    fn validates_session_timestamps() {
        ...
        assert!(matches!(
            session.validate(),
            Err(ZapAgentError::InvalidTimestampOrder {
    ...
    fn capability_negotiation_must_not_be_empty() {
        ...
        assert!(matches!(
            request.validate(),
            Err(ZapAgentError::EmptyCapabilityNegotiation { .. })
        ));
    }
    ...
    fn result_requires_terminal_status_and_error_for_failure() {
        ...
        assert!(matches!(
            result.validate(),
            Err(ZapAgentError::ResultStatusNotTerminal)
        ));

        result.status = AgentStatus::Failed;
        assert!(matches!(
            result.validate(),
            Err(ZapAgentError::FailedResultMissingError)
        ));
    ...
    fn validates_nested_error_depth() {
        ...
        assert!(matches!(
            error.validate(),
            Err(ZapAgentError::ErrorCauseTooDeep { .. })
        ));
    }
}
