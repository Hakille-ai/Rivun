...
use uuid::Uuid;
use rivun_net::RivunNetError;
use rivun_net::durable_replay::DurableNonceStore;
        ...
        for nonce in &initial_nonces {
            if store.contains(nonce) {
                let err = store
                    ...
                    .unwrap_err();
                if matches!(err, RivunNetError::ReplayedDatagramNonce { .. }) {
                    rejected += 1;
                }
            }
        }
...
fn stress_test_nonce_store_clock_jumps() {
    ...
    // Scenario B: Clock jump backward (system clock set back 1 hour)
    {
        ...
        assert!(matches!(err, RivunNetError::ReplayedDatagramNonce { .. }));
    }
}
