//! Encrypted UDP transport for Rivun.
//!
...
pub use mesh::{
    ...
    PeerHealthState, PeerMeshInfo, PhiAccrualDetector, SwarmMeshTopology, RivunRelayEnvelope,
};
use thiserror::Error;
use tokio::{net::UdpSocket, sync::RwLock};
use uuid::Uuid;
use rivun_core::{RivunError as CoreError, RivunFlags, RivunFrame};

pub mod noise {
    use super::{Result, RivunNetError};
    ...
    impl NoiseHandshake {
        pub fn initiator() -> Result<Self> {
            let params = NOISE_PATTERN
                .parse::<NoiseParams>()
                .map_err(|err| RivunNetError::Noise(err.to_string()))?;
            Ok(Self {
        ...
        pub fn into_transport_key(self) -> Result<[u8; 32]> {
            ...
            let len = transport
                .write_message(b"rivun-transport-key-v1", &mut ciphertext)
                .map_err(|err| RivunNetError::Noise(err.to_string()))?;
...
pub type Result<T> = std::result::Result<T, RivunNetError>;
...
impl TransportKey {
    pub fn from_hex(input: &str) -> Result<Self> {
        ...
        if bytes.len() != KEY_LEN {
            return Err(RivunNetError::InvalidTransportKeyLength(bytes.len()));
        }
...
#[derive(Debug, Clone)]
pub struct RivunEndpointConfig {
    pub bind: SocketAddr,
    pub node_id: Uuid,
    pub peers: Vec<Peer>,
...
#[derive(Debug)]
pub struct InboundRivun {
    pub peer: Peer,
    pub from_addr: SocketAddr,
    pub frame: RivunFrame,
}
...
impl RivunEndpoint {
    pub async fn bind(config: RivunEndpointConfig) -> Result<Self> {
        if config.max_datagram_size > MAX_DATAGRAM_SIZE {
            return Err(RivunNetError::DatagramTooLarge {
    ...
    pub async fn send(&self, target: Uuid, payload: Bytes) -> Result<()> {
        let frame = RivunFrame::new(self.node_id, target, RivunFlags::ENCRYPTED, payload)?;
        self.send_frame(target, &frame).await
    }

    pub async fn send_frame(&self, target: Uuid, frame: &RivunFrame) -> Result<()> {
    ...
    fn validate_outbound_frame(&self, target: Uuid, frame: &RivunFrame) -> Result<()> {
        if frame.header.source_node != self.node_id {
            return Err(RivunNetError::OutboundSourceMismatch {
        ...
        if frame.header.flags.contains(RivunFlags::BROADCAST) {
            if frame.header.target_node != Uuid::nil() {
                return Err(RivunNetError::InvalidBroadcastTarget(
                    frame.header.target_node,
                ));
            }
        } else if frame.header.target_node != target {
            return Err(RivunNetError::OutboundTargetMismatch {
                target,
    ...
    pub async fn broadcast(&self, payload: Bytes) -> Result<()> {
        ...
        let frame = RivunFrame::new(
            self.node_id,
            Uuid::nil(),
            RivunFlags::ENCRYPTED | RivunFlags::BROADCAST,
            payload,
        )?;
    ...
    pub async fn recv(&self) -> Result<InboundRivun> {
    ...
    async fn peer_by_id(&self, node_id: Uuid) -> Result<Peer> {
        self.peers
            ...
            .get(&node_id)
            .cloned()
            .ok_or(RivunNetError::UnknownPeer(node_id))
    }

    async fn decode_inbound(&self, datagram: &[u8], from_addr: SocketAddr) -> Result<InboundRivun> {
        if datagram.len() > self.max_datagram_size {
            return Err(RivunNetError::DatagramTooLarge {
                max: self.max_datagram_size,
                actual: datagram.len(),
        ...
        if envelope.target != self.node_id {
            return Err(RivunNetError::NotForThisNode(envelope.target));
        }
        let peer = self.peer_by_id(envelope.source).await?;
        if peer.addr != from_addr {
            return Err(RivunNetError::PeerAddressMismatch {
        ...
        let frame = RivunFrame::decode(&plaintext)?;

        if frame.header.source_node != envelope.source {
            return Err(RivunNetError::SourceMismatch {
        ...
        if frame.header.flags.contains(RivunFlags::BROADCAST) {
            ...
            return Err(RivunNetError::TargetMismatch {
    ...
    fn next_nonce_counter(&self) -> Result<u64> {
        self.next_nonce
            ...
            .map_err(|_| RivunNetError::NonceCounterExhausted)
    }
...
impl DatagramEnvelope {
    fn parse(datagram: &[u8]) -> Result<Self> {
        if datagram.len() < DATAGRAM_HEADER_LEN {
            return Err(RivunNetError::DatagramTooShort {
                expected: DATAGRAM_HEADER_LEN,
                actual: datagram.len(),
            });
        }
        if datagram[0..4] != DATAGRAM_MAGIC {
            return Err(RivunNetError::InvalidDatagramMagic);
        }
        if datagram[4] != DATAGRAM_VERSION {
            return Err(RivunNetError::UnsupportedDatagramVersion(datagram[4]));
        }
        let reserved = [datagram[5], datagram[6], datagram[7]];
        if reserved != [0_u8; 3] {
            return Err(RivunNetError::NonzeroDatagramReserved(reserved));
        }
...
impl NonceReplayCache {
    ...
    fn remember(&mut self, node_id: Uuid, nonce: [u8; NONCE_LEN]) -> Result<()> {
        ...
        if let Some(durable) = &mut self.durable {
            ...
            return Err(RivunNetError::ReplayedDatagramNonce { node_id });
        }
...
fn encrypt_datagram(
    ...
    let cipher = ChaCha20Poly1305::new_from_slice(&key.0).map_err(|_| RivunNetError::Aead)?;
...
mod tests {
    ...
    async fn endpoints_exchange_encrypted_frames() {
        let key = [42_u8; 32];
        let a = RivunEndpoint::bind(RivunEndpointConfig::new(
            "127.0.0.1:0".parse().unwrap(),
            id(1),
        ))
        .await
        .unwrap();
        let b = RivunEndpoint::bind(RivunEndpointConfig::new(
            "127.0.0.1:0".parse().unwrap(),
            id(2),
        ))
        .await
    ...
    async fn broadcast_sends_nil_target_frames_to_all_peers() {
        ...
        .await
        .unwrap();
        let c = RivunEndpoint::bind(RivunEndpointConfig::new(
            "127.0.0.1:0".parse().unwrap(),
            id(3),
        ))
        .await
        ...
        for inbound in [inbound_b, inbound_c] {
            ...
            assert!(inbound.frame.header.flags.contains(RivunFlags::BROADCAST));
    ...
    fn datagram_parse_rejects_nonzero_reserved_bytes() {
        ...
        assert!(matches!(
            ...
            Err(RivunNetError::NonzeroDatagramReserved([1, 0, 0]))
        ));
    }
    ...
    async fn endpoint_rejects_replayed_datagram_nonce() {
        ...
        assert!(matches!(
            ...
            Err(RivunNetError::ReplayedDatagramNonce { node_id }) if node_id == source
        ));
    }
    ...
    async fn endpoint_rejects_broadcast_frame_with_non_nil_target() {
        ...
        assert!(matches!(
            endpoint.decode_inbound(&datagram, peer_addr).await,
            Err(RivunNetError::InvalidBroadcastTarget(node_id)) if node_id == target
        ));
    }
    ...
    async fn endpoint_rejects_datagram_envelope_targeting_another_node() {
        ...
        assert!(matches!(
            endpoint.decode_inbound(&datagram, peer_addr).await,
            Err(RivunNetError::NotForThisNode(node_id)) if node_id == wrong_target
        ));
    }
    ...
    async fn inbound_nonce_cache_can_be_disabled_for_specialized_tests() {
        ...
        let target = id(2);
        let peer_addr = "127.0.0.1:9001".parse().unwrap();
        let mut config = RivunEndpointConfig::new("127.0.0.1:0".parse().unwrap(), target);
        config.inbound_nonce_cache_capacity = 0;
        let endpoint = RivunEndpoint::bind(config).await.unwrap();
        endpoint.add_peer(Peer::new(source, peer_addr, key.0)).await;
    ...
    async fn endpoint_uses_configured_nonce_prefix_for_datagrams() {
        let key = [42_u8; 32];
        let a = RivunEndpoint::bind(
            RivunEndpointConfig::new("127.0.0.1:0".parse().unwrap(), id(1))
    ...
    async fn endpoint_rejects_oversized_datagram_buffer_config() {
        let mut config = RivunEndpointConfig::new("127.0.0.1:0".parse().unwrap(), id(1));
        config.max_datagram_size = MAX_DATAGRAM_SIZE + 1;

        assert!(matches!(
            RivunEndpoint::bind(config).await,
            Err(RivunNetError::DatagramTooLarge {
                max: MAX_DATAGRAM_SIZE,
                actual
    ...
    async fn refuses_to_send_after_nonce_counter_exhaustion() {
        ...
        assert!(matches!(
            ...
            Err(RivunNetError::NonceCounterExhausted)
        ));
    }
    ...
    async fn rejects_outbound_frame_with_wrong_source() {
        ...
        assert!(matches!(
            ...
            Err(RivunNetError::OutboundSourceMismatch { endpoint: endpoint_id, frame: frame_id })
    ...
    async fn rejects_outbound_frame_with_wrong_unicast_target() {
        ...
        assert!(matches!(
            endpoint.send_frame(id(2), &frame).await,
            Err(RivunNetError::OutboundTargetMismatch { target, frame: frame_id })
    ...
    async fn rejects_outbound_broadcast_frame_with_non_nil_target() {
        ...
        assert!(matches!(
            endpoint.send_frame(id(2), &frame).await,
            Err(RivunNetError::InvalidBroadcastTarget(node_id)) if node_id == id(2)
        ));
    }
    ...
    async fn rejects_unknown_peer() {
        ...
        assert!(matches!(
            ...
            Err(RivunNetError::UnknownPeer(_))
        ));
    }
