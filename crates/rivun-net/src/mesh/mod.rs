...
pub use relay::{MAX_RELAY_HOPS, RELAY_ENVELOPE_MAGIC, RELAY_ENVELOPE_VERSION, RivunRelayEnvelope};
...
mod tests {
    ...
    fn test_relay_envelope_roundtrip_and_forwarding() {
        ...
        let relay = RivunRelayEnvelope::new(n1, n2, n3, payload.clone());
        assert_eq!(relay.hop_count, 1);

        let encoded = relay.encode();
        let decoded = RivunRelayEnvelope::decode(&encoded).expect("decode failed");
