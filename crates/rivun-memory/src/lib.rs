//! Auditable local memory stores for Rivun.
...
use thiserror::Error;
use uuid::Uuid;
use rivun_journal::{JournalProfile, JournalQuery, JournalRecord, JournalRecordInput, JournalStore};
...
pub type Result<T> = std::result::Result<T, RivunMemoryError>;
...
impl MemoryJournalStore {
    ...
    fn append_record(
        ...
        if actual > self.max_record_bytes {
            return Err(RivunMemoryError::RecordTooLarge {
...
impl MemoryStore for MemoryJournalStore {
    ...
    fn get(&self, id: Uuid) -> Result<MemoryRecord> {
        ...
        if !tombstones.is_empty() {
            return Err(RivunMemoryError::NotFound(id));
        }
        ...
        let record = records.first().ok_or(RivunMemoryError::NotFound(id))?;
...
impl MemoryStore for JsonlMemoryStore {
    ...
    fn get(&self, id: Uuid) -> Result<MemoryRecord> {
        ...
        records.remove(&id).ok_or(RivunMemoryError::NotFound(id))
    }
...
fn memory_record_from_journal(record: &JournalRecord) -> Result<MemoryRecord> {
    let id = record.id.ok_or(RivunMemoryError::NotFound(Uuid::nil()))?;
    let metadata = record
        .metadata
        .get("metadata")
        .cloned()
...
fn memory_tombstone_from_journal(record: &JournalRecord) -> Result<MemoryTombstone> {
    ...
    let record_id = record
        .tombstone_for
        .ok_or(RivunMemoryError::NotFound(Uuid::nil()))?;
    let reason = record
        .metadata
        .get("reason")
        .and_then(Value::as_str)
...
fn verify_entries(
    ...
    if require_non_empty && entries.is_empty() {
        return Err(RivunMemoryError::EmptyStore);
    }
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        if !seen_entry_ids.insert(entry_id) {
            return Err(RivunMemoryError::DuplicateEntryId { line, id: entry_id });
        }
        ...
        {
            return Err(RivunMemoryError::EntryHashMismatch { line, id: entry_id });
        }
        ...
        if entry_uses_chain && entry.previous_entry_hash() != expected_previous_hash.as_deref() {
            return Err(RivunMemoryError::EntryChainMismatch { line, id: entry_id });
        }
        ...
        match entry {
            MemoryEntry::Record(record) => {
                ...
                if record.schema_version != MEMORY_SCHEMA_VERSION {
                    return Err(RivunMemoryError::UnsupportedSchemaVersion {
                        line,
                ...
                if record.body_hash != hash_bytes(&body) {
                    return Err(RivunMemoryError::BodyHashMismatch {
                        line,
                        id: record.id,
                    });
                }
            ...
            MemoryEntry::Tombstone(tombstone) => {
                ...
                if !live_record_ids.contains(&tombstone.record_id) {
                    return Err(RivunMemoryError::TombstoneTargetMissing {
                        line,
                        id: tombstone.id,
...
fn validate_namespace(namespace: &str) -> Result<()> {
    if namespace.trim().is_empty() {
        return Err(RivunMemoryError::EmptyNamespace);
    }
    Ok(())
}
...
fn validate_subject(subject: &str) -> Result<()> {
    if subject.trim().is_empty() {
        return Err(RivunMemoryError::EmptySubject);
    }
    Ok(())
}
...
fn now_micros() -> Result<u64> {
    let duration = SystemTime::now()
        ...
        .map_err(|_| RivunMemoryError::ClockBeforeUnixEpoch)?;
...
mod tests {
    ...
    fn journal_store_round_trips_jsonl_and_prunes() {
        ...
        assert!(matches!(
            store.get(first.id),
            Err(RivunMemoryError::NotFound(_))
        ));
        assert_eq!(
    ...
    fn verify_detects_tampered_entry_hash() {
        ...
        assert!(matches!(
            store.verify(),
            Err(RivunMemoryError::EntryHashMismatch { line: 1, id }) if id == record.id
        ));
    }
    ...
    fn verify_detects_removed_middle_entry() {
        ...
        assert!(matches!(
            store.verify(),
            Err(RivunMemoryError::EntryChainMismatch { line: 2, id }) if id == third.id
        ));
    }
    ...
    fn prune_drops_orphaned_tombstones() {
        ...
        assert!(matches!(
            pruned_store.verify(),
            Err(RivunMemoryError::EmptyStore)
        ));
