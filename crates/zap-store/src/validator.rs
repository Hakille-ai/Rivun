...
fn extract_declared_paths_from_toml(toml_str: &str) -> (HashSet<String>, HashSet<String>, HashSet<String>) {
...
impl DomainPackPolicyValidator {
    pub fn validate_bundle_policies(bundle: &DomainPackBundle) -> DomainPackValidationResult {
        ...
        for (rel_path, content) in &bundle.files {
            ...
            let is_policy = declared_policies.contains(rel_path)
                || (lower_path.contains("policy") && (lower_path.ends_with(".toml") || lower_path.ends_with(".policy")));
            ...
            if is_policy {
                if let Ok(content_str) = std::str::from_utf8(content) {
                    match PolicySet::from_toml_str(content_str) {
                        ...
                        Err(e) => {
                            result.syntax_errors.push(format!("policy syntax error in {rel_path}: {e}"));
                        }
                    }
                } else {
                    result.syntax_errors.push(format!("non-UTF8 policy file in {rel_path}"));
                }
            } else if is_route {
                if let Ok(content_str) = std::str::from_utf8(content) {
                    match toml::from_str::<RouteTable>(content_str) {
                        Ok(route_table) => {
                            if let Err(e) = route_table.validate() {
                                result.syntax_errors.push(format!("route table validation error in {rel_path}: {e}"));
                            } else {
                                result.route_rule_count += route_table.routes.len();
                            }
                        }
                        Err(e) => {
                            result.syntax_errors.push(format!("route syntax error in {rel_path}: {e}"));
                        }
                    }
                } else {
                    result.syntax_errors.push(format!("non-UTF8 route file in {rel_path}"));
                }
            } else if is_schema {
                if let Ok(content_str) = std::str::from_utf8(content) {
                    if let Err(e) = serde_json::from_str::<serde_json::Value>(content_str) {
                        result.syntax_errors.push(format!("invalid JSON schema in {rel_path}: {e}"));
                    }
                } else {
                    result.syntax_errors.push(format!("non-UTF8 schema file in {rel_path}"));
                }
            }
        }
    ...
    pub fn validate_dir_policies(pack_dir: &Path) -> DomainPackValidationResult {
        ...
        let (declared_policies, declared_routes, declared_schemas) = fs::read_to_string(pack_dir.join("pack.toml"))
            .ok()
        ...
        fn walk_and_validate(
            ...
            let Ok(entries) = fs::read_dir(current_dir) else {
                result.syntax_errors.push(format!("failed to read directory {}", current_dir.display()));
                return;
            };
            ...
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    walk_and_validate(base_dir, &path, declared_policies, declared_routes, declared_schemas, result);
                    ...
                    if is_policy {
                        if let Ok(content_str) = fs::read_to_string(&path) {
                            ...
                            result.syntax_errors.push(format!("failed to read policy file {rel_path}"));
                        }
                    } else if is_route {
                        if let Ok(content_str) = fs::read_to_string(&path) {
                            match toml::from_str::<RouteTable>(&content_str) {
                            ...
                            result.syntax_errors.push(format!("failed to read route file {rel_path}"));
                        }
                    } else if is_schema {
                        if let Ok(content_str) = fs::read_to_string(&path) {
                            if let Err(e) = serde_json::from_str::<serde_json::Value>(&content_str) {
                            ...
                            result.syntax_errors.push(format!("failed to read schema file {rel_path}"));
                        }
                    }
                }
            }
        ...
        walk_and_validate(pack_dir, pack_dir, &declared_policies, &declared_routes, &declared_schemas, &mut result);
        result.valid = result.syntax_errors.is_empty();
        result
    }
}
