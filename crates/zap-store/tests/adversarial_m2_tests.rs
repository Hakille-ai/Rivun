...
fn test_path_traversal_zip_slip_vulnerability() {
    ...
    let manifest = DomainPackBundleManifest {
        schema_version: 1,
        pack_id: "com.zap.malicious".to_string(),
...
fn test_transitive_dependency_resolution() {
    ...
    let deps_a = vec![DomainPackDependencySpec {
        pack_id: "com.zap.b".to_string(),
        version_req: "^1.0.0".to_string(),
        optional: false,
    }];

    let plan = resolver.resolve("com.zap.a", "1.0.0", &deps_a).unwrap();
    ...
    assert!(
        resolved_ids.contains(&"com.zap.c".to_string()),
