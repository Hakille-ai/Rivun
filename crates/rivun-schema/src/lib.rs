//! Typed message contracts for Rivun envelopes.
//!
//! Rivun keeps model planning and machine intent outside the wire protocol. This
...
use thiserror::Error;
use rivun_envelope::RivunMessageKind;

pub const MESSAGE_CONTRACT_SCHEMA_VERSION: u8 = 1;

#[derive(Debug, Error)]
pub enum RivunSchemaError {
    ...
    InvalidKind {
        ...
        kind: String,
        #[source]
        source: rivun_envelope::RivunEnvelopeError,
    },
...
pub type Result<T> = std::result::Result<T, RivunSchemaError>;
...
impl MessageContract {
    ...
    pub fn validate_static(&self) -> Result<()> {
        ...
        if self.schema_version != MESSAGE_CONTRACT_SCHEMA_VERSION {
            return Err(RivunSchemaError::UnsupportedSchemaVersion(
                self.schema_version,
            ));
        }
        if self.kind.trim().is_empty() {
            return Err(RivunSchemaError::EmptyKind(name));
        }
        if self.kind != "*" {
            RivunMessageKind::from_str(&self.kind).map_err(|source| RivunSchemaError::InvalidKind {
                contract: name.clone(),
        ...
        if self.subject.trim().is_empty() {
            return Err(RivunSchemaError::EmptySubject(name));
        }
        if self
            .content_type
            .as_deref()
        ...
        {
            return Err(RivunSchemaError::EmptyContentType(name));
        }
        if matches!(self.max_body_bytes, Some(0)) {
            return Err(RivunSchemaError::InvalidMaxBodyBytes(name));
        }
        Ok(())
    }
    ...
    pub fn validate_message(&self, message: &MessageParts<'_>) -> Result<()> {
        ...
        if !field_matches(&self.kind, message.kind) {
            return Err(RivunSchemaError::KindMismatch {
                contract: name,
        ...
        if !subject_pattern_matches(&self.subject, message.subject) {
            return Err(RivunSchemaError::SubjectMismatch {
                contract: name,
        ...
        if let Some(expected) = &self.content_type {
            ...
            if !expected.eq_ignore_ascii_case(actual) {
                return Err(RivunSchemaError::ContentTypeMismatch {
        ...
        {
            return Err(RivunSchemaError::BodyTooLarge {
                contract: self.contract_name(),
                max,
...
impl MessageContractSet {
    ...
    pub fn validate_message(&self, message: &MessageParts<'_>) -> Result<Option<String>> {
        ...
        if self.require_match {
            return Err(RivunSchemaError::NoMatchingContract {
...
impl BodyContract {
        ...
        match self.format {
            ...
            BodyFormat::Empty => {
                if body.is_empty() {
                    ...
                    Err(RivunSchemaError::BodyNotEmpty {
            ...
            BodyFormat::Utf8 => {
                std::str::from_utf8(body)
                    .map(|_| ())
                    .map_err(|_| RivunSchemaError::BodyNotUtf8 {
                        contract: contract.to_string(),
                    })
            }
            BodyFormat::JsonValue => {
                serde_json::from_slice::<Value>(body)
                    .map(|_| ())
                    .map_err(|source| RivunSchemaError::BodyNotJson {
                        contract: contract.to_string(),
                        source,
                    })
            }
    ...
    fn validate_json_object(&self, contract: &str, body: &[u8]) -> Result<()> {
        ...
        let object = value
            .as_object()
            .ok_or_else(|| RivunSchemaError::BodyNotJsonObject {
                contract: contract.to_string(),
            })?;
        for field in &self.required_json_fields {
            if !object.contains_key(field) {
                return Err(RivunSchemaError::MissingBodyField {
                    contract: contract.to_string(),
                    field: field.clone(),
                });
            }
        ...
        if !self.allowed_json_fields.is_empty() {
            for field in object.keys() {
                ...
                {
                    return Err(RivunSchemaError::DisallowedBodyField {
                        contract: contract.to_string(),
                        field: field.clone(),
                    });
                }
...
impl MetadataContract {
    fn validate(&self, contract: &str, metadata: &[u8]) -> Result<()> {
        if let Some(max) = self.max_bytes
            && metadata.len() > max
        {
            return Err(RivunSchemaError::MetadataTooLarge {
                contract: contract.to_string(),
                max,
                actual: metadata.len(),
            });
        ...
        let value = serde_json::from_slice::<Value>(metadata).map_err(|source| {
            RivunSchemaError::MetadataNotJson {
                contract: contract.to_string(),
                source,
            }
        })?;
        let object = value
            .as_object()
            .ok_or_else(|| RivunSchemaError::MetadataNotJsonObject {
                contract: contract.to_string(),
            })?;
        for field in &self.required_json_fields {
            if !object.contains_key(field) {
                return Err(RivunSchemaError::MissingMetadataField {
                    contract: contract.to_string(),
                    field: field.clone(),
                });
            }
...
mod tests {
    ...
    fn contract_set_can_require_a_match() {
        ...
        let error = set
            .validate_message(&parts(br#"{"temperature_c":20}"#))
            .unwrap_err();
        assert!(matches!(error, RivunSchemaError::NoMatchingContract { .. }));
    }
