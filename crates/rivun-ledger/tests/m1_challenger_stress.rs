...
use rivun_ledger::{
    ...
    SignedActionReceipt, SignedReceiptSegmentManifest, RivunLedgerError, hash_bytes,
};
...
fn test_signature_and_manifest_tampering() {
    ...
    assert!(
        matches!(
            verify_res,
            Err(RivunLedgerError::InvalidSignature) | Err(RivunLedgerError::Base64(_))
        ),
    ...
    assert!(
        matches!(
            verify_res,
            Err(RivunLedgerError::SegmentManifestSignerNodeMismatch { .. })
