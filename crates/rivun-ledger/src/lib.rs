//! Signed action receipts for Rivun nodes.
//!
...
use rivun_journal::{JournalProfile, JournalQuery, JournalRecordInput, JournalStore};
...
pub const RECEIPT_REPLICATION_CONTENT_TYPE: &str = "application/rivun-receipts+json";
pub const RECEIPT_REPLICATION_REQUEST_SUBJECT: &str = "rivun.receipts.request";
pub const RECEIPT_REPLICATION_RESPONSE_SUBJECT: &str = "rivun.receipts.response";
...
pub const RECEIPT_SEGMENT_MANIFEST_CONTENT_TYPE: &str =
    "application/rivun-receipt-segment-manifest+json";
pub const RECEIPT_JOURNAL_CONTENT_TYPE: &str = "application/rivun-receipt+json";
pub const SIGNED_MANIFEST_EXTENSION: &str = "zjmanifest.json.sig";

const RECEIPT_SIGNATURE_DOMAIN: &[u8] = b"Rivun-ACTION-RECEIPT-v1";
const RECEIPT_SEGMENT_MANIFEST_SIGNATURE_DOMAIN: &[u8] = b"Rivun-RECEIPT-SEGMENT-MANIFEST-v1";
...
pub enum RivunLedgerError {
    ...
    Journal(#[from] rivun_journal::RivunJournalError),
    #[error("receipt output {0} already exists")]
    OutputExists(PathBuf),
}

pub type Result<T> = std::result::Result<T, RivunLedgerError>;
...
impl ActionReceipt {
    pub fn validate_static(&self) -> Result<()> {
        ...
        let flags = RivunFlags::from_bits(self.flags).ok_or(RivunLedgerError::InvalidReceiptField {
            field: "flags",
            reason: "contains unknown bits",
        })?;
        let flags_require_consensus = flags.contains(RivunFlags::REQUIRES_CONSENSUS);
...
impl ReceiptReplicationRequest {
    pub fn validate(&self) -> Result<()> {
        if self.schema_version != RECEIPT_REPLICATION_SCHEMA_VERSION {
            return Err(RivunLedgerError::UnsupportedReplicationSchemaVersion(
                self.schema_version,
            ));
        }
        ...
        {
            return Err(RivunLedgerError::InvalidReplicationWindow { after, until });
        }
    ...
    pub fn effective_limit(&self) -> Result<usize> {
        ...
        if limit == 0 || limit > MAX_RECEIPT_REPLICATION_LIMIT {
            return Err(RivunLedgerError::InvalidReplicationLimit {
                actual: limit,
...
impl ReceiptJournalStore {
    ...
    pub fn with_options(mut self, options: rivun_journal::JournalOptions) -> Self {
    ...
    pub fn rotate_and_seal_segment(&self, sequence: u64) -> Result<SignedReceiptSegmentManifest> {
        let keypair = self
            .keypair
            .as_ref()
            .ok_or_else(|| RivunLedgerError::InvalidReceiptField {
                field: "keypair",
    ...
    pub fn load_signed_manifest(&self, sequence: u64) -> Result<SignedReceiptSegmentManifest> {
        let path = self.signed_manifest_path(sequence);
        if !path.exists() {
            return Err(RivunLedgerError::Io(std::io::Error::new(
    ...
    pub fn export_jsonl(&self, out: &Path, force: bool) -> Result<usize> {
        if out.exists() && !force {
            return Err(RivunLedgerError::OutputExists(out.to_path_buf()));
        }
...
impl ReceiptSegmentManifest {
    pub fn from_receipts(
        ...
        let first = receipts
            .first()
            .ok_or(RivunLedgerError::EmptyReceiptSegment)?;
        let node_id = first.receipt.node_id;
        ...
        for receipt in receipts {
            ...
            if receipt.receipt.node_id != node_id {
                return Err(RivunLedgerError::ReceiptSegmentNodeMismatch {
            ...
            {
                return Err(RivunLedgerError::ReceiptSegmentOutOfOrder {
                    previous,
    ...
    pub fn validate(&self) -> Result<()> {
        if self.schema_version != RECEIPT_SEGMENT_MANIFEST_SCHEMA_VERSION {
            return Err(RivunLedgerError::UnsupportedSegmentManifestSchemaVersion(
                self.schema_version,
            ));
        }
...
impl SignedReceiptSegmentManifest {
    pub fn sign(signer: &Keypair, manifest: ReceiptSegmentManifest) -> Result<Self> {
        ...
        if manifest.node_id != signer.node_id() {
            return Err(RivunLedgerError::SegmentManifestNodeMismatch {
    ...
    pub fn verify(&self) -> Result<()> {
        ...
        if derived_node_id != self.signer_node_id {
            return Err(RivunLedgerError::SegmentManifestSignerNodeMismatch {
        ...
        verifying_key
            ...
            .map_err(|_| RivunLedgerError::InvalidSignature)
    }
...
impl ReceiptSegmentIndex {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.schema_version != RECEIPT_SEGMENT_INDEX_SCHEMA_VERSION {
            return Err(RivunLedgerError::UnsupportedSegmentIndexSchemaVersion(
                self.schema_version,
            ));
        }
        ...
        for entry in &self.entries {
            ...
            if !segment_ids.insert(entry.segment_id) {
                return Err(RivunLedgerError::DuplicateReceiptSegmentIndexEntry {
                    field: "segment_id",
            ...
            if let Some(previous) = previous_sequence {
                ...
                if entry.segment_sequence != expected {
                    return Err(RivunLedgerError::ReceiptSegmentSequenceGap {
                        expected,
            ...
            {
                return Err(RivunLedgerError::ReceiptSegmentChainMismatch {
...
impl SignedActionReceipt {
    ...
    pub fn new_message_with_pact(
        ...
        let receipt = ActionReceipt {
            ...
            consensus_required: frame.header.flags.contains(RivunFlags::REQUIRES_CONSENSUS),
    ...
    pub fn verify(&self) -> Result<()> {
        ...
        if self.receipt.node_id != self.signer_node_id {
            return Err(RivunLedgerError::ReceiptNodeMismatch {
        ...
        let derived_node_id = node_id_from_public_key(&public_key_bytes);
        if derived_node_id != self.signer_node_id {
            return Err(RivunLedgerError::SignerNodeMismatch {
                declared: self.signer_node_id,
                derived: derived_node_id,
            });
        }
...
pub(crate) fn validate_artifact_hash(field: &'static str, value: &str) -> Result<()> {
    let hash =
        value
            ...
            .ok_or_else(|| RivunLedgerError::InvalidArtifactHash {
                field,
                value: value.to_string(),
            })?;
    if hash.len() != 64 || !hash.chars().all(|ch| ch.is_ascii_hexdigit()) {
        return Err(RivunLedgerError::InvalidArtifactHash {
            field,
            value: value.to_string(),
        });
    }
...
fn build_poa_receipt(frame: &RivunFrame, required_threshold: Option<u16>) -> Option<PoaReceipt> {
...
pub(crate) fn decode_fixed<const N: usize>(encoded: &str, kind: &'static str) -> Result<[u8; N]> {
    ...
    if decoded.len() != N {
        return Err(RivunLedgerError::InvalidKeyLength {
            kind,
            expected: N,
            actual: decoded.len(),
        });
...
mod tests {
    ...
    use bytes::Bytes;
    use rivun_core::{RivunFlags, RivunFrame};
    use rivun_crypto::{certify_frame, sign_frame};
    ...
    fn receipt_detects_mutation() {
        ...
        assert!(matches!(
            receipt.verify(),
            Err(RivunLedgerError::InvalidSignature)
        ));
    }
    ...
    fn receipt_verify_rejects_invalid_static_hashes() {
        ...
        assert!(matches!(
            receipt.verify(),
            Err(RivunLedgerError::InvalidArtifactHash {
                field: "output_hash",
                ..
            })
        ));
    ...
    fn receipt_replication_request_filters_receipts() {
        ...
        assert!(matches!(
            ...
            Err(RivunLedgerError::InvalidReplicationWindow { .. })
        ));
    }
    ...
    fn receipt_replication_response_verifies_nested_receipts() {
        ...
        assert!(matches!(
            wrong_node.verify(),
            Err(RivunLedgerError::ReceiptNodeMismatch { .. })
        ));
    }
    ...
    fn receipt_replication_rejects_bad_limit() {
        ...
        assert!(matches!(
            request.validate(),
            Err(RivunLedgerError::InvalidReplicationLimit { .. })
        ));
    }
    ...
    fn receipt_segment_manifest_rejects_mixed_nodes_and_out_of_order_receipts() {
        ...
        assert!(matches!(
            ...
            Err(RivunLedgerError::ReceiptSegmentNodeMismatch { .. })
        ));
        ...
        assert!(matches!(
            ...
            Err(RivunLedgerError::ReceiptSegmentOutOfOrder { .. })
        ));
    }
    ...
    fn receipt_segment_index_rejects_chain_mismatch_and_sequence_gap() {
        ...
        assert!(matches!(
            ...
            Err(RivunLedgerError::ReceiptSegmentChainMismatch { .. })
        ));
        ...
        assert!(matches!(
            gap.validate(),
            Err(RivunLedgerError::ReceiptSegmentSequenceGap {
                expected: 2,
                actual: 3
            })
        ));
    ...
    fn signed_segment_manifest_store_integration() {
        ...
        let node = Keypair::generate();
        let source = Keypair::generate();
        let options = rivun_journal::JournalOptions {
