//! Cryptographic batch sealing and swarm quorum multi-signatures for Rivun receipts.
//!
...
use uuid::Uuid;
use rivun_crypto::{Keypair, PoaValidatorSet, node_id_from_public_key};

use crate::{
    HASH_PREFIX, MerkleMountainRange, SignedActionReceipt, RivunLedgerError, decode_fixed,
...
impl ReceiptBatchSeal {
    ...
    pub fn signing_message(&self) -> Result<Vec<u8>, RivunLedgerError> {
    ...
    pub fn validate_static(&self) -> Result<(), RivunLedgerError> {
    ...
    pub fn sign_with_validator(
        &self,
        keypair: &Keypair,
    ) -> Result<BatchValidatorSignature, RivunLedgerError> {
    ...
    pub fn verify_quorum(&self, validator_set: &PoaValidatorSet) -> Result<bool, RivunLedgerError> {
        self.validate_static()?;
        validator_set
            .validate_static()
            .map_err(RivunLedgerError::from)?;
        ...
        for val_sig in &self.validator_signatures {
            ...
            let descriptor = validator_set
                ...
                .ok_or(RivunLedgerError::InvalidReceiptField {
                    field: "validator_signatures",
...
impl SignedReceiptBatch {
    ...
    pub fn verify(&self, validator_set: Option<&PoaValidatorSet>) -> Result<bool, RivunLedgerError> {
...
mod tests {
    ...
    use rivun_crypto::{POA_VALIDATOR_SET_SCHEMA_VERSION, PoaValidatorDescriptor};
