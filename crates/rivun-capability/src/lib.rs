//! Capability discovery and permission contracts for Rivun.
//!
...
pub const CAPABILITY_QUERY_SUBJECT: &str = "rivun.capability.query";
pub const CAPABILITY_RESPONSE_SUBJECT: &str = "rivun.capability.response";
pub const CAPABILITY_ANNOUNCE_SUBJECT: &str = "rivun.capability.announce";
pub const CAPABILITY_CONTENT_TYPE: &str = "application/rivun-capability+json";
...
pub type Result<T> = std::result::Result<T, RivunCapabilityError>;
...
fn validate_capability_id(input: &str) -> Result<()> {
    if input.trim().is_empty() {
        return Err(RivunCapabilityError::EmptyCapabilityId);
    }
    if input.len() > 128 {
        return Err(RivunCapabilityError::CapabilityIdTooLong(input.to_string()));
    }
    if !input
        .bytes()
        .all(|byte| matches!(byte, b'a'..=b'z' | b'0'..=b'9' | b'.' | b':' | b'_' | b'-' | b'*'))
    {
        return Err(RivunCapabilityError::InvalidCapabilityId(input.to_string()));
    }
    Ok(())
}
...
fn verify_cache_entries(
    ...
    if require_non_empty && entries.is_empty() {
        return Err(RivunCapabilityError::EmptyCapabilityCache);
    }
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        if entry.schema_version != CAPABILITY_CACHE_SCHEMA_VERSION {
            return Err(RivunCapabilityError::UnsupportedCacheSchemaVersion {
                line,
                version: entry.schema_version,
            });
        }
        if !seen_ids.insert(entry.id) {
            return Err(RivunCapabilityError::DuplicateCacheEntryId { line, id: entry.id });
        }
        validate_cache_entry(entry)?;
        ...
        {
            return Err(RivunCapabilityError::CacheEntryHashMismatch { line, id: entry.id });
        }
        ...
        {
            return Err(RivunCapabilityError::CacheEntryChainMismatch { line, id: entry.id });
        }
...
fn validate_cache_entry(entry: &CapabilityCacheEntry) -> Result<()> {
    if entry.peer_node_id != entry.advertisement.node_id {
        return Err(RivunCapabilityError::CachePeerMismatch {
            id: entry.id,
    ...
    for grant in &entry.advertisement.grants {
        if !entry.advertisement.capabilities.contains(&grant.capability) {
            return Err(RivunCapabilityError::CacheGrantNotAdvertised {
                id: entry.id,
...
fn now_micros() -> Result<u64> {
    let duration = SystemTime::now()
        ...
        .map_err(|_| RivunCapabilityError::ClockBeforeUnixEpoch)?;
...
mod tests {
    ...
    fn capability_cache_detects_tampered_entry_hash() {
        ...
        assert!(matches!(
            cache.verify(),
            Err(RivunCapabilityError::CacheEntryHashMismatch { line: 1, id }) if id == entry.id
        ));
    }
    ...
    fn capability_cache_detects_removed_middle_entry() {
        ...
        assert!(matches!(
            cache.verify(),
            Err(RivunCapabilityError::CacheEntryChainMismatch { line: 2, id }) if id == third.id
        ));
    }
    ...
    fn capability_cache_rejects_grants_for_missing_capabilities() {
        ...
        assert!(matches!(
            ...
            Err(RivunCapabilityError::CacheGrantNotAdvertised { capability, .. })
