...
pub trait AsyncRivunDriver: Send + Sync + 'static {
...
/// Adapts any synchronous `RivunDriver` into a fully compliant `AsyncRivunDriver`.
pub struct SyncDriverAdapter<D> {
    inner: D,
}

impl<D: RivunDriver> SyncDriverAdapter<D> {
...
impl<D: RivunDriver + Send + Sync + 'static> AsyncRivunDriver for SyncDriverAdapter<D> {
    fn init<'a>(
        &'a mut self,
        _ctx: &'a mut DriverContext,
    ) -> BoxFuture<'a, Result<(), DriverError>> {
...
/// Extension trait to convert any `RivunDriver` directly into an `AsyncRivunDriver`.
pub trait RivunDriverExt: RivunDriver + Sized {
    fn into_async(self) -> SyncDriverAdapter<Self> {
        SyncDriverAdapter::new(self)
    }
}

impl<D: RivunDriver> RivunDriverExt for D {}
...
mod tests {
    ...
    impl AsyncRivunDriver for AsyncEchoDriver {
        fn execute_async<'a>(
            &'a mut self,
            _ctx: &'a mut DriverContext,
            input: DriverInput<'a>,
    ...
    impl RivunDriver for SyncEchoDriver {
