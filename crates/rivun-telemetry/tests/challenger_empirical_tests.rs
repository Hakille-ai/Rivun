...
use rivun_ledger::{ReceiptSegmentManifest, SignedReceiptSegmentManifest};
use rivun_store::DomainPackRegistry;
use rivun_telemetry::{
...
fn test_challenger_tar_gz_decompression_and_contents() {
    let node_id = Uuid::new_v4();
    let metrics = "rivun_replay_drops_total 42\nrivun_peers_active 3\n";
    ...
    while offset + 512 <= tar_bytes.len() {
        ...
        match name {
            ...
            "metrics.prom" => {
                ...
                assert!(s.contains("rivun_replay_drops_total 42"));
            }
            "diagnostics.txt" => {
                found_diagnostics = true;
                let s = std::str::from_utf8(content).unwrap();
                assert!(s.contains(&format!("Rivun Incident Snapshot Node ID: {node_id}")));
            }
