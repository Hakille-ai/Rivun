//! Deterministic route planning for ZAP envelopes.
...
pub type Result<T> = std::result::Result<T, ZapRouterError>;
    ...
    pub fn validate(&self) -> Result<()> {
        for (index, route) in self.routes.iter().enumerate() {
            ...
            if route.requires_peer_grant.is_some() && route.target.peer.is_none() {
                return Err(ZapRouterError::InvalidPeerGrantRequirement(name));
            }
            if let Some(subject) = route.matches.subject.as_deref()
                && subject.trim().is_empty()
            {
                return Err(ZapRouterError::EmptySubjectPattern(name));
            }
        }
        Ok(())
    }
...
impl RouteTarget {
    ...
    fn validate(&self, route: &str) -> Result<()> {
        ...
        if set == 1 {
            ...
            Err(ZapRouterError::InvalidTarget {
...
mod tests {
    ...
    fn rejects_peer_grant_requirement_without_peer_target() {
        ...
        assert_eq!(
            error,
            ZapRouterError::InvalidPeerGrantRequirement("bad-grant".to_string())
        );
    }
}
