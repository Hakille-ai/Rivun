...
use zap_envelope::{ZapEnvelope, ZapEnvelopeRef};

fn envelope(c: &mut Criterion) {
    let envelope = ZapEnvelope::action("device.echo", Bytes::from_static(b"benchmark-payload"))
        .unwrap()
    ...
    c.bench_function("zenv_action_parse", |b| {
        b.iter(|| ZapEnvelopeRef::parse(&encoded).unwrap())
    });
}
