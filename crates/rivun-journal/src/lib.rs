//! Binary append-only journal segments for Rivun local stores.
//!
//! `rivun-journal` is intentionally generic: receipts, memory, and future audit
...
const ENTRY_HASH_DOMAIN: &[u8] = b"Rivun-JOURNAL-ENTRY-v1";

pub const SEGMENT_EXTENSION: &str = "zjseg";
pub const INDEX_EXTENSION: &str = "zjidx";
pub const MANIFEST_EXTENSION: &str = "zjmanifest.json";

pub type Result<T> = std::result::Result<T, RivunJournalError>;

#[derive(Debug, Error)]
pub enum RivunJournalError {
...
impl JournalStore {
    ...
    pub fn seal_segment(&self, sequence: u64) -> Result<JournalSegmentManifest> {
        ...
        if !segment_path.exists() {
            return Err(RivunJournalError::MissingJournal(segment_path));
        }
    ...
    pub fn rotate_and_seal(&self) -> Result<JournalSegmentManifest> {
        ...
        let last = segments
            .last()
            .ok_or_else(|| RivunJournalError::MissingJournal(self.dir.clone()))?;
    ...
    pub fn load_manifest(&self, sequence: u64) -> Result<JournalSegmentManifest> {
        ...
        if !path.exists() {
            return Err(RivunJournalError::MissingJournal(path));
        }
    ...
    pub fn verify(&self) -> Result<JournalVerificationReport> {
        if !self.dir.exists() {
            return Err(RivunJournalError::MissingJournal(self.dir.clone()));
        }
    ...
    pub fn recover_partial_tail(&self) -> Result<Option<PartialTail>> {
        ...
        for segment in segments {
            ...
            if let Err(RivunJournalError::TruncatedRecord { path, offset }) = result {
    ...
    pub fn remove_dir_if_allowed(out: &Path, force: bool) -> Result<()> {
        if out.exists() {
            if !force {
                return Err(RivunJournalError::OutputExists(out.to_path_buf()));
            }
    ...
    fn segment_index_needs_rebuild(&self, segment: &SegmentInfo) -> Result<bool> {
        ...
        let last_entry = match read_last_index_entry(&index_path) {
            Ok(entry) => entry,
            Err(RivunJournalError::Json(_)) => return Ok(true),
    ...
    pub fn segments(&self) -> Result<Vec<SegmentInfo>> {
        ...
        for entry in fs::read_dir(&self.dir)? {
            ...
            if profile != self.profile {
                return Err(RivunJournalError::SegmentProfileMismatch {
                    path,
            ...
            if header_sequence != sequence {
                return Err(RivunJournalError::InvalidSegmentName { path });
            }
    ...
    fn read_record(
        ...
        if record.entry_hash != entry.entry_hash {
            return Err(RivunJournalError::InvalidEntryHash {
                ...
                &mut |record| {
                    if let Some(prev) = previous_hash.as_deref() {
                        if record.previous_entry_hash != hash_or_none(Some(prev)) {
                            return Err(RivunJournalError::HashChainMismatch {
                                path: segment.path.clone(),
...
fn read_segment_header(path: &Path) -> Result<(JournalProfile, u64, Uuid)> {
    ...
    if &magic != SEGMENT_MAGIC {
        return Err(RivunJournalError::InvalidSegmentMagic {
    ...
    F: FnMut(JournalRecord) -> Result<()>,
{
    ...
    while offset < bytes.len() {
        if bytes.len() - offset < 4 {
            ...
            return Err(RivunJournalError::TruncatedRecord {
                path: path.to_path_buf(),
...
fn encode_record(
    ...
    if metadata.len() > u32::MAX as usize {
        return Err(RivunJournalError::FieldTooLarge {
            field: "metadata",
    ...
    let len = u32::try_from(body.len()).map_err(|_| RivunJournalError::FieldTooLarge {
        field: "record",
        max: u32::MAX as usize,
    })?;
...
fn decode_record_at(
    ...
    if &magic != RECORD_MAGIC {
        return Err(RivunJournalError::InvalidRecordMagic {
            path: path.to_path_buf(),
            offset,
        });
    }
    ...
    if version != RECORD_VERSION {
        return Err(RivunJournalError::UnsupportedRecordVersion {
            path: path.to_path_buf(),
            offset,
            version,
        });
...
fn segment_sequence_from_path(path: &Path) -> Result<u64> {
    let stem = path
        ...
        .ok_or_else(|| RivunJournalError::InvalidSegmentName {
            path: path.to_path_buf(),
        })?;
    stem.parse::<u64>()
        .map_err(|_| RivunJournalError::InvalidSegmentName {
            path: path.to_path_buf(),
        })
}
...
mod tests {
    ...
    fn journal_detects_tampering() {
        ...
        assert!(matches!(
            store.verify(),
            Err(RivunJournalError::InvalidEntryHash { .. })
        ));
    }
