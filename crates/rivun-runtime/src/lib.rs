//! Sandboxed WebAssembly execution for Rivun action drivers.
//!
//! Driver ABI v1:
//! - export `memory`
...
pub use rivun_capability::{DEFAULT_MAX_HOST_CALL_BYTES, DriverPermissions};
...
pub(crate) const ALLOC_EXPORT: &str = "rivun_alloc";
pub(crate) const DEALLOC_EXPORT: &str = "rivun_dealloc";
pub(crate) const EXECUTE_EXPORT: &str = "rivun_execute";
pub(crate) const HOST_MODULE: &str = "rivun";
...
pub type Result<T> = std::result::Result<T, RivunRuntimeError>;
    ...
    pub fn execute(
        ...
        let instance = match linker.instantiate(&mut store, &driver.module) {
            ...
            Err(_error) if wall_clock_exceeded(started, limits.timeout_ms) => {
                return Err(RivunRuntimeError::Timeout {
...
pub(crate) fn expect_memory(module: &Module, export: &'static str) -> Result<()> {
    match module
        .get_export(export)
        .ok_or(RivunRuntimeError::MissingExport(export))?
    {
        ExternType::Memory(_) => Ok(()),
        actual => Err(RivunRuntimeError::InvalidExportType {
            export,
            expected: "memory",
        ...
        .ok_or(RivunRuntimeError::MissingExport(export))?
    {
        ExternType::Func(func) => {
                ...
                && val_types_equal(&results, expected_results)
            {
                ...
                Err(RivunRuntimeError::InvalidFunctionSignature {
                    export,
                    expected,
...
fn execute_instance(
    ...
    let memory = instance
        ...
        .ok_or(RivunRuntimeError::MissingExport(MEMORY_EXPORT))?;
    let alloc = instance
        .get_typed_func::<i32, i32>(&mut *store, ALLOC_EXPORT)
        .map_err(|_| RivunRuntimeError::MissingExport(ALLOC_EXPORT))?;
    let dealloc = instance
        .get_typed_func::<(i32, i32), ()>(&mut *store, DEALLOC_EXPORT)
        .map_err(|_| RivunRuntimeError::MissingExport(DEALLOC_EXPORT))?;
    let execute = instance
        .get_typed_func::<(i32, i32, i32, i32), i64>(&mut *store, EXECUTE_EXPORT)
        .map_err(|_| RivunRuntimeError::MissingExport(EXECUTE_EXPORT))?;
    ...
    if result_len_usize > limits.max_output_bytes {
        return Err(RivunRuntimeError::OutputTooLarge {
...
fn write_memory(
    ...
    if ptr < 0 {
        return Err(RivunRuntimeError::InvalidPointer {
            ptr: ptr as u32,
    ...
    memory
        ...
        .map_err(|err| RivunRuntimeError::MemoryAccess(err.to_string()))
}
...
fn read_memory(
    ...
    let end = start
        ...
        .ok_or(RivunRuntimeError::InvalidPointer { ptr, len })?;
    if end > memory.data_size(&mut *store) {
        return Err(RivunRuntimeError::InvalidPointer { ptr, len });
    }
...
pub(crate) fn ensure_i32_len(len: usize) -> Result<()> {
    if len > i32::MAX as usize {
        return Err(RivunRuntimeError::InputTooLarge(len));
    }
    Ok(())
}
...
pub(crate) fn validate_permissions(permissions: DriverPermissions) -> Result<()> {
    if permissions.max_host_call_bytes == 0 {
        return Err(RivunRuntimeError::InvalidHostCallLimit);
    }
    if permissions.network {
        return Err(RivunRuntimeError::PermissionDenied("network"));
    }
    if permissions.filesystem {
        return Err(RivunRuntimeError::PermissionDenied("filesystem"));
    }
    if permissions.clock {
        return Err(RivunRuntimeError::PermissionDenied("clock"));
    }
    if permissions.environment {
        return Err(RivunRuntimeError::PermissionDenied("environment"));
    }
    Ok(())
}
...
mod tests {
    ...
    fn invalid_abi_is_not_cached() {
        ...
        assert!(matches!(
            ...
            Err(RivunRuntimeError::MissingExport("rivun_execute"))
        ));
    ...
    fn rejects_bad_execute_signature() {
        let wasm = wat::parse_str(
            ...
            (module
              ...
              (func (export "rivun_alloc") (param i32) (result i32) i32.const 0)
              (func (export "rivun_dealloc") (param i32 i32))
              (func (export "rivun_execute") (param i32 i32 i32 i32) (result i32) i32.const 0))
            "#,
        )
        .unwrap();
        let executor = WasmExecutor::new().unwrap();
        ...
        assert!(matches!(
            error,
            RivunRuntimeError::InvalidFunctionSignature {
                export: "rivun_execute",
                ..
            }
        ));
    ...
    fn rejects_ungranted_permissions() {
        ...
        assert!(matches!(
            ...
            Err(RivunRuntimeError::PermissionDenied("network"))
        ));
    }
    ...
    fn host_emit_event_records_auditable_call_when_granted() {
        let wasm = wat::parse_str(
            r#"
            (module
              (import "rivun" "emit_event" (func $emit_event (param i32 i32) (result i32)))
              (memory (export "memory") 1)
              (data (i32.const 1024) "machine-ready")
              (data (i32.const 2048) "ok")
              (func (export "rivun_alloc") (param i32) (result i32) i32.const 3072)
              (func (export "rivun_dealloc") (param i32 i32))
              (func (export "rivun_execute") (param i32 i32 i32 i32) (result i64)
                i32.const 1024
                i32.const 13
                call $emit_event
                drop
    ...
    fn host_call_byte_limit_is_enforced() {
        let wasm = wat::parse_str(
            r#"
            (module
              (import "rivun" "device_call" (func $device_call (param i32 i32) (result i32)))
              (memory (export "memory") 1)
    ...
    fn memory_limit_is_enforced() {
        let wasm = wat::parse_str(
            ...
            (module
              ...
              (func (export "rivun_alloc") (param i32) (result i32) i32.const 0)
              (func (export "rivun_dealloc") (param i32 i32))
              (func (export "rivun_execute") (param i32 i32 i32 i32) (result i64) i64.const 0))
            "#,
        )
        .unwrap();
        let executor = WasmExecutor::new().unwrap();
    ...
    fn executes_wat_text_driver() {
        let wat = r#"
            (module
              ...
              (func (export "rivun_alloc") (param i32) (result i32) i32.const 2048)
              (func (export "rivun_dealloc") (param i32 i32))
              (func (export "rivun_execute") (param i32 i32 i32 i32) (result i64)
                i64.const 1024
                i64.const 32
    ...
    fn wall_clock_timeout_interrupts_long_running_driver() {
        ...
        assert!(matches!(result, Err(RivunRuntimeError::Timeout { .. })));
