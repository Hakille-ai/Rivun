...
use crate::{
    DriverPermissions, ExecutionLimits, HostCallKind, HostCallRecord, Result, RivunRuntimeError,
...
const ALLOC_EXPORT: &str = "rivun_alloc";
const DEALLOC_EXPORT: &str = "rivun_dealloc";
const EXECUTE_EXPORT: &str = "rivun_execute";
const HOST_MODULE: &str = "rivun";
...
impl AsyncWasmExecutor {
    ...
    pub async fn execute_async(
        ...
        let action_len: i32 = action
            .len()
            .try_into()
            .map_err(|_| RivunRuntimeError::InputTooLarge(action.len()))?;
        let payload_len: i32 = payload
            .len()
            .try_into()
            .map_err(|_| RivunRuntimeError::InputTooLarge(payload.len()))?;
        ...
        match tokio::time::timeout(Duration::from_millis(timeout_ms), fut).await {
            Ok(res) => res,
            Err(_) => Err(RivunRuntimeError::Timeout {
                limit_ms: timeout_ms,
            }),
        }
    }
    ...
    async fn execute_internal(
        ...
        memory
            ...
            .map_err(|e| RivunRuntimeError::MemoryAccess(e.to_string()))?;
        memory
    ...
    fn bind_host_functions(&self, linker: &mut Linker<AsyncStoreState>) -> Result<()> {
        // rivun.emit_event(ptr: i32, len: i32) -> i32
        linker.func_wrap(
            HOST_MODULE,
            HOST_EMIT_EVENT,
        ...
        // rivun.memory_read(key_ptr: i32, key_len: i32, out_ptr: i32, out_len: i32) -> i32
        linker.func_wrap(
            HOST_MODULE,
            HOST_MEMORY_READ,
        ...
        // rivun.memory_write(ptr: i32, len: i32) -> i32
        linker.func_wrap(
            HOST_MODULE,
            HOST_MEMORY_WRITE,
            |mut caller: Caller<'_, AsyncStoreState>, ptr: i32, len: i32| -> i32 {
        ...
        // rivun.device_call(ptr: i32, len: i32) -> i32
        linker.func_wrap(
            HOST_MODULE,
            HOST_DEVICE_CALL,
            |mut caller: Caller<'_, AsyncStoreState>, ptr: i32, len: i32| -> i32 {
        ...
        // rivun.async_stream_read(stream_id: i32, ptr: i32, max_len: i32) -> i32
        ...
        // rivun.async_stream_write(stream_id: i32, ptr: i32, len: i32) -> i32
        linker.func_wrap_async(
            HOST_MODULE,
        ...
        // rivun.async_device_call(port: i32, cmd_ptr: i32, cmd_len: i32, out_ptr: i32, max_out_len: i32) -> i32
        linker.func_wrap_async(
            HOST_MODULE,
...
(module
  ...
  (func (export "rivun_alloc") (param i32) (result i32) (i32.const 1024))
...
(module
  (import "rivun" "async_stream_read" (func $stream_read (param i32 i32 i32) (result i32)))
  (import "rivun" "async_stream_write" (func $stream_write (param i32 i32 i32) (result i32)))
  (memory (export "memory") 1)
...
"#;
    ...
    async fn test_async_wasm_timeout() {
        ...
        assert!(matches!(
            err,
            RivunRuntimeError::Timeout { .. } | RivunRuntimeError::Wasmtime(_)
        ));
    }
