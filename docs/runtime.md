...
rivun drivers use ABI v1:

```wat
(memory (export "memory") 1)
(func (export "@@rivun_HEADER@@alloc") (param i32) (result i32))
(func (export "@@rivun_HEADER@@dealloc") (param i32 i32))
...
`@@rivun_HEADER@@execute` returns `(result_ptr << 32) | result_len`.
...
When a driver config includes `manifest = "..."`, `check-config` also verifies the signed RivunStore manifest. The manifest must match the configured action, the local driver hash, the supported ABI version, and the author signature.
...
The async executor preserves ABI v1 and the core `rivun` host-import signatures,
...
environment access. ABI v2 foundations add a small `rivun` import module for
...
`memory_write` is committed by `rivun-node` only when `[memory] dir` is set and
`allow_driver_write = true`.

`rivun-capability` is the source of truth for permission declarations and
